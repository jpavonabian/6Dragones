...
void do_name_generator( CHAR_DATA *ch, char *argument )
{
    ...
    if ( infile == NULL ) {
        send_to_char( "No se ha podido encontrar el archivo de ese nombre generado.\r\n", ch );
        return;
    }
...
void name_generator( char *argument )
{
    ...
    infile = FileOpen( NAMEGEN_FILE, "r" );
    if ( infile == NULL ) {
        log_string( "No se ha podido encontrar el archivo de ese nombre generado." );
        return;
    }
...
void pick_name( char *argument, char *filename )
{
    ...
    if ( infile == NULL ) {
        log_printf( "No se ha podido encontrar %s", filename );
        return;
    }
...
void save_auth_list( void )
{
    ...
    if ( ( fpout = FileOpen( AUTH_FILE, "w" ) ) == NULL ) {
        bug( "%s", "No se ha podido abrir auth.dat para escribir." );
        perror( AUTH_FILE );
        return;
    }
...
void check_auth_state( CHAR_DATA *ch )
{
    ...
    if ( old_auth->state == AUTH_OFFLINE               /* checking as they enter the game
        ...
        ch_printf( ch,
                   "&R\r\nLos Administradores han decidido que el nombre %s\r\n"
                   "Es inaceptable para el juego. Deberás cambiarte el nombre.\r\n"
                   "El nombre que elijas deberá ser medieval y original.\r\n"
                   "No se permiten insultos o agresiones de cualquier tipo en referencia\r\n"
                   "a algún miembro de la Administración o jugador. Mira la 'ayuda nombre'.\r\n", ch->name );
    }
    else if ( old_auth->state == AUTH_DENIED ) {
        set_char_color( AT_RED, ch );
        send_to_char( "Se te ha eliminado del juego por elegir un nombre inapropiado.\r\n", ch );
        ...
        ch_printf( ch,
                   "\r\n&GLos Administradores han aceptado el nombre %s.\r\n"
                   "Ya puedes jugar a %s sin restricciones.\r\n", ch->name, sysdata.mud_name );
...
void do_authorize( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !arg1 || arg1[0] == '\0' ) {
        send_to_char( "Para aprobar un nombre esperando autorización: aut <nombre> <aceptar>\r\n", ch );
        send_to_char( "Para denegar un nombre esperando autorización:    aut <nombre> denegar\r\n", ch );
        send_to_char( "Para pedir el cambio de un nombre esperando autorización: aut <nombre> cambiar\r\n", ch );
        send_to_char( "Para limpiar la lista de autorizaciones: aut fixlist\r\n", ch );

        set_char_color( AT_DIVIDER, ch );
        send_to_char( "\r\n--- Personajes esperando autorización ---\r\n", ch );
    ...
    if ( !str_cmp( arg1, "fixlist" ) ) {
        send_to_pager( "Limpiando lista de autorización...\r\n", ch );
        clear_auth_list(  );
        send_to_pager( "Listo.\r\n", ch );
        return;
    }
    ...
    if ( auth != NULL ) {
        if ( auth->state == AUTH_OFFLINE || auth->state == AUTH_LINK_DEAD ) {
            offline = TRUE;
            if ( arg2[0] == '\0' || !str_cmp( arg2, "aceptar" ) || !str_cmp( arg2, "si" ) ) {
                auth->state = AUTH_AUTHED;
                auth->authed_by = QUICKLINK( ch->name );
                save_auth_list(  );
                snprintf( buf, MSL, "%s: authorized", auth->name );
                to_channel( buf, "Auth", level );
                ch_printf( ch, "Has autorizado a %s.\r\n", auth->name );
                return;
            }
            else if ( !str_cmp( arg2, "no" ) || !str_cmp( arg2, "denegar" ) ) {
                auth->state = AUTH_DENIED;
                auth->denied_by = QUICKLINK( ch->name );
                save_auth_list(  );
                snprintf( buf, MSL, "%s: Nombre denegado", auth->name );
                to_channel( buf, "Auth", level );
                ch_printf( ch, "Has denegado el nombre %s.\r\n", auth->name );
                /*
                 * Addition so that denied names get added to reserved list - Samson 10-18-98
                 */
                snprintf( buf, MSL, "%s añadir", auth->name );
                do_reserve( ch, buf );                 /* Samson 7-27-98 */
                return;
            }
            else if ( !str_cmp( arg2, "rechazar" ) || !str_cmp( arg2, "cambiar" ) ) {
                auth->state = AUTH_CHANGE_NAME;
                auth->change_by = QUICKLINK( ch->name );
                save_auth_list(  );
                snprintf( buf, MSL, "%s: deberá cambiarse el nombre", auth->name );
                to_channel( buf, "Auth", level );
                ch_printf( ch, "Pides a %s que cambie su nombre.\r\n", auth->name );
                /*
                ...
                send_to_char( "Argumento inválido.\r\n", ch );
                return;
            }
        }
        else {
            ...
            if ( arg2[0] == '\0' || !str_cmp( arg2, "aceptar" ) || !str_cmp( arg2, "si" ) ) {
                ...
                snprintf( buf, MSL, "%s: autorizado", victim->name );
                to_channel( buf, "Auth", level );
                ch_printf( ch, "Has autorizado a %s.\r\n", victim->name );
                ch_printf( victim,
                           "\r\n&GLos Administradores han aceptado el nombre %s.\r\n"
                ...
                send_to_char( "&RSe te ha eliminado del juego por elegir un nombre inapropiado.\r\n", victim );
                snprintf( buf, MSL, "%s: Nombre denegado", victim->name );
                to_channel( buf, "Auth", level );
                ch_printf( ch, "Has denegado el nombre %s.\r\n", victim->name );
                remove_from_auth( victim->name );
                /*
                 ...
                 */
                snprintf( buf, MSL, "%s añadir", victim->name );
                do_reserve( ch, buf );
#ifdef SAMSONRENT
                if ( ch->level == 1 )
                    do_quit( ch, ( char * ) "yes" );
                ...
                auth->change_by = QUICKLINK( ch->name );
                save_auth_list(  );
                snprintf( buf, MSL, "%s: deberá cambiar su nombre", victim->name );
                to_channel( buf, "Auth", level );
                ch_printf( victim,
                           "&R\r\nLos Administradores han decidido que el nombre %s es inaceptable para el juego.\r\n"
        ...
        send_to_char( "No hay jugadores esperando autorización.\r\n", ch );
        return;
    }
}
...
void do_name( CHAR_DATA *ch, char *argument )
{
    ...
    if ( auth_name == NULL ) {
        send_to_char( "No puedes cambiarte el nombre una vez autorizado.\r\n", ch );
        return;
    }
    if ( auth_name->state < AUTH_CHANGE_NAME ) {
        send_to_char( "No puedes cambiarte el nombre hasta que un administrador te de permiso.\r\n", ch );
        return;
    }

    argument[0] = UPPER( argument[0] );
    if ( !check_parse_name( argument, FALSE ) ) {
        send_to_char( "¿Cambiarte el nombre a qué?\r\n", ch );
return;
}
    ...
    if ( !str_cmp( ch->name, argument ) ) {
        send_to_char( "¡Ese ya es tu nombre!\r\n", ch );
        return;
    }
    ...
    if ( tmp ) {
        send_to_char( "Ese nombre ya ha sido escogido. Por favor, elige otro.\r\n", ch );
        return;
    }
    ...
    send_to_char( "Tu nombre ha sido cambiado. Deberás esperar a que sea aprobado para poder jugar, o rechazado para elegir otro.\r\n", ch );
    log_printf( "%s se ha cambiado el nombre a %s.", auth_name->name, ch->name );
...
void do_mpapplyb( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !victim->desc ) {
        send_to_char( "No en links caídos.\r\n", ch );
        return;
    }
    ...
    if ( !IS_IMMORTAL( victim ) ) {
        ...
        snprintf( buf, MSL, "Please welcome new player %s to the realms of Six Dragons!",
                  victim->name );
        announce( buf );
        act( AT_WHITE, "¡$n entra a este mundo en una blanca columna de luz cegadora!", victim, NULL,
             NULL, TO_ROOM );
    ...
    if ( NOT_AUTHED( victim ) ) {
        snprintf( buf, MSL, "%s@%s Nuevo jugador entrando al juego.\r\n", victim->name,
                  victim->desc->host );
        to_channel( buf, "Auth", level );
    }
    if ( ch->pcdata )
...
void auth_update( void )
{
    ...
    strcpy( showbuf, "--- Personajes esperando autorización ---\r\n" );
    for ( auth = first_auth_name; auth; auth = auth->next ) {
        if ( auth != NULL && auth->state < AUTH_CHANGE_NAME ) {
            snprintf( buf, MIL, "Name: %s      Estado: %s\r\n", auth->name,
                      ( auth->state == AUTH_ONLINE ) ? "En línea" : "Desconectado" );
...
void do_reserve( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !*arg ) {
        ...
        send_to_char
            ( "Para añadir un nombre: reservar <nombre> añadir\r\nPara quitar un nombre de la lista: reservar <nombre> <quitar>\r\nPara mostrar la lista de nombres reservados: <reservar> <mostrar>\r\n",
              ch );
return;
    }
    ...
    if ( !str_cmp( arg2, "quitar" ) ) {
        for ( res = first_reserved; res; res = res->next )
            if ( !str_cmp( arg, res->name ) ) {
                ...
                DISPOSE( res );
                save_reserved(  );
                send_to_char( "Nombre quitado. Ya no está reservado.\r\n", ch );
                return;
            }
        ch_printf( ch, "El nombre %s no está en la lista de reservados.\r\n", arg );
        return;

    }

    if ( !str_cmp( arg2, "añadir" ) ) {
        for ( res = first_reserved; res; res = res->next )
            if ( !str_cmp( arg, res->name ) ) {
                ch_printf( ch, "El nombre %s ya estaba reservado.\r\n", arg );
                return;
            }
