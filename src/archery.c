...
void lodge_projectile( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    switch ( projectile_pchance ) {
        ...
        case 3:                                       /* Hit in the arm */
            ...
            act( AT_ACTION, "El proyectil se clava en tu brazo.", ch, NULL, victim, TO_VICT );
            act( AT_ACTION, "El proyectil se clava en su brazo.", ch, NULL, victim, TO_CHAR );
            break;
        ...
        case 6:                                       /* Hit in the leg */
            ...
            act( AT_ACTION, "el proyectil se te clava en la pierna.", ch, NULL, victim, TO_VICT );
            act( AT_ACTION, "El proyectil se clava en su pierna.", ch, NULL, victim, TO_CHAR );
            break;
        ...
        case 10:                                      /* Hit in the chest */
            ...
            act( AT_ACTION, "El proyectil se te clava en una costilla.", ch, NULL, victim, TO_VICT );
            act( AT_ACTION, "El proyectil se clava en su costilla.", ch, NULL, victim, TO_CHAR );
            break;
    }
}
...
void do_draw( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( bow = get_eq_char( ch, WEAR_MISSILE_WIELD ) ) == NULL ) {
        send_to_char( "¡No estás empuñando ningún arco!\r\n", ch );
        return;
    }

    if ( ( quiver = find_quiver( ch ) ) == NULL ) {
        send_to_char( "¡No estás usando ningún carcaj!\r\n", ch );
        return;
    }
    ...
    if ( get_eq_char( ch, WEAR_WIELD ) != NULL )
        hand_count++;
    if ( hand_count > 1 ) {
        send_to_char( "necesitas una mano libre para disparar.\r\n", ch );
        return;
    }

    if ( get_eq_char( ch, WEAR_HOLD ) != NULL ) {
        send_to_char( "¡Necesitas una mano libre!\r\n", ch );
        return;
    }

    if ( ( arrow = find_projectile( ch, quiver ) ) == NULL ) {
        send_to_char( "¡Tu carcaj no tiene flechas!!\r\n", ch );
        return;
    }
    ...
    if ( bow->value[6] != arrow->value[4] ) {
        send_to_char( "¡No tienes el tipo de proyectil adecuado para este arma!\r\n", ch );
        obj_from_obj( arrow );
    ...
    act_printf( AT_ACTION, ch, quiver, NULL, TO_ROOM, "$n dispara hacia %s con $p.", arrow->short_descr );
    act_printf( AT_ACTION, ch, quiver, NULL, TO_CHAR, "Disparas a %s con $p.", arrow->short_descr );
...
void do_dislodge( CHAR_DATA *ch, char *argument )
{
    ...
    if ( argument[0] == '\0' ) {                       /* empty */
        send_to_char( "¿Extraer qué?\r\n", ch );
        return;
    }

    if ( get_eq_char( ch, WEAR_LODGE_RIB ) != NULL ) {
        arrow = get_eq_char( ch, WEAR_LODGE_RIB );
        act( AT_CARNAGE, "De un brusco tirón sacas $p de tu costilla, gritando de dolor.", ch, arrow, NULL,
             TO_CHAR );
        act( AT_CARNAGE, "$n saca con un brusco tirón y un grito de dolor $p de su costilla.", ch, arrow, NULL,
             TO_ROOM );
        ...
        act( AT_CARNAGE, "De un brusco tirón sacas $p de tu brazo, gritando de dolor.", ch, arrow, NULL,
             TO_CHAR );
        act( AT_CARNAGE, "$n saca con un brusco tirón y un grito de dolor $p de su brazo.", ch, arrow, NULL,
             TO_ROOM );
        unequip_char( ch, arrow );
        ...
        act( AT_CARNAGE, "De un brusco tirón sacas $p de tu pierna, gritando de dolor.", ch, arrow, NULL,
             TO_CHAR );
        act( AT_CARNAGE, "$n saca con un brusco tirón y un grito de dolor $p de su pierna.", ch, arrow, NULL,
             TO_ROOM );
        unequip_char( ch, arrow );
        ...
        send_to_char( "No tienes proyectiles clavados en tu cuerpo.\r\n", ch );
        return;
    }
}
                          ...
                          short dist, short dt, char *stxt, short color )
{
    ...
    if ( IS_SET( ch->in_room->room_flags, ROOM_SAFE ) ) {
         ...
         */
        if ( projectile ) {
            ch_printf( ch, "Tu %s se desintegra en el aire.",
                       myobj( projectile ) );
            act( color, "¡$p se desintegra!", ch, projectile, NULL, TO_ROOM );
            if ( projectile->in_obj )
                obj_from_obj( projectile );
            if ( projectile->carried_by )
                obj_from_char( projectile );
            ...
            ch_printf( ch, "%s se desintegra.", stxt );
            act( color, "¡$t se desintegra!", ch, aoran( stxt ), NULL, TO_ROOM );
        }
        return rNONE;
    }
                      ...
                      short dt, short range )
{
    ...
    if ( ( pexit = find_door( ch, arg, TRUE ) ) == NULL ) {
        if ( ( victim = get_char_room( ch, arg ) ) == NULL ) {
            ...
            if ( who_fighting( ch ) == victim ) {
                send_to_char( "¡Estás demasiado cerca!\r\n", ch );
                return rNONE;
            }
        }
    }
    ...
    if ( !victim ) {
        if ( IS_SET( ch->in_room->room_flags, ROOM_PRIVATE )
            ...
            send_to_char( "Esa sala es pribada.\r\n", ch );
            return rNONE;
        }
        if ( ch->in_room->tunnel > 0 ) {
            ...
            if ( count >= ch->in_room->tunnel ) {
                send_to_char( "No puedes disparar hacia esa sala.\r\n", ch );
                return rNONE;
            }
        }
    }
    ...
    if ( pexit && !pexit->to_room ) {
        send_to_char( "¡Le vas a disparar a una pared!?\r\n", ch );
        return rNONE;
    }
    ...
    if ( pexit && IS_SET( pexit->exit_info, EX_CLOSED ) ) {
        if ( IS_SET( pexit->exit_info, EX_SECRET ) || IS_SET( pexit->exit_info, EX_DIG ) )
            send_to_char( "¡Vas a disparar a una pared!?\r\n", ch );
        else
            send_to_char( "¡Vas a disparar a una puerta!?\r\n", ch );
        return rNONE;
    }
    ...
    if ( pexit && arg1[0] != '\0' ) {
        if ( ( vch = scan_for_victim( ch, pexit, arg1 ) ) == NULL ) {
            send_to_char( "No ves a tu objetivo.\r\n", ch );
            return rNONE;
        }
        ...
        if ( IS_SET( vch->in_room->room_flags, ROOM_NOMISSILE ) ) {
            send_to_char( "No puedes conseguir un tiro limpio.\r\n", ch );
            return rNONE;
        }
        ...
        if ( vch->num_fighting > max_fight( vch ) ) {
            send_to_char( "Hay mucha actividad para conseguir un tiro limpio.\r\n", ch );
            return rNONE;
        }
    }
    if ( vch ) {
    ...
    if ( projectile ) {
        ...
        if ( pexit ) {
            if ( weapon ) {
                act( AT_GREEN, "&GDisparass $p hacia $T.", ch, projectile, dir_name[dir], TO_CHAR );
                act( AT_GREEN, "&G$n dispara $p hacia $T.", ch, projectile, dir_name[dir], TO_ROOM );
            }
            else {
                act( AT_GREEN, "&GLanzas $p hacia $T.", ch, projectile, dir_name[dir], TO_CHAR );
                act( AT_GREEN, "&G$n lanza $p hacia $T.", ch, projectile, dir_name[dir], TO_ROOM );
            }
        }
        else {
            if ( weapon ) {
                act( AT_GREEN, "&GDisparas $p a $N.", ch, projectile, victim, TO_CHAR );
                act( AT_GREEN, "&G$n dispara $p a $N.", ch, projectile, victim, TO_NOTVICT );
                act( AT_GREEN, "&G¡$n te dispara $p!", ch, projectile, victim, TO_VICT );
            }
            else {
                act( AT_GREEN, "&GLanzas $p a $N.", ch, projectile, victim, TO_CHAR );
                act( AT_GREEN, "&G$n lanza $p a $N.", ch, projectile, victim, TO_NOTVICT );
                act( AT_GREEN, "&G¡$n te lanza $p!", ch, projectile, victim, TO_VICT );
            }
        }
    }
    else if ( skill ) {
        ...
        if ( skill->type == SKILL_SPELL ) {
            color = AT_MAGIC;
            if ( pexit ) {
                act( AT_MAGIC, "Lanzas $t hacia $T.", ch, aoran( stxt ), dir_name[dir], TO_CHAR );
                act( AT_MAGIC, "$n lanza $t hacia $T.", ch, stxt, dir_name[dir], TO_ROOM );
            }
            else {
                act( AT_MAGIC, "Lanzas $t a $N.", ch, aoran( stxt ), victim, TO_CHAR );
                act( AT_MAGIC, "$n lanza $t a $N.", ch, stxt, victim, TO_NOTVICT );
                act( AT_MAGIC, "¡$n te lanza $t!", ch, stxt, victim, TO_VICT );
            }
        }
    }
    else {
    ...
    while ( dist <= range ) {
        ...
        if ( IS_SET( pexit->exit_info, EX_CLOSED ) ) {
            ...
            if ( projectile ) {
                act_printf( color, ch, NULL, NULL, TO_CHAR,
                            "Ves %s clavarse en una puerta en el %s.",
                            myobj( projectile ), dir_name[dir] );
                act_printf( color, ch, projectile, NULL, TO_ROOM,
                            "$p vuela desdde %s y se clava en la puerta de %s.",
                            dtxt, dir_name[dir] );
                if ( projectile->in_obj )
                    obj_from_obj( projectile );
                if ( projectile->carried_by )
                ...
                act_printf( color, ch, NULL, NULL, TO_CHAR,
                            "Ves %s golpear una puerta del %s en la distancia.", stxt,
                            dir_name[dir] );
                snprintf( buf, MSL,
                          "%s vuela desde %s y se clava en la puerta del %s.",
        ...
        if ( victim && ch->in_room == victim->in_room ) {
            if ( projectile )
                act( color, "$p vuela desde $T.", ch, projectile, dtxt, TO_ROOM );
            else
                act( color, "$t vuela desde $T.", ch, aoran( stxt ), dtxt, TO_ROOM );
        ...
        if ( dist == range ) {
            if ( projectile ) {
                act( color, "$t vuela hacia $T y cae a la tierra sin causar daños.", ch,
                     myobj( projectile ), ( char * ) dir_name[dir], TO_CHAR );
                act( color, "$p vuela desde $y cae al suelo.", ch,
                ...
                act( color, "$t vuela hacia $T sin causar daños.", ch, ( char * ) stxt,
                     ( char * ) dir_name[dir], TO_CHAR );
                act( color, "$t vuela desde $T y cae al suelo.", ch,
        ...
        if ( ( pexit = get_exit( ch->in_room, dir ) ) == NULL ) {
            if ( projectile ) {
                act( color, "$t golpea una pared del $T y rebota sin hacer daño.",
                     ch, myobj( projectile ), ( char * ) dir_name[dir], TO_CHAR );
                act( color, "$p vuela desde $T y golpea una pared.", ch,
                ...
                act( color, "$t golpea una pared del $T.", ch, ( char * ) stxt,
                     ( char * ) dir_name[dir], TO_CHAR );
                act( color, "$t golpea una pared del $T y cae al suelo.", ch,
...
void do_fire( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( bow = get_eq_char( ch, WEAR_MISSILE_WIELD ) ) == NULL ) {
        send_to_char( "¡No estás blandiendo un arma de proyectiles!!\r\n", ch );
        return;
    }

    one_argument( argument, arg );
    if ( arg[0] == '\0' && ch->fighting == NULL ) {
        send_to_char( "¿Disparar a qué o quién?\r\n", ch );
        return;
    }
    ...
    if ( ( arrow = get_eq_char( ch, WEAR_HOLD ) ) == NULL ) {
        send_to_char( "¡No sostienes un proyectil!\r\n", ch );
        return;
    }
    ...
    if ( bow->value[6] != arrow->value[4] ) {
        ...
        switch ( bow->value[6] ) {
            case PROJ_BOLT:
                msg = "No tienes tornillos...\r\n";
                break;
            case PROJ_ARROW:
                msg = "No tienes flechas...\r\n";
                break;
            case PROJ_DART:
                msg = "No tienes dardos...\r\n";
                break;
            case PROJ_STONE:
                msg = "No tienes piedras...\r\n";
                break;
        }
        send_to_char( msg, ch );
        return;
