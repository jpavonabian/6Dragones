...
void do_activate( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ivnum == 18636 || ivnum == 1501 ) {
        ...
        if ( !( room = get_room_index( rvnum ) ) ) {
            send_to_char( "No te podemos enviar allí. Por favor informa a un administrador.\r\n", ch );
            return;
        }

        send_to_char( "La máquina del tiempo comienza a funcionar.\r\n", ch );
        char_from_room( ch );
    ...
    if ( ch->level < 5 && ch->pcdata->portalstone > 1 ) {
        send_to_char( "El portal de piedra está recargando sus energías mágicas.\r\n", ch );
    ...
    if ( ch->level < 5 && ch->pcdata->portalstone == 1 ) {
        if ( chance == 1 ) {
            send_to_char
                ( "el portal de piedra está recargando sus energías mágicas y estará listo muy pronto.\r\n",
                  ch );
            ch->pcdata->portalstone -= 1;
        }
        if ( chance == 2 ) {
    ...
    if ( !xIS_SET( ch->act, PLR_ACTIVATE ) ) {
        if ( !arg || arg[0] == '\0' ) {
            ...
            send_to_char
                ( "Sintaxis: Activar < nombre del lugar> - Te transporta al lugar elegido después de que el portal se active.\r\n",
                  ch );
            return;
        }
    }
    ...
    if ( !arg || arg[0] == '\0' ) {
        ...
        if ( ch->race == RACE_DRAGON ) {
            send_to_char( "&G12.Under the Volcano           10 - 20 [&RDragon Only Area&G]\r\n",
                          ch );
            send_to_char( "&cSintaxis: Activar < número de lugar ( 1 - 12 )>\r\n", ch );
        }
        else {
        ...
        // Only for the holidays
            ...
            send_to_char( "&cSintaxis: Activar < número de lugar ( 1 - 13 )>\r\n", ch );
        }
        return;
    }
    ...
    if ( !( room = get_room_index( rvnum ) ) ) {
        send_to_char( "No se te ha podido transportar ahí. Por favor avisa a alguien de la administración.\r\n", ch );
        return;
    }
    ...
    act( AT_ORANGE, "¡Una extraña luz de colores te envuelve!", ch, NULL, ch, TO_CHAR );
    act( AT_ORANGE, "¡A $n le envuelve una extraña luz de colores!", ch, NULL, ch, TO_ROOM );
    char_from_room( ch );
