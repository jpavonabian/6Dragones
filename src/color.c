...
int colorcode( const char *src, char *dst, DESCRIPTOR_DATA *d, int dstlen, int *vislen )
{
    ...
    dst[0] = '\0';                                     /* Initialize the the default
                                                        * NOTHING */
    ...
    switch ( *src ) {
        case '&':                                     /* NORMAL, Foreground colour */
            switch ( src[1] ) {
                ...
                case 'D':                             /* Reset to custom color for * * * 
                ...
                default:                              /* Unknown sequence, return all *
                                                        * * * * * * * * * the chars */
                    dst[0] = src[0];
                    dst[1] = src[1];
                    dst[2] = '\0';
        ...
        switch ( src[i] ) {
            ...
            case '}':                                 /* BLINK Foreground colour */
                *dst = '\0';
                ...
                break;                                 /* this was missing - if you have
                                                        * issues, remove it */
    ...
    *result = '\0';
    ...
    if ( txt && *txt && d ) {
        ...
        while ( ( colstr = strpbrk( prevstr, "&^}hH" ) ) != NULL ) {
            ...
            if ( colstr > prevstr ) {
                if ( ( MAX_STRING_LENGTH * 10 - ( reslen = strlen( result ) ) ) <=
                     ( colstr - prevstr ) ) {
                    bug( "%s: OVERFLOW in internal MAX_STRING_LENGTH buffer!",
                         __PRETTY_FUNCTION__ );
                    break;
                }
                strncat( result, prevstr, ( colstr - prevstr ) );   /* Leave this one
                                                                     * alone! BAD
                                                                     * THINGS(TM) will
                                                                     * happen if you
                                                                     * don't! */
                result[reslen + ( colstr - prevstr )] = '\0';   /* strncat will NOT NULL
                                                                 * terminate this! */
            }
...
void set_char_color( short AType, CHAR_DATA *ch )
{
    ...
    if ( !ch->desc ) {
        bug( "%s: NULL descriptor after send_to_char! CH: %s", __FUNCTION__,
...
void write_to_pager( DESCRIPTOR_DATA *d, const char *txt, unsigned int length )
{
    ...
    pageroffset = d->pagepoint - d->pagebuf;           /* pager fix (goofup fixed
                                                        * 08/21/97) */
    ...
    d->pagepoint = d->pagebuf + pageroffset;           /* pager fix (goofup fixed
                                                        * 08/21/97) */
    strncpy( d->pagebuf + d->pagetop, txt, length );   /* Leave this one alone! BAD
...
void set_pager_color( short AType, CHAR_DATA *ch )
{
    ...
    if ( !ch->desc ) {
        bug( "%s: NULL descriptor after send_to_pager! CH: %s", __FUNCTION__,
             ch->name ? ch->name : "Unknown?!?" );
        return;
    }
    ch->desc->pagecolor = ch->colors[AType];
...
void act_tell( CHAR_DATA *to, CHAR_DATA *teller, const char *format, CHAR_DATA *ch, void *arg1,
               void *arg2, int type )
{
