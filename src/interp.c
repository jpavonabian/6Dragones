            ...
            if ( ( ( !IS_SET( cmd->flags, CMD_FULLNAME ) && !str_prefix( command, cmd->name ) )
                 ...
                 && ( cmd->level <= trust
                      || ( !IS_NPC( ch ) && ch->pcdata->council
                           ...
                           && cmd->level <= ( trust + MAX_CPD ) ) || ( !IS_NPC( ch )
                                                                       && VLD_STR( ch->pcdata->
                                                                                   bestowments )
                                                                       && is_name( cmd->name,
                                                                                   ch->pcdata->
...
*/
    ...
    if ( ( !IS_NPC( ch ) && xIS_SET( ch->act, PLR_LOG ) ) || fLogAll || loglvl == LOG_BUILD
    ...
    if ( !found ) {
        ...
        if ( !local_channel_hook( ch, command, argument ) && !check_skill( ch, command, argument )
            ...
            if ( ( pexit = find_door( ch, command, TRUE ) ) != NULL
                 && ( IS_SET( pexit->exit_info, EX_xAUTO )
                      || ( IS_SET( pexit->exit_info, EX_AUTOFULL )
                           && !str_cmp( pexit->keyword, autofull ) ) ) ) {
                if ( IS_SET( pexit->exit_info, EX_CLOSED )
                     &&
                     ( ( !IS_AFFECTED( ch, AFF_PASS_DOOR ) && !xIS_SET( ch->act, PLR_SHADOWFORM ) )
                       || ( IS_SET( pexit->exit_info, EX_NOPASSDOOR ) || !IS_IMMORTAL( ch ) ) ) ) {
...
//      send_to_char("Huh? - Make sure you properly spelled your command.\r\n", ch);
            error( ch );
        }
        return;
    }
    ...
    if ( xIS_SET( ch->act, PLR_TEASE ) ) {
        if ( str_cmp( command, "north" ) && str_cmp( command, "east" ) && str_cmp( command, "west" )
             ...
             && str_cmp( command, "give" ) && str_cmp( command, "inv" )
             && str_cmp( command, "inventory" ) && str_cmp( command, "e" )
             && str_cmp( command, "w" ) && str_cmp( command, "n" ) && str_cmp( command, "s" ) ) {
...
bool check_social( CHAR_DATA *ch, char *command, char *argument )
{
    ...
    if ( !arg || arg[0] == '\0' ) {
        if ( !social->others_no_arg || !social->char_no_arg ) {
            ...
            bug( "%s: Incomplete Social %s, has NULL others_no_arg or NULL char_no_arg",
                 __FUNCTION__, social->name );
            return TRUE;
        }
        ...
        if ( !social->vict_found || !social->char_found || !social->others_found ) {
            send_to_char( "Incomplete social, try again soon.\r\n", ch );
            bug( "%s: Incomplete Social %s, NULL vict_found, NULL char_found or NULL others_found",
                 __FUNCTION__, social->name );
            return TRUE;
        }
        ...
        if ( !social->char_auto || !social->others_auto ) {
            send_to_char( "Incomplete social, try again soon.\r\n", ch );
            bug( "%s: Incomplete Social %s, NULL char_auto or NULL others_auto", __FUNCTION__,
                 social->name );
            return TRUE;
        }
        ...
        if ( !social->char_found || !social->others_found || !social->vict_found ) {
            send_to_char( "Incomplete social, try again soon.\r\n", ch );
            bug( "%s: Incomplete Social %s, NULL char_found, NULL others_found or NULL vict_found",
                 __FUNCTION__, social->name );
            return TRUE;
        }
        act( AT_SOCIAL, social->others_found, ch, NULL, victim, TO_NOTVICT );
        ...
        if ( !IS_NPC( ch ) && IS_NPC( victim ) && !IS_AFFECTED( victim, AFF_CHARM )
            ...
            switch ( number_bits( 4 ) ) {
                case 0:
                    if ( IS_EVIL( ch ) && !is_safe( victim, ch, TRUE ) )    /* was * * *
                                                                             * * * *
                                                                             * IS_EVIL(ch) 
                                                                             * * * * * *
                                                                             * * * * *
                        ...
                        act( AT_ACTION, "$n acts like $N doesn't even exist.", victim, NULL, ch,
                             TO_NOTVICT );
                        act( AT_ACTION, "You just ignore $N.", victim, NULL, ch, TO_CHAR );
                        act( AT_ACTION, "$n appears to be ignoring you.", victim, NULL, ch,
                             TO_VICT );
                    }
                    break;
...
void send_timer( struct timerset *vtime, CHAR_DATA *ch )
{
    ...
    ch_printf( ch, "Time (in secs): min %ld %ld avg: %ld %ld max %ld %ld"
               "\r\n", vtime->min_time.tv_sec, vtime->min_time.tv_usec, ntime.tv_sec, ntime.tv_usec,
               vtime->max_time.tv_sec, vtime->max_time.tv_usec );
    return;
}
...
char                   *check_cmd_flags( CHAR_DATA *ch, CMDTYPE * cmd )
{
    ...
    if ( !IS_NPC( ch ) && IS_IMMORTAL( ch ) && ( get_trust( ch ) < LEVEL_AJ_LT ) ) {
        if ( IS_SET( cmd->flags, CMD_ADMIN ) || IS_SET( cmd->flags, CMD_ENFORCER )
             || IS_SET( cmd->flags, CMD_BUILDER ) ) {
            bool                    fUseable = FALSE;

            if ( IS_SET( cmd->flags, CMD_ADMIN ) && IS_SET( ch->pcdata->flags, PCFLAG_ADMIN ) )
                fUseable = TRUE;

            if ( IS_SET( cmd->flags, CMD_ENFORCER )
                 && IS_SET( ch->pcdata->flags, PCFLAG_ENFORCER ) )
                fUseable = TRUE;
            ...
            if ( fUseable == FALSE )
                snprintf( cmd_flag_buf, MSL,
                          "%s is not a command allowed to you by your current status.\r\n",
                          cmd->name );
            else
                cmd_flag_buf[0] = '\0';
        }
