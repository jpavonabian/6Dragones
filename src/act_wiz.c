...
CHAR_DATA              *get_waiting_desc( CHAR_DATA *ch, char *name )
{
    ...
    if ( number_of_hits == 1 )
        ...
        send_to_char( "No hay nadie pendiente de autorizar.\r\n", ch );
        return NULL;
    }
}
    ...
    if ( !IS_NPC( ch ) ) {
        if ( argument[0] != '\0' )
            if ( !parse_for_n( argument ) ) {
                send_to_char( "La línea debe incluir $n\r\n", ch );
                return;
            }
        ...
        send_to_char( "&YTu bamfin ha cambiado.\r\n", ch );
    }
    return;
}
...
void do_bamfout( CHAR_DATA *ch, char *argument )
{
    if ( !IS_NPC( ch ) ) {
        ...
        send_to_char( "&YTu bamfout ha cambiado.\r\n", ch );
    }
    return;
}
...
void do_retire( CHAR_DATA *ch, char *argument )
{
    ...
    if ( get_trust( victim ) >= get_trust( ch ) ) {
        send_to_char( "&RNunca intentes usar este comando con un rango superior...\r\n", ch );
        send_to_char( "&REstás avisado....\r\n", ch );
        act( AT_RED, "$n ha intentado retirarte en contra de tus deseos.", ch, NULL, victim, TO_VICT );
        return;
    }
...
void do_ostat( CHAR_DATA *ch, char *argument )
{
    ...
    if ( obj->pIndexData->vnum == 16243 || obj->pIndexData->vnum == 16244
         ...
         || obj->pIndexData->vnum == 56106 || obj->pIndexData->vnum == 10967
         || obj->pIndexData->vnum == 16246 || obj->pIndexData->vnum == 11250 ) {
        ch_printf( ch, "&cCrafts: &w%d %d %d %d %d %d %d %d %d %d\r\n%d %d %d %d %d %d %d %d %d %d",
                   ...
                   obj->craft13, obj->craft14, obj->craft15, obj->craft16, obj->craft17,
                   obj->craft18, obj->craft19, obj->craft20 );
    }
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC( victim ) )
        ch_printf( ch, "&cClan &w%s\t&c Influence &w%d\t&c Slice Vnum&w %d\r\n", victim->clan,
                   victim->influence, victim->slicevnum );
    if ( !IS_NPC( victim ) ) {
...
ext_flag_string(&victim->affected_by2, a2_flags) : "Nothing"); */
    ...
    if ( victim->pcdata && victim->pcdata->ship )
        pager_printf( ch,
                      ...
                      victim->pcdata->ship, victim->pcdata->boatvnum, victim->pcdata->watervnum,
                      victim->pcdata->hull );
...
void do_users( CHAR_DATA *ch, char *argument )
{
    ...
    for ( d = first_descriptor; d; d = d->next ) {
        ...
        if ( ( d->original && !str_cmp( d->original->name, "Zlorth" ) )
             || ( d->character && !str_cmp( d->character->name, "Zlorth" ) ) )
   ...
   /******************************************************************/
    ...
    if ( !remove( buf ) ) {
        ...
        if ( !remove( buf ) )
            ...
            ch_printf( ch,
                       "Unknown error #%d - %s (staff data). Report to http://6d.genesismuds.com\r\n",
                       errno, strerror( errno ) );
        ...
        set_char_color( AT_WHITE, ch );
        ch_printf( ch, "Unknown error #%d - %s. Report to http://6d.genesismuds.com\r\n",
                   errno, strerror( errno ) );
...
void do_cset( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !str_cmp( arg, "happy" ) ) {
        ...
        if ( sysdata.happy ) {
            echo_to_all( AT_RED,
                         "[&WEvent&R] &CIt is time for 6Dragons HAPPY HOUR! Get killing!\r\n",
                         ECHOTAR_ALL );
            echo_to_all( AT_RED,
                         "Enjoy an hour of Double Experience when fighting, Double Experience when Crafting or Triple Experience if Grouped with other players.\r\n",
                         ECHOTAR_ALL );
        }
        else
...
void do_promote( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !str_cmp( arg2, "level" ) ) {
        ...
        victim->level = value;
        set_char_color( AT_WHITE, victim );
        send_to_char( "sientes que has alcanzado un nuevo poder...\r\n\r\n", victim );
        set_char_color( AT_LBLUE, victim );
        if ( victim->level < 100 ) {
            snprintf( buf, MIL,
                      "\r\n\r\n¡Dad la bienvenida al nuevo miembro de la administración de &rSeis Dragones &W%s!",
                      victim->name );
        ...
        send_to_char( "Ahora tienes a tu disposición todas las habilidades, todos los hechizos y puedes hablar en todas las lenguas.\r\n", victim );
        ...
        interpret( victim, ( char * ) "escuchar todos" );
        interpret( victim, ( char * ) "holylight" );
        interpret( victim, ( char * ) "shadowform" );
        xSET_BIT( victim->act, PLR_STAFF );
        send_to_char( "Teclea config staff para información de áreas.\r\n", victim );
        restore_char( victim );
        interpret( victim, ( char * ) "hablar todos" );
        interpret( victim, ( char * ) "titulo aprendiz de la administración de &r6 Dragones&D." );
        interpret( victim, ( char * ) "salvar" );
#ifdef I3
        victim->pcdata->i3chardata->i3perm = I3PERM_IMM;
        interpret( victim, ( char * ) "i3listen dchat" );
#endif
        make_stafflist(  );
        send_to_char( "\r\n&PEl canal principal de la administración es el inmortales.\r\n\r\n", victim );
        send_to_char( "\r\n&YPor favor añade tu email con el comando email, aunque no es necesario.\r\n\r\n",
                      victim );
        send_to_char
            ( "\r\n&CAhora tienes acceso al canal ichat y puedes hablar con los administradores de otros muds.\r\n",
              victim );
        send_to_char
            ( "\r\n&Rtienes &Y6 Dragones - como ser un administrador eficiente&R en tus manos.\r\n",
              victim );
        return;
    }
    return;
...
/*
if( puppet == NULL && pnum == -1 && ch->pcdata->alt1 =! NULL && ch->pcdata->alt2 =! NULL && ch->pcdata->alt3 =! NULL && ch->pcdata->alt4 =! NULL && ch->pcdata->alt5 =! NULL )
{
do_loadup( ch, ch->pcdata->alt1 );
puppet =  get_char_world( ch, ch->pcdata->alt1 );
do_loadup( ch, ch->pcdata->alt2 );
...
{
send_to_char("\r\nYou cannot use puppet load all if one of your alts is NULL.\r\n", ch);
return FALSE;
}
*/
    if ( pnum == 1 || pnum == -1 ) {
...
void do_puppeteer( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !str_cmp( arg, "load" ) ) {
        if ( !argument || argument[0] == '\0' ) {
            send_to_char( "Usage: puppeteer load (altname)\r\n       puppeteer load\r\n", ch );
            return;
        }
        if ( !str_cmp( argument, "all" ) ) {
            send_to_char
                ( "\r\nLoad all temporary disabled due to causing crashes with NULL values\r\n",
                  ch );
            return;
            /*
        ...
        if ( ch->pcdata->alt1 && !str_cmp( argument, ch->pcdata->alt1 )
             && ( exists_player( argument ) ) )
            puppetload( ch, 1 );
        if ( ch->pcdata->alt2 && !str_cmp( argument, ch->pcdata->alt2 )
             && ( exists_player( argument ) ) )
            puppetload( ch, 2 );
        if ( ch->pcdata->alt3 && !str_cmp( argument, ch->pcdata->alt3 )
             && ( exists_player( argument ) ) )
            puppetload( ch, 3 );
        if ( ch->pcdata->alt4 && !str_cmp( argument, ch->pcdata->alt4 )
             && ( exists_player( argument ) ) )
            puppetload( ch, 4 );
        if ( ch->pcdata->alt5 && !str_cmp( argument, ch->pcdata->alt5 )
             && ( exists_player( argument ) ) )
            puppetload( ch, 5 );
    ...
    if ( !str_cmp( arg, "quit" ) ) {
        ...
        else if ( !str_cmp( argument, ch->pcdata->alt1 ) && ch->pcdata->alt1 ) {
            puppet = get_char_world( ch, ch->pcdata->alt1 );
            if ( puppet && IS_PUPPET( puppet ) ) {
                REMOVE_BIT( puppet->pcdata->flags, PCFLAG_PUPPET );
                // d->character->desc->puppet = 0;
        ...
        else if ( !str_cmp( argument, ch->pcdata->alt2 ) && ch->pcdata->alt2 ) {
            puppet = get_char_world( ch, ch->pcdata->alt2 );
            if ( puppet && IS_PUPPET( puppet ) ) {
                REMOVE_BIT( puppet->pcdata->flags, PCFLAG_PUPPET );
                // d->character->desc->puppet = 0;
        ...
        else if ( !str_cmp( argument, ch->pcdata->alt3 ) && ch->pcdata->alt3 ) {
            puppet = get_char_world( ch, ch->pcdata->alt3 );
            if ( puppet && IS_PUPPET( puppet ) ) {
                REMOVE_BIT( puppet->pcdata->flags, PCFLAG_PUPPET );
                // d->character->desc->puppet = 0;
        ...
        else if ( !str_cmp( argument, ch->pcdata->alt4 ) && ch->pcdata->alt4 ) {
            puppet = get_char_world( ch, ch->pcdata->alt4 );
            if ( puppet && IS_PUPPET( puppet ) ) {
                REMOVE_BIT( puppet->pcdata->flags, PCFLAG_PUPPET );
                // d->character->desc->puppet = 0;
        ...
        else if ( !str_cmp( argument, ch->pcdata->alt5 ) && ch->pcdata->alt5 ) {
            puppet = get_char_world( ch, ch->pcdata->alt5 );
            if ( puppet && IS_PUPPET( puppet ) ) {
                REMOVE_BIT( puppet->pcdata->flags, PCFLAG_PUPPET );
                // d->character->desc->puppet = 0;
    ...
    send_to_char( "Usage: puppeteer alt1-5 (altname)\r\n", ch );
    send_to_char( "       puppeteer load/quit (all)\r\n", ch );
    send_to_char( "       switch (altname), force (altname) (command)\r\n", ch );
    ch_printf( ch, "Your current alts are: %s, %s, %s, %s, %s.\r\n", ch->pcdata->alt1,
               ch->pcdata->alt2, ch->pcdata->alt3, ch->pcdata->alt4, ch->pcdata->alt5 );
    return;
...
void do_slay( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !str_cmp( arg2, "immolate" ) ) {
        ...
        act( AT_FIRE, "$n releases a searing fireball in your direction.", ch, NULL, victim,
             TO_VICT );
        act( AT_FIRE, "$n points at $N, who bursts into a flaming inferno.", ch, NULL, victim,
             TO_NOTVICT );
    }

    else if ( !str_cmp( arg2, "shatter" ) ) {
        act( AT_LBLUE,
             "You freeze $N with a glance and shatter the frozen corpse into tiny shards.", ch,
             NULL, victim, TO_CHAR );
        act( AT_LBLUE,
             "$n freezes you with a glance and shatters your frozen body into tiny shards.", ch,
             NULL, victim, TO_VICT );
        ...
        act( AT_BLOOD, "$n reaches out with a clawed finger and calmly slits your throat.", ch,
             NULL, victim, TO_VICT );
...
void do_casar( CHAR_DATA *ch, char *argument)
{
    char arg1[MAX_INPUT_LENGTH];
    char arg2[MAX_INPUT_LENGTH];
    CHAR_DATA *victim;
    ...
    argument = one_argument( argument, arg2 );
       ...
       if ( arg1[0] == '\0' || arg2[0] == '\0' )
          {
             send_to_char("uso: casar <esposo> <esposa>\n\r",ch);
             return;
          }
       ...
       if ( ((victim = get_char_world(ch,arg1)) == NULL) ||
          ...
          {
          send_to_char("¡Los dos personajes deben estar jugando!\n\r", ch );
          return;
          }
       if ( IS_NPC(victim) || IS_NPC(victim2))
          {
          send_to_char("¡Un mob no puede ser casado!\n\r", ch);
          return;
          }

	if( !(victim->pcdata->pareja== NULL)) {
send_to_char("ya está casado.\r\n", ch);
return;
}
	if( !(victim2->pcdata->pareja== NULL)) {
...
return;
        ...
        {
    char                    buf[MSL];
            snprintf( buf, MSL, "decir %s, ¿aceptas a %s en la riqueza y en la pobreza, en la salud y en la enfermedad y prometes serle fiel hasta que la muerte os separe?", victim->name, victim2->name);
                interpret( ch, ( char * ) buf );
                interpret( victim, ( char * ) "decir Si, quiero" );
            snprintf( buf, MSL, "decir %s, ¿aceptas a %s en la riqueza y en la pobreza, en la salud y en la enfermedad y prometes serle fiel hasta que la muerte os separe?", victim2->name, victim->name);
                interpret( ch, ( char * ) buf );
...
/*       if (victim->level < 10 || victim2->level < 10)
          {
          send_to_char("No tienen nivel para casarse.\n\r", ch);
          return;
          }*/
...
void do_divorciar( CHAR_DATA *ch, char *argument)
   {
    char arg1[MAX_INPUT_LENGTH];
    char arg2[MAX_INPUT_LENGTH];
    CHAR_DATA *victim;
    ...
    argument = one_argument( argument, arg2 );
       ...
       if ( arg1[0] == '\0' || arg2[0] == '\0' )
          {
          send_to_char("uso: divorciar <esposo> <esposa>\n\r",ch);
          return;
          }
       if ( ((victim = get_char_world(ch,arg1)) == NULL) ||
            ((victim2 = get_char_world(ch,arg2)) == NULL))
          {
          send_to_char("¡Los dos jugadores tienen que estar jugando!\n\r", ch );
          return;
          }

       if ( IS_NPC(victim) || IS_NPC(victim2))
          {
          send_to_char("¡Deja en paz a los mobs!\n\r", ch);
          return;
          }

       if ( !str_cmp( victim->pcdata->pareja, victim2->name ) && !str_cmp( victim2->pcdata->pareja, victim->name ) )
/*       if (victim->pcdata->pareja!= victim2->name || victim2->pcdata->pareja!= victim->name)
          {
          send_to_char("¡No están casados!!\n\r",ch);
          return;
          } */
       {
       send_to_char("Ya no están casados.\n\r", ch);
       send_to_char("ya no estás casado.\n\r", victim);
       send_to_char("Ya no estás casada.\n\r", victim2);

       act(AT_WHITE,"$n y $N se han divorciado.",victim, NULL, victim2, TO_NOTVICT);

       victim->pcdata->pareja = str_dup( "" );
       victim2->pcdata->pareja = str_dup( "" );
                            STRFREE( victim->pcdata->pareja);
                            STRFREE( victim2->pcdata->pareja);
       return;
       }
       else
       {
       send_to_char("¡No están casados!", ch);
       return;
       }
}

void do_irpareja( CHAR_DATA *ch, char *argument)
{
	char buf[MSL];
    CHAR_DATA              *victim;
	...
	act( AT_RED, "&c$n &cimboca la fuerza de su amor y desaparece en una nube de color rosa.&D", ch, NULL, NULL, TO_ROOM );
	char_from_room( ch );
	char_to_room( ch, victim->in_room );
	act( AT_RED, "&c$n &cimboca la fuerza de su amor y aparece en una nube de color rosa.&D", ch, NULL, NULL, TO_ROOM );
	do_look( ch, "auto" );
	return;
}
