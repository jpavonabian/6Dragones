...
void check_water_mobs( CHAR_DATA *ch )
{
    ...
    if ( IS_AFFECTED( ch, AFF_NOSCENT ) ) {
        ...
        if ( chance < 6 ) {
            send_to_char
                ( "Ves una criatura a distancia, pero no puede detectar tu presencia.\r\n",
                  ch );
            return;
        }
    }
...
if(chance>55) {
send_to_char("Te obligas a parecer una víctima indefensa.\r\n", ch);
vnum=number_range(41030,41034);
    ...
    interpret( mob, ( char * ) "mpecho &b¡Una forma oscura nada de repente hacia ti!.&D" );
    interpret( mob, ( char * ) "mpecho" );
...
void check_random_mobs( CHAR_DATA *ch )
{
    ...
    if ( ( ch->in_room->sector_type == SECT_HROAD ) || ( ch->in_room->sector_type == SECT_VROAD )
        ...
        send_to_char
            ( "\r\nUna cuadrilla del ejército del rey cabalga garantizando que las turbas salvajes se mantengan fuera de las carreteras.\r\n",
              ch );
        return;
    }
    ...
    if ( IS_AFFECTED( ch, AFF_NOSCENT ) ) {
        ...
        if ( chance < 6 ) {
            send_to_char
                ( "Ves una criatura a distancia, pero no detecta tu presencia.\r\n",
                  ch );
            return;
        }
    }
   ...
   if ( ch->Class != CLASS_BEAST )
   {
    ...
    if ( IS_AFFECTED( ch, AFF_VICTIM ) ) {
        ...
        chance += lck_app[get_curr_lck( ch )].luck;
        if ( chance > 55 ) {
            send_to_char( "Te obligas a aparecer como una víctima indefensa.\r\n", ch );
    ...
    interpret( mob, ( char * ) "mpecho" );
    interpret( mob,
               ( char * ) "mpecho &w¡Se escucha un estruendo cuando tú eres envoscado de repente!&D" );
    interpret( mob, ( char * ) "mpecho" );
...
{
send_to_char("Comando desactivado por que no sirve para nada.\r\n", ch);
return;
...
// No tiny mobs here, talking giant squid tentacles, killer whale, giant sea turtle, pirates
    ...
    if ( mob->pIndexData->vnum == 41049 ) {
        interpret( mob, ( char * ) "mpasound &b¡Una nave invasora acaba de abordar tu barco!&D" );
        send_to_char( "&b¡Una nave invasora acaba de abordar tu barco!\r\n", ch );
    }
    else {
        interpret( mob, ( char * ) "mpasound &b¡Una forma oscura nada de repente hacia tu barco!&D" );
        send_to_char( "&B¡Una forma oscura nada de repente hacia tu barco!\r\n", ch );
    }
