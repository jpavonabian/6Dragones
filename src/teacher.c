...
void do_teacher( CHAR_DATA *ch, char *argument )
{
    ...
    for ( mob = ch->in_room->first_person; mob; mob = mob->next_in_room )
        if ( IS_NPC( mob )
            ...
            break;
    if ( !mob ) {
        send_to_char( "No hay maestro aquí.\r\n", ch );
        return;
    }

    if ( xIS_SET( mob->act, ACT_NEWBPRACTICE ) ) {
        act( AT_TELL, "\r\n&W$n te cuenta 'Puedo enseñarte cualquier hechizo o habilidad antes de nivel 20.", mob, NULL,
             ch, TO_VICT );
        send_to_char( "&WDespués del nivel 20, tendrás que buscar a los maestros.'\r\n", ch );
        return;
    }
    act( AT_TELL, "\r\n$n te cuenta 'Puedo enseñarte lo siguiente:'\r\n", mob, NULL, ch, TO_VICT );
...
void do_listteachers( CHAR_DATA *ch, char *argument )
{
     ...
     * check the world for an exact match 
     */
    send_to_char( "&CLa lista actual de maestros  y localizaciones en 6Dragones&D\r\n\r\n", ch );
    send_to_char( "&rnombre del maestro                  Vnum del maestro        Room de localización del maestro \r\n",
                  ch );
    send_to_char( "&R-----------------------------------------------------------------------\r\n",
                  ch );
    for ( wch = first_char; wch; wch = wch->next ) {
