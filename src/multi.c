...
bool IS_UNDERTWENTY( CHAR_DATA *ch, int sn )
{
    ...
    if ( IS_THIRDCLASS( ch ) ) {
        if ( skill_table[sn]->skill_level[ch->thirdclass] < 21
             || skill_table[sn]->skill_level[ch->secondclass] < 21
             || skill_table[sn]->skill_level[ch->Class] < 21 )
            return TRUE;
    }
    else if ( IS_SECONDCLASS( ch ) ) {
        if ( skill_table[sn]->skill_level[ch->secondclass] < 21
             || skill_table[sn]->skill_level[ch->Class] < 21 )
            return TRUE;
    }
...
bool CAN_LEARN( CHAR_DATA *ch, int sn, bool canlearnnow )
{
    ...
    if ( !IS_NPC( ch ) ) {
        ...
        if ( ch->pcdata->tradeclass == 21
             && ( sn == gsn_gather || sn == gsn_bake || sn == gsn_mix ) ) {
            return TRUE;
        }
