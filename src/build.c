 ...
 */
const char             *const ex_pmisc[] ={"indefinido",	"vórtice",	"vacio",	"resvalon",
	"hielo",		"misterio"
};

const char             *const ex_pwater[] ={
	"corriente",	"ola"	"remolino",	"geiser"
};
const char             *const ex_pair[] = {
	"viento",	"tormenta",	"ventisca",	"brisa"
};

const char             *const ex_pearth[] = { 	"desprendimiento",	"socabón",	"movedizas",	"terremoto"
};

const char             *const ex_pfire[] = {"lava",	"vapor"
};
const char             *const ex_flags[] = {
    "espuerta", "cerrada", "bloqueada", "secreta", "nadar", "noforzable", "volar", "escalable",
    "cavable", "eatkey", "traspasar", "oculta", "pasaje", "portal", "r1", "r2",
    "puede_escalarse", "puede_entrar", "puede_salir", "auto", "nohuir", "buscable",
    "derribada", "noderribar", "nomob", "ventana", "puede_mirar", "isbolt", "bolted",
    "autofull"
};

const char             *const container_flags[] = {
    "cerrable", "noforzable", "cerrado", "bloqueado", "eatkey", "r1", "r2", "r3",
...
const char             *const sec_flags[] = {
    "interior", "carretera", "campo", "bosque", "colinas",
    "montanya", "agua_nadable", "agua_nonadable", "bajoelagua", "aire",
    "desierto", "area_ent", "suelooceano", "bajotierra", "lava",
    "pantano", "ciudad", "vroad", "hroad", "mar",
    "jungla", "pastizal", "cruce", "thickforest", "altamontanya",
    "artico", "cascada", "rio", "muelle", "lago",
    "campin", "portalstone", "deepmud", "arenasmovedizas", "pasto",
    "valle", "montanyarodeable", "playa", "niebla", "nochange",
    "cielo", "nube", "nieve", "mineral", "max"
};

const char             *const r_flags[] = {
    "oscura", "mortal", "nomob", "techada", "almacenmascota", "noflecha",
    "bv06", "nomagica", "tunnel", "pribada", "segura", "solitaria",
    "tiendamascota", "norecall", "tradeskills", "nodejartodo", "silenciosa", "logspeech",
    "nodejar", "clanstoreroom", "nosummon", "noastral", "teleport", "teleshowdesc",
    "sinsuelo", "anchor", "arena", "iluminada", "banco", "quest",
    "prototipo", "dnd"
};

const char             *const o_flags[] = {
    "brillante", "hum", "oscuro", "leal", "malvado", "invisible", "magico", "nodejar", "bendecir",
    "antibueno", "antimalvado", "antineutral", "nodesvestir", "inventario",
    "antimago", "antiladron", "antiguerrero", "anticlerigo", "organico", "metal",
    "donacion", "clanobject", "clancorpse", "antivampiro", "antidruida",
    "oculto", "envenenado", "cubre", "r22", "enterrado", "prototipo",
    "nolocalizable", "groundrot", "saqueable", "pkdisarmed", "lodged",
    "skelerot", "override", "clancontainer", "nogroup", "rojo", "azul", "purpura", "amarillo", "blanco", "azuloscuro", "verde", "azulclaro", "verdeoscuro", "cyan", "rosa", "rojooscuro", "gris", "grisoscuro", "naranja"
};

const char             *const w_flags[] = {
    "coger", "dedo", "cuello", "cuerpo", "cabeza", "piernas", "pies", "manos", "brazos",
    "escudo", "encima", "cintura", "munyeca", "blandido", "sostenido", "_dual_", "orejas", "ojos",
    "misil", "espalda", "cara", "tobillo", "costilla", "brazo", "pierna",
    "vaina", "hombros", "l_wing", "r_wing"
};

const char             *const item_w_flags[] = {
    "coger", "dedo", "dedo", "cuello", "cuello", "cuerpo", "cabeza", "piernas", "pies",
    "manos", "brazos", "escudo", "encima", "cintura", "munyeca", "munyeca", "blandido",
    "sostenido", "dual", "orejas", "ojos", "misil", "espalda", "cara", "tobillo", "tobillo",
    "costilla", "brazo", "pierna", "vaina", "hombros", "l_wing", "r_wing"
};
...
const char             *const o_types[] = {
    "ninguno", "luz", "pergamino", "varita", "baston", "arma",
    "_fireweapon", "_missile", "tesoro", "armadura", "pocion", "_worn",
    "mueble", "basura", "_oldtrap", "contenedor", "_note", "contenedorbebida",
    "llave", "comida", "moneda", "lapiz", "bote", "cuerpo",
    "cuerpo_pc", "fuente", "pastilla", "sangre", "charcosangre", "restos",
    "pipa", "contenedorhierbas", "hierba", "incienso", "fuego", "libro",
    "interruptor", "palanca", "tirador", "boton", "dial", "runa",
    "bolsaruna", "match", "trap", "mapa", "portal", "papel",
    "yesca", "ganzua", "espiga", "disease", "aceite", "combustible",
    "pieza", "thowing", "armamisil", "proyectil", "carcaj", "pala",
    "pomada", "cocinable", "llavero", "olor", "chance", "afilado",
    "argolla", "crudo", "instrumento", "esqueleto", "recurso", "pintura", "piedra",
    "herramienta", "estufa", "carvon", "vaina", "sabotaje", "forja", "recurso_bolsa",
...
"nada", "fuerza", "destreza", "inteligencia", "sabiduria", "constitucion",
"sexo", "clase", "nivel", "edad", "altura", "peso", "mana", "vida", "movimiento",
"oro", "experiencia", "armadura", "hitroll", "damroll", "save_poison", "save_rod",
"save_para", "save_breath", "save_spell", "carisma", "afectado", "resistente",
"inmune", "suceptible", "weaponspell", "suerte", "backstab", "pick", "track",
...
const char             *const a_types[] = {
    "none", "fuerza", "destreza", "inteligencia", "sabiduria", "constitucion",  // (0- 5)
    "sexo", "clase", "raza", "edad", "altura", "peso", "mana", "vida", "movimiento",   // 14
    "unused", "unused", "armadura", "hitroll", "damroll", "save_poison", "save_rod",
    "save_para", "save_breath", "save_spell", "carisma", "affected", "resistant",  // 27
    "immune", "susceptible", "weaponspell", "suerte", "unused", "unused", "unused",   // 34
    "unused", "unused", "unused", "unused", "unused", "unused", "unused", "unused",
    "unused",
    "unused", "unused", "unused", "unused", "unused", "unused", "unused", "unused", "unused",   // 52
    "unused", "unused", "unused", "wearspell", "removespell", "emocion", "mental",
    "stripsn", "remove", "unused", "full", "thirst", "drunk", "sangre", "unused",    // 67
    "recurringspell", "contagious", "xaffected", "odor", "roomflag", "sectortype",
    "roomlight", "televnum", "teledelay", "unused", "unused", "unused", "percentage"    // 80
};

const char             *const a_flags[] = {
    "ceguera", "invisible", "detectar_maldad",
...
const char             *const act_flags[] = {
    ...
    "questgiver", "sigilist", "craftshop", "shipwright",
    "beastmeld", "beastpet", "max"
};
...
const char             *const plr_flags[] = {
    ...
    "boat",
    "music", "battle",
    "autowake", "extreme", "norename", "communication", "crafts", "r64", "r65", "r66", "r67", "r68",
    "r69",
    "r70", "r71", "r72", "r73", "r74", "r75", "r76", "r77", "r78", "r79", "r80", "r81", "telloff"
};
...
void do_goto( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_SET( ch->pcdata->flags, PCFLAG_BUILDWALK ) && !IS_NPC( ch ) ) {
        if ( !xIS_SET( ch->act, PLR_WIZINVIS ) && ch->level <= 108 )
        {
										if ( !VLD_STR( ch->pcdata->bamfout ) )
						                act( AT_IMMORT,
			                     "&C$n desaparece envuelto en una misteriosa niebla.&D",
			                     ch, NULL, NULL, TO_ROOM );
			                                 else
			act( AT_CYAN, ch->pcdata->bamfout, ch, NULL, NULL, TO_ROOM );
				}
            }
        ...
        if ( !xIS_SET( ch->act, PLR_WIZINVIS ) && ch->level <= 108 )
                {
					                           ...
					                           act( AT_IMMORT,
					                     "&C$n aparece envuelto en una misteriosa niebla.&D",
					                     ch, NULL, NULL, TO_ROOM );
					                                 else
			act( AT_CYAN, ch->pcdata->bamfin, ch, NULL, NULL, TO_ROOM );
				}
}
    if ( IS_SET( ch->pcdata->flags, PCFLAG_BUILDWALK ) && !IS_NPC( ch ) ) {
...
void do_mset( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' || ( arg2[0] == '\0' && ch->substate != SUB_REPEATCMD )
        ...
        if ( ch->level == ( MAX_LEVEL ) )
            send_to_char( "PLAYER ONLY: puppet xp dexp recast hull\r\n", ch );
        return;
    }
    ...
    if ( !victim && get_trust( ch ) < LEVEL_IMMORTAL ) {
     ...
     if ( !(
//str_cmp( ch->name, "novick" ) ||
str_cmp( ch->name, "zlorth" )
// || str_cmp( ch->name, "mewrsuin" )
|| str_cmp( ch->name, "raoden" ) ||
str_cmp( ch->name, "vladaar" ) ) && !IS_NPC( victim ) ) {
        if ( !str_cmp( ch->pcdata->lastip, victim->pcdata->lastip ) ) {
            send_to_char( "No puedes modificar tus propios personajes.\r\n", ch );
            return;
        }
    }
    ...
    if ( get_trust( ch ) < LEVEL_AJ_CAPTAIN && IS_NPC( victim )
    ...
    if ( !str_cmp( arg2, "tradeclass" ) ) {
        ...
        if ( value < -1 || value >= 26 ) {
            ch_printf( ch, "Tradeclass range is 20 to 25, -1 removes tradeclass.\n" );
            return;
        }
...
void do_aassign( CHAR_DATA *ch, char *argument )
{
    ...
    if ( get_trust( ch ) >= LEVEL_AJ_CPL || ( is_name( buf, ch->pcdata->bestowments )
    ...
    for ( vnum = tarea->low_o_vnum; vnum <= tarea->hi_o_vnum; vnum++ ) {
        ...
        if ( AREA_VERSION_WRITE > 0 )
            switch ( pObjIndex->item_type ) {
                ...
                case ITEM_SCROLL:
                    fprintf( fpout, "'%s' '%s' '%s'\n",
                             IS_VALID_SN( pObjIndex->value[1] ) ? skill_table[pObjIndex->
                                                                              value[1]]->name :
                             "NONE",
    ...
    for ( vnum = lrange; vnum <= trange; vnum++ ) {
        ...
        if ( vnum < 58000 && vnum > 54000 ) {
            pager_printf( ch, "[%5d][Desc %s][Progs %s] %s %s\r\n", vnum, room->description
                          ...
                          room->mudprogs ? "&GY&D" : "&RN&D", room->name, room->owner );
        }
        else {
