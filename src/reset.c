    ...
    switch ( pReset->command ) {
        ...
        case 'M':
            ...
            snprintf( buf, MAX_STRING_LENGTH, "%2d) %s (%d) -> %s Room: %d [%d]%s\r\n", *num,
                      mobname, pReset->arg1, roomname, pReset->arg3, pReset->arg2,
                      pReset->ch ? " [Reseted]" : "" );
                ...
                switch ( tReset->command ) {
                    case 'E':
                        ...
                        snprintf( buf + strlen( buf ), MAX_STRING_LENGTH - strlen( buf ),
                                  ...
                                  tReset->arg1, mobname, wear_locs[tReset->arg3], tReset->arg2,
                                  tReset->obj ? " [Reseted]" : "" );
                        break;
                        ...
                        switch ( gReset->command ) {
                            case 'P':
                                ...
                                snprintf( buf + strlen( buf ),
                                          ...
                                          "%2d) (put) %s (%d) -> %s (%d) [%d]%s\r\n", *num, objname,
                                          gReset->arg1, roomname, obj ? obj->vnum : gReset->arg3,
                                          gReset->arg2, gReset->obj ? " [Reseted]" : "" );
                                break;
                        }
                    }
                }
            ...
            for ( tReset = pReset->first_reset; tReset; tReset = tReset->next_reset ) {
                ...
                switch ( tReset->command ) {
                    case 'P':
                        ...
                        snprintf( buf + strlen( buf ), MAX_STRING_LENGTH - strlen( buf ),
                                  "%2d) (put) %s (%d) -> %s (%d) [%d]%s\r\n", *num, objname,
                                  tReset->arg1, roomname, obj ? obj->vnum : tReset->arg3,
                                  tReset->arg2, tReset->obj ? " [Reseted]" : "" );
                        break;
                    ...
                    case 'T':
                        snprintf( buf + strlen( buf ), MAX_STRING_LENGTH - strlen( buf ),
                                  ...
                                  flag_string( tReset->extra, trap_flags ), objname,
                                  obj ? obj->vnum : 0 );
                        break;
        ...
        case 'D':
            ...
            snprintf( buf, MAX_STRING_LENGTH, "%2d) %s [%d] the %s [%d] door %s (%d)\r\n", *num,
                      mobname, pReset->arg3, objname, pReset->arg2, roomname, pReset->arg1 );
            break;
        ...
        case 'T':
            ...
            snprintf( buf, MAX_STRING_LENGTH, "%2d) Trap: %d %d %d %d (%s) -> %s (%d)\r\n",
                      *num, pReset->extra, pReset->arg1, pReset->arg2, pReset->arg3,
                      flag_string( pReset->extra, trap_flags ), roomname, room ? room->vnum : 0 );
            break;
    }
    return buf;
}
...
void instaroom( ROOM_INDEX_DATA *pRoom, bool dodoors )
{
    ...
    for ( rch = pRoom->first_person; rch; rch = rch->next_in_room ) {
        ...
        preset->obj = NULL;
        if ( rch->reset )                              /* Set the old ch reset ch to NULL 
                                                        * so it can reset later */
            rch->reset->ch = NULL;
        preset->ch = rch;
        rch->reset = preset;
                        ...
                        switch ( tReset->command ) {
                            ...
                            case 'E':
                                if ( !( pObjIndex = get_obj_index( tReset->arg1 ) ) ) {
                                    bug( "%s: %s: 'E' or 'G': bad obj vnum %d.", __FUNCTION__,
                                            ...
                                            case 'P':
                                                if ( !
                                                    ...
                                                    bug( "%s: %s: 'P': bad obj vnum %d.",
                                                         __FUNCTION__, filename, gReset->arg1 );
                                                    continue;
                                                }
                                                iNest = gReset->extra;
                                                ...
                                                if ( !
                                                    ...
                                                    bug( "%s: %s: 'P': bad objto vnum %d.",
                                                         __FUNCTION__, filename, gReset->arg3 );
                                                    continue;
                                                }
                                                if ( iNest >= MAX_NEST ) {
                ...
                if ( pReset->first_reset ) {
                    for ( tReset = pReset->first_reset; tReset; tReset = tReset->next_reset ) {
                        ...
                        switch ( tReset->command ) {
                                ...
                                if ( !IS_SET( tReset->extra, TRAP_OBJ ) ) {
                                    ...
                                    if ( tReset->arg3 > 0 ) {
                                        if ( !( pObjToIndex = get_obj_index( tReset->arg3 ) ) ) {
                                            bug( "%s: %s: 'T': bad objto vnum %d.", __FUNCTION__,
                                                 filename, tReset->arg3 );
                                            continue;
                                        }
                                    ...
                                    pobj =
                                        make_trap( tReset->arg2, tReset->arg1,
                                                   number_fuzzy( to_obj->level ), tReset->extra );
                            ...
                            case 'P':
                                if ( !( pObjIndex = get_obj_index( tReset->arg1 ) ) ) {
                                    bug( "%s: %s: 'P': bad obj vnum %d.", __FUNCTION__, filename,
                                         tReset->arg1 );
                                    continue;
                                }
                                iNest = tReset->extra;
                                ...
                                if ( !( pObjToIndex = get_obj_index( tReset->arg3 ) ) ) {
                                    bug( "%s: %s: 'P': bad objto vnum %d.", __FUNCTION__, filename,
                                         tReset->arg3 );
                                    continue;
                                }

                                if ( iNest >= MAX_NEST ) {
                                    bug( "%s: %s: 'P': Exceeded nesting limit of %d. Room %d.",
    ...
    switch ( letter ) {
        ...
        case 'G':
            if ( !room->last_mob_reset ) {
                bug( "%s: Can't add '%c' reset to room: last_mob_reset is NULL.", __FUNCTION__,
                     letter );
                return NULL;
            }
            room->last_obj_reset = pReset;
            LINK( pReset, room->last_mob_reset->first_reset, room->last_mob_reset->last_reset,
                  next_reset, prev_reset );
            return pReset;

        case 'P':
            if ( !room->last_obj_reset ) {
                bug( "%s: Can't add '%c' reset to room: last_obj_reset is NULL.", __FUNCTION__,
                     letter );
                return NULL;
            }
            LINK( pReset, room->last_obj_reset->first_reset, room->last_obj_reset->last_reset,
                  next_reset, prev_reset );
            return pReset;
        ...
        for ( pReset = ch->in_room->first_reset; pReset; pReset = pReset_next ) {
            ...
            for ( tReset = pReset->first_reset; tReset; tReset = tReset_next ) {
                ...
                for ( gReset = tReset->first_reset; gReset; gReset = gReset_next ) {
                    ...
                    nfind++;
                    if ( nfind == num ) {
                        UNLINK( gReset, tReset->first_reset, tReset->last_reset, next_reset,
                                prev_reset );
                        delete_reset( gReset );
                        send_to_char( "Reset deleted.\r\n", ch );
                        return;
