...
void do_daze( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC( victim ) && ( get_curr_dex( victim ) - get_curr_dex( ch ) >= 2 ) && chance < 95 ) {
        act( AT_WHITE,
             "You try a daze technique on $N, but $E deftly sidesteps it.", ch, NULL, victim,
             TO_CHAR );
    ...
    if ( IS_NPC( ch ) && ( victim->level - ch->level > 9 ) && chance < 95 ) {
        act( AT_WHITE,
             "You try a daze technique on $N, but $E is too powerful for such a technique.", ch,
             NULL, victim, TO_CHAR );
        WAIT_STATE( ch, skill_table[gsn_daze]->beats );
        return;
    }
    ...
    if ( chance < 15 ) {
        act( AT_WHITE,
             "You try a daze technique on $N, but $E sees it coming.", ch, NULL, victim, TO_CHAR );
        WAIT_STATE( ch, skill_table[gsn_daze]->beats );
        return;
    }
    ...
    if ( !arg || arg[0] == '\0' ) {
        ...
        for ( morph = morph_start; morph; morph = morph->next ) {
            ...
            if ( !str_cmp( morph->short_desc, "a black bear" ) ||
                 ...
                 !str_cmp( morph->short_desc, "a vampire bat" ) ||
                 !str_cmp( morph->short_desc, "a gryphon" ) ||
                 !str_cmp( morph->short_desc, "an owl" ) ||
                 !str_cmp( morph->short_desc, "a bass fish" ) )
                continue;
            /*
             * Check Level 
             */
...
{
    if ( ( skill_table[gsn_disguise]->skill_level[ch->Class] == -1
           && !IS_IMMORTAL( ch ) ) && ( LEARNED( ch, gsn_disguise ) < 1 ) ) {
        error( ch );
        return;
    }
