...
void add_to_wizinfo( char *name, WIZINFO_DATA * wiz )
{
    ...
    if ( !wiz->email )
        wiz->email = STRALLOC( "No Establecido" );
...
void fread_info( WIZINFO_DATA * wiz, FILE * fp )
{
    ...
    for ( ;; ) {
        ...
        switch ( UPPER( word[0] ) ) {
            ...
            case 'L':
                KEY( "Nivel", wiz->level, fread_number( fp ) );
                break;
...
void do_wizinfo( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_pager( "Información de contacto para la ADMINISTRACIÓN:\r\r\n\n", ch );
    send_to_pager( "Nombre         Dirección Email\r\n", ch );
    send_to_pager( "------------+-------------------\r\n", ch );
...
void do_finger( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( victim = get_char_world( ch, buf ) ) != NULL ) {
        if ( IS_SET( victim->pcdata->flags, PCFLAG_PRIVACY ) && !IS_IMMORTAL( ch ) ) {
            ch_printf( ch, "%s tiene la privacidad activada.\r\n", victim->name );
            return;
        }
     ...
     */
        ...
        if ( stat( fingload, &fst ) == -1 || !check_parse_name( capitalize( argument ), FALSE ) ) {
            ch_printf( ch, "&YNo hay un jugador llamado '%s'.\r\n", argument );
            return;
        }
        ...
        if ( !temproom ) {
            ...
            send_to_char( "Error fatal, reporrta a los ADMINISTRADORES.\r\n", ch );
            return;
        }
    ...
    if ( !skip ) {
        send_to_char( "&w          Información de personaje\r\n", ch );
        send_to_char( "          -----------\r\n", ch );
        ch_printf( ch, "&wNombre      : &G%-12s &wEdad de MUD: &G%d\r\n", victim->name,
                   calculate_age( victim ) );
        ch_printf( ch, "&wNivel     : &G%-12d &w    Sexo: &G%s\r\n", victim->level,
                   victim->sex == SEX_MALE ? "Hombre" : victim->sex ==
                   SEX_FEMALE ? "Mujer" : "Otro" );
        ch_printf( ch, "&wRaza      : &G%-12s &w  Clase: &G",
        ...
        if ( xIS_SET( victim->act, PLR_EXTREME ) ) {
            ch_printf( ch, "&wModo: &G¡6D EXTREMO!&w    desde:&G %d &wnivel\r\n",
                       victim->pcdata->extlevel );
        }
        else if ( !xIS_SET( victim->act, PLR_EXTREME ) ) {
            send_to_char( "&wModo: &GNormal\r\n", ch );
        }
        if ( victim->pcdata->rprate == 1 ) {
            send_to_char( "&wClasificación rolera : &GNovato\r\n", ch );
        }
        else if ( victim->pcdata->rprate == 2 ) {
            send_to_char( "&wClasificación rolera : &GAmateur\r\n", ch );
        }
        else if ( victim->pcdata->rprate == 3 ) {
            send_to_char( "&wClasificación rolera : &Gaprendiz\r\n", ch );
        }
        else if ( victim->pcdata->rprate == 4 ) {
            send_to_char( "&wClasificación rolera : &GMedio\r\n", ch );
        }
        else if ( victim->pcdata->rprate == 5 ) {
            send_to_char( "&wClasificación rolera : &GJugador rolero\r\n", ch );
        }
        else if ( victim->pcdata->rprate == 6 ) {
            send_to_char( "&wClasificación rolera : &GMedia alta\r\n", ch );
        }
        else if ( victim->pcdata->rprate == 7 ) {
            send_to_char( "&wClasificación rolera : &Gbuena\r\n", ch );
        }
        else if ( victim->pcdata->rprate == 8 ) {
            send_to_char( "&wClasificación rolera : &Gperfecta\r\n", ch );
        }
        else if ( victim->pcdata->rprate == 9 ) {
            send_to_char( "&wClasificación rolera : &GExperto\r\n", ch );
        }
        else if ( victim->pcdata->rprate == 10 ) {
            send_to_char( "&wClasificación rolera : &GMaestro\r\n", ch );
        }
        ...
        ch_printf( ch, "&wPágina  : &G%s\r\n",
                   victim->pcdata->homepage != NULL ? victim->pcdata->homepage : "No especificada" );
        if ( ch->level == LEVEL_AJ_GENERAL ) {
            ch_printf( ch, "&wEmail     : &G%s\r\n",
                       victim->pcdata->email != NULL ? victim->pcdata->email : "No especificado" );
        }
        if ( !loaded )
            send_to_char( "&wÚltima conexión   : &GConectado ahora!\r\n", ch );
        else {
            ch_printf( ch, "&wÚltima conexión   : &G%s", ctime( &laston ) );
        }
        int                     score = 0;
...
/* Volk - fun stuff, display percentage of areas discovered */
        ...
        if ( IS_IMMORTAL( ch ) ) {
            send_to_char( "&wInformación de los ADMINISTRADORES\r\n", ch );
            send_to_char( "--------------------\r\n", ch );
            if ( !str_cmp( victim->name, "Zlorth" ) ) // Allows me to have my alts at
            {
                ch_printf( ch, "&wInformación de la IP       : &G%s\r\n",
                           !victim->desc ? "Unknown" : "127.0.0.1" );
                ch_printf( ch, "&wÚltima IP Conocida : &G127.0.0.1\r\n" );
            }
            else {
                ch_printf( ch, "&wInformación de la IP       : &G%s\r\n",
                           !victim->desc ? "Unknown" : victim->desc->host );
                ch_printf( ch, "&wÚltima IP Conocida : &G%s\r\n",
                           !victim->pcdata->lastip ? "Unknown" : victim->pcdata->lastip );
            }

            ch_printf( ch, "&wTiempo jugado   : &G%ld horas\r\n",
                       ( long int ) GET_TIME_PLAYED( victim ) );
            ch_printf( ch, "&wAutorizado por : &G%s\r\n",
                       victim->pcdata->authed_by ? victim->pcdata->authed_by : ( sysdata.
                                                                                 WAIT_FOR_AUTH ?
                                                                                 "No Autorizado" :
                                                                                 "The Code" ) );
            ch_printf( ch, "&wEstado de la Privacidad: &G%s\r\n",
                       IS_SET( victim->pcdata->flags, PCFLAG_PRIVACY ) ? "Activada" : "Desactivada" );
            snprintf( buf2, MAX_STRING_LENGTH, "list %s", buf );
        }
        ch_printf( ch, "&wBiografía:\r\n&G%s\r\n",
                   victim->pcdata->bio ? victim->pcdata->bio : "Sin crear" );
    }
...
void do_email( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !str_cmp( argument, "clear" ) ) {
        ...
        send_to_char( "Dirección de Email borrada.\r\n", ch );
        return;
    }
    ...
    send_to_char( "Dirección de Email establecida.\r\n", ch );
}
...
void do_homepage( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char( "Página establecida.\r\n", ch );
}
...
void do_privacy( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_SET( ch->pcdata->flags, PCFLAG_PRIVACY ) ) {
        send_to_char( "Flag privacidad activado.\r\n", ch );
        return;
    }
    else {
        send_to_char( "Flag privacidad desactivado.\r\n", ch );
        return;
    }
}
