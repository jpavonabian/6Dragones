...
/*    Thought might fix the problem with object currtypes, apparantly not yet */
    ...
    if ( obj->pIndexData->vnum == 16243 || obj->pIndexData->vnum == 16244
         ...
         || obj->pIndexData->vnum == 11250 || obj->pIndexData->vnum == 16246 ) {
...
/*    ch->pcdata->tmpclass = 0;
                        ch->pcdata->tmprace = 0;  */// culprit for human form not working
    ch->pcdata->tmproom = 0;
    ...
    ch->fightingip = NULL;
    ch->pcdata->recast = 0;                            /* Initalize recast - Samson
                                                        * 1-4-99 */
    ch->pcdata->invite = 0;
    ch->pcdata->adjust = 0;
        ...
        switch ( UPPER( word[0] ) ) {
            ...
            case 'B':
                ...
                if ( !strcmp( word, "Bank" ) ) {
                    ...
                    if ( !bank ) {
                        if ( !preload )
                            ch_printf( ch, "La cuenta %s no existe.\r\n", bname );
                    }
                    else
                        ch->pcdata->bank = bank;
            ...
            case 'P':
                ...
                KEY( "pareja", ch->pcdata->pareja, fread_string( fp ) );
...
// beast master	stuff
            ...
            case 'S':
                ...
                if ( !strcmp( word, "Site" ) ) {
                    if ( !preload && !copyover )
                        ch_printf( ch, "Última conexión desde: %s", fread_word( fp ) );
                    else
                        fread_to_eol( fp );
                    fMatch = TRUE;
                    if ( preload )
                    ...
                    if ( ( obj->pIndexData = get_obj_index( vnum ) ) == NULL ) {
                        ...
                        if ( AREA_VERSION_WRITE >= 8
                             && ( obj->pIndexData->vnum == 16243 || obj->pIndexData->vnum == 16244
                                  ...
                                  || obj->pIndexData->vnum == 11248
                                  || obj->pIndexData->vnum == 11249
                                  || obj->pIndexData->vnum == 11250
                                  || obj->pIndexData->vnum == 16246 ) ) {
...
void fwrite_mobile( FILE * fp, CHAR_DATA *mob )
{
    ...
    if ( mob->in_room ) {
        fprintf( fp, "Room        %d\n", ( mob->in_room == get_room_index( ROOM_VNUM_LIMBO )
                                           && mob->was_in_room ) ? mob->was_in_room->
                 vnum : mob->in_room->vnum );
    }
