...
void start_editing_nolimit( CHAR_DATA *ch, char *old_text, short max_total )
{
    ...
    send_to_char
        ( "&cComienza a escribir.\r\nMientras escribes puedes usar los siguientes comandos.\r\n/? = ayuda /s = salvar y salir /c = limpiar /l = listar /i = insertar línea /g = Ir a un número de línea\r\n",
          ch );
    send_to_char( "-----------------------------------------------------------------------\r\n",
                  ch );
    if ( ch->editor )
...
void edit_buffer( CHAR_DATA *ch, char *argument )
{
    ...
    d = ch->desc;
    if ( d == NULL ) {
        send_to_char( "No tienes conexión.\r\n", ch );
        return;
    }
    ...
    if ( ( argument[0] == '/' && argument[1] != '/' ) || argument[0] == '\\' ) {
        ...
        if ( !str_cmp( cmd + 1, "?" ) )
            ...
            send_to_char( "Uh? Escribe '/?' para ver la lista de comandos válidos del editor.\r\n", ch );
        if ( cmd[1] != 'a' && cmd[1] != 's' )
            send_to_char( "> ", ch );
        return;
    }
    ...
    if ( edd->max_size != NOLIMIT && TOTAL_BUFFER_SIZE( edd ) + linelen + 2 >= edd->max_size ) {
        send_to_char( "No cabe más texto.\r\n", ch );
...
void editor_print_info( CHAR_DATA *ch, EDITOR_DATA * edd, char *argument )
{
    ch_printf( ch, "Editando: %s\r\n", edd->desc ? edd->desc : "(Null description)" );
    ch_printf( ch, "Líneas totales: %4d   en línea:  %4d\r\n", edd->line_count,
               get_online_index( edd ) );
    ch_printf( ch, "Tamaño del texto: %4d   Máximo: ", TOTAL_BUFFER_SIZE( edd ) );
    if ( edd->max_size == NOLIMIT )
        send_to_char( "Infinito\r\n", ch );
    else
        ch_printf( ch, "%d\r\n", edd->max_size );
    return;
}
...
void editor_help( CHAR_DATA *ch, EDITOR_DATA * edd, char *argument )
{
    ...
    const char             *editor_help[] = {
        ...
        "Comandos del editor\r\n"
            "---------------------------------\r\n"
            "/l [rango]      lista las líneas escritas\r\n"
            "/c              limpia el texto escrito\r\n"
            "/d <número linea>       borra la línea\r\n"
            "/g <número de línea>       sitúa el cursor en la línea especificada\r\n"
            "/i <número de línea>       inserta una línea vacía\r\n"
            "/r <'cadena vieja'> <'cadena nueva'>  Reemplaza\r\n"
            "/a              Aborta la edición sin salvar\r\n"
            "/p              Muestra información general\r\n"
            "/! <comando>    Ejecuta un comando fuera del editor (no uses otro comando de edición)\r\n"
            "/s              salva y sale\r\n" "/f [rango]      formatea el texto.\r\n"
            "Escribe/? <comando>  para más información.\r\n\r\n",
        "/l [rango]: Lista laslíneas escritas. Rango opcional\r\n"
            "   para mostrar las líneas introducidas entre un número y otro.\r\n",
        "/c: Limpia todo el texto, dejando una línea vacía.\r\n",
        "/d <línea>: borra una línea. Si borras la línea en la que estás\r\n"
            "   el punto de inserción se moverá hacia abajo si es posible.\r\n",
        "/g <línea>: Mueve el punto de inserción a una línea especificada.\r\n",
        "/i <línea>: Inserta una línea vacía en la posición anterior a la línea dada.\r\n",
        "/r <viejo> <nuevo>: Reemplaza cadenas de texto. La vieja\r\n"
            "  por la nueva. Sensible a mayúsculas. Usa comillas simples para reemplazar múltiples palabras.\r\n",
        "/a: Aborta la edición, saliendo sin salvar\r\n"
            "   lo que habías escrito.\r\n",
        "/p: muestra información detallada de qué estás editando.\r\n",
        "/!: Comando de escape. Ejecuta el comando indicado en el mud\r\n"
            "   como si no estubieses en el editor. Solo permitido a inmortales\r\n"
            "   pues puede hacer que el juego caiga.\r\n",
        "/s: Salva la edición y sale del editor.\r\n",
        "/f [rango] [ancho]; Formatea el párrafo. Toma el texto que\r\n"
            "   esté situado en el rango de líneas dado.\r\n"
            "   Si especificas un ancho se formateará en ese número de columnas especificado.\r\n",
    };
    ...
    if ( arg[i] == NULL )
        send_to_char( "No hay ayuda sobre eso.\r\n", ch );
    else
...
void editor_search_and_replace( CHAR_DATA *ch, EDITOR_DATA * edd, char *argument )
{
    ...
    if ( word_src[0] == '\0' || word_dst[0] == '\0' ) {
        send_to_char( "Es necesario especificar ambas cadenas.\r\n", ch );
        return;
    }
    ...
    if ( cloned_edd->max_size != NOLIMIT
        ...
        send_to_char( "Operación cancelada por superar el máximo texto permitido.\r\n",
                      ch );
        discard_editdata( cloned_edd );
    }
    else {
        ch_printf( ch, "Reemplazando '%s' por '%s'...\r\n", word_src, word_dst );
        discard_editdata( edd );
        ch->editor = cloned_edd;
        ch_printf( ch, "Reemplazadas %d cadena(s).\r\n", repl_count );
    }
    return;
}
...
void editor_insert_line( CHAR_DATA *ch, EDITOR_DATA * edd, char *argument )
{
    ...
    if ( argument[0] == '\0' || !is_number( argument ) ) {
        send_to_char( "Se te olvidó el número de líneas.\r\n", ch );
        return;
    }
    lineindex = atoi( argument );
    if ( lineindex < 1 || lineindex > edd->line_count ) {
        ch_printf( ch, "Está fuera de rango. (1-%d).\r\n", edd->line_count );
        return;
    }
    newline = make_new_line( ( char * ) "" );
    attach_line_range( edd, lineindex, newline );
    ch_printf( ch, "Línea insertada en %d.\r\n", lineindex );
    return;
}
...
void editor_delete_line( CHAR_DATA *ch, EDITOR_DATA * edd, char *argument )
{
    ...
    if ( argument[0] == '\0' || !is_number( argument ) ) {
        send_to_char( "has olvidado el número de línea.\r\n", ch );
        return;
    }
    lineindex = atoi( argument );
    if ( lineindex < 1 || lineindex > edd->line_count ) {
        ch_printf( ch, "Línea fuera de rango (1-%d).\r\n", edd->line_count );
        return;
    }
    if ( edd->line_count == 1 ) {
        if ( edd->first_line->line[0] != '\0' ) {
            ...
            edd->text_size = 0;
            send_to_char( "Borrada la línea 1.\r\n", ch );
        }
        else
            send_to_char( "El texto está vacío.\r\n", ch );
        return;
    }
    del_line = detach_line_range( edd, lineindex, lineindex );  /* TEST ME */
    discard_line_list( del_line );
    ch_printf( ch, "borrada la línea %d.\r\n", lineindex );
    return;
}
...
void editor_goto_line( CHAR_DATA *ch, EDITOR_DATA * edd, char *argument )
{
    ...
    if ( !VLD_STR( argument ) || !is_number( argument ) ) {
        send_to_char( "Se te olvidó el número de línea.\r\n", ch );
        return;
    }
    lineindex = atoi( argument );
    if ( lineindex < 1 || lineindex > edd->line_count ) {
    ...
    ch_printf( ch, "Cursor en la línea %d.\r\n", lineindex );
    return;
}
...
void editor_format( CHAR_DATA *ch, EDITOR_DATA * edd, char *argument )
{
    ...
    if ( argument[0] != '\0' ) {
        ...
        if ( str_cmp( arg1, "all" ) ) {
            if ( arg1[0] == '\0' ) {
                send_to_char( "Introduce el inicio y el final.\r\n", ch );
                return;
            }
            if ( !is_number( arg1 ) ) {
                send_to_char( "Los argumentos han de ser números.\r\n", ch );
                return;
            }
            ...
            if ( atoi( arg1 ) < 1 || atoi( arg1 ) > edd->line_count || atoi( arg1 ) < linefrom ) {
                ch_printf( ch, "El segundo  argumento tiene que estar entre %d y %d como máximo\r\n",
        ...
        if ( argument[0] == '\0' )
            ...
            send_to_char( "El argumento ha de ser numérico.\r\n", ch );
            return;
        }
        else {
            width = atoi( argument );
            if ( width < 15 || width > 240 ) {
                send_to_char( "De 15 a 250 carácteres.\r\n", ch );
                return;
            }
        }
    }
    ...
    ch_printf( ch, "Formateando líneas %d-%d (%d líneas) en %d columnas...\r\n", linefrom, lineto,
    ...
    if ( edd->max_size != NOLIMIT && TOTAL_BUFFER_SIZE( edd ) >= edd->max_size ) {
        ...
        ch_printf( ch, "Formateadas %d líneas.\r\n", 40 );
    }
    if ( complete_format )
