...
void affect_modify( CHAR_DATA *ch, AFFECT_DATA *paf, bool fAdd )
{
        ...
        case APPLY_FULL:
            if ( !IS_NPC( ch ) )
                ch->pcdata->condition[COND_FULL] =
                    URANGE( 0, ch->pcdata->condition[COND_FULL] + mod, STATED );
            break;

        case APPLY_THIRST:
            if ( !IS_NPC( ch ) )
                ch->pcdata->condition[COND_THIRST] =
                    URANGE( 0, ch->pcdata->condition[COND_THIRST] + mod, STATED );
            break;
    ...
    if ( !IS_NPC( ch ) && saving_char != ch && ( wield = get_eq_char( ch, WEAR_WIELD ) ) != NULL
        ...
        if ( depth == 0 ) {
            depth++;
            act( AT_ACTION, "Eres demasiado débil para blandir $p por más tiempo.", ch, wield, NULL, TO_CHAR );
            act( AT_ACTION, "$n desempuña $p.", ch, wield, NULL, TO_ROOM );
...
void discover_area( CHAR_DATA *ch, AREA_DATA *area )
{
    ...
    ch_printf( ch, "&O¡Descubres una nueva zona llamada %s!&d\r\n", area->name );
    gain_exp( ch, exp );
}
...
void equip_char( CHAR_DATA *ch, OBJ_DATA *obj, int iWear )
{
    ...
    if ( ( IS_OBJ_STAT( obj, ITEM_ANTI_EVIL ) && IS_EVIL( ch ) )
         ...
         */
        if ( loading_char != ch ) {
            act( AT_MAGIC, "$p te da un calambrazo y cae al suelo.", ch, obj, NULL, TO_CHAR );
            act( AT_MAGIC, "$n deja caer al suelo $p precipitadamente.", ch, obj, NULL, TO_ROOM );
        }
        if ( obj->carried_by )
            obj_from_char( obj );
...
void extract_char( CHAR_DATA *ch, bool fPull )
{
        ...
        for ( wch = first_char; wch; wch = wch->next ) {
            if ( wch->mount == ch ) {
                ...
                if ( wch->in_room == ch->in_room ) {
                    act( AT_SOCIAL, "$N se derrumba debajo de ti...", wch, NULL,
                         ch, TO_CHAR );
                    act( AT_SOCIAL, "Tristemente, desmontas $N por última vez.", wch, NULL, ch,
                         TO_CHAR );
                    act( AT_PLAIN, "$n desmonta $N por última vez.", wch, NULL, ch,
                         TO_ROOM );
                }
            }
            if ( wch->pcdata && wch->pcdata->pet == ch ) {
                ...
                wch->pcdata->pet = NULL;
                if ( wch->in_room == ch->in_room )
                    act( AT_SOCIAL, "Lloras por la pérdida de $N.", wch, NULL, ch, TO_CHAR );
            }
        }
    }
    ...
    if ( !fPull ) {
        ...
        if ( ( wch = get_char_room( ch, ( char * ) "healer" ) ) != NULL ) {
            act( AT_MAGIC, "$n pronuncia algunos encantamientos haciendo gestos con sus manos y sus dedos.",
                 wch, NULL, NULL, TO_ROOM );
            act( AT_MAGIC, "¡$n aparece en un remolino de colores!", ch, NULL, NULL,
                 TO_ROOM );
            snprintf( buf, MSL, "decir bienvenido a la tierra de los vivos, %s\r\n",
                      capitalize( ch->name ) );
            snprintf( buf, MSL,
                      "decir %s, si lo necesita puede ir a la funeraria a por su cadáver.\r\n",
                      capitalize( ch->name ) );
            interpret( wch, buf );
        }
        else
...
// testing this to see if lets you look bear, fish, etc for animalform
    ...
    if ( ms > 15 )                                     /* range 1 to 20 -- feel free to *
        ...
        switch ( number_range( UMAX( 1, ( ms / 5 - 15 ) ), ( ms + 4 ) / 5 ) ) {
            ...
            case 2:
                ...
                break;
            case 3:
                t = "No consigues tu objetivo....\r\n";
                break;
            case 4:
                t = "Lo intentas pero huye de ti...\r\n";
                break;
            case 5:
                t = "¡Desaparece al intentar ponerle la mano encima!\r\n";
                break;
            case 6:
                t = "¡No puedes, no deja de moverse!\r\n";
                break;
            case 7:
                t = "¡Oh, no! ¡Sangre!\r\n";
                break;
            ...
            case 9:
                ...
                break;
            case 10:
                t = "¡Desaparece al intentar tocarlo!\r\n";
                break;
            case 11:
                t = "¡anda, vuela?!?!?\r\n";
                break;
            case 12:
                ...
                break;
            case 13:
                t = "¡Tienes el universo en la palma de tu mano!\r\n";
                break;
            case 14:
                t = "Estás muy mal.\r\n";
                break;
            case 15:
                t = "No tienes manos... 'NO!'\r\n";
                break;
            case 16:
                t = "¡No encuentras tus manos!\r\n";
                break;
            case 17:
                t = "¡Gritas de dolor!\r\n";
                break;
            case 18:
                t = "¿¿Qué iba yo a hacer?!?!\r\n";
                break;
            case 19:
                t = "¡Encuentras la respuesta correcta!\r\n";
                break;
            case 20:
                t = "has encontrado la pregunta a todas tus respuestas.\r\n";
                break;
        }
    else {
        ...
        switch ( number_range( 1, sub / 10 ) ) {
            ...
            case 3:
                ...
                break;
            case 4:
                t = "...Está debajo de otras cosas...\r\n";
                break;
            case 5:
                t = "Golpeas tu nariz al intentarlo.\r\n";
                break;
            case 6:
                t = "¿El qué?  Veo dos... No, tres...\r\n";
                break;
        }
    }
    send_to_char( t, ch );
...
OBJ_DATA               *find_obj( CHAR_DATA *ch, char *argument, bool carryonly )
{
    ...
    if ( arg2[0] == '\0' ) {
        ...
        if ( !IS_OBJ_STAT( container, ITEM_COVERING )
            ...
            act( AT_PLAIN, "$d necesita abrirse primero.", ch, NULL, container->name, TO_CHAR );
            return NULL;
        }
        ...
        if ( !obj )
            act( AT_PLAIN,
                 IS_OBJ_STAT( container,
                              ITEM_COVERING ) ? "No veo nada como eso debajo de $p." :
                 "No veo nada como eso en $p.", ch, container, NULL, TO_CHAR );
...
{
    if ( !victim )                                     /* Gorog - panicked attempt to * *
                                                        * stop crashes */
        return FALSE;
    if ( !ch ) {
...
bool can_dye_obj( CHAR_DATA *ch, OBJ_DATA *obj )
{
    if ( !IS_OBJ_STAT( obj, ITEM_ROJO ) && !IS_OBJ_STAT( obj, ITEM_AZUL ) && !IS_OBJ_STAT( obj, ITEM_PURPURA ) && !IS_OBJ_STAT( obj, ITEM_AMARILLO ) && !IS_OBJ_STAT( obj, ITEM_BLANCO ) && !IS_OBJ_STAT( obj, ITEM_AZULOSCURO ) && !IS_OBJ_STAT( obj, ITEM_VERDE ) && !IS_OBJ_STAT( obj, ITEM_AZULCLARO ) && !IS_OBJ_STAT( obj, ITEM_VERDEOSCURO ) && !IS_OBJ_STAT( obj, ITEM_CYAN ) && !IS_OBJ_STAT( obj, ITEM_ROSA ) && !IS_OBJ_STAT( obj, ITEM_ROJOOSCURO ) && !IS_OBJ_STAT( obj, ITEM_GRIS ) && !IS_OBJ_STAT( obj, ITEM_GRISOSCURO ) && !IS_OBJ_STAT( obj, ITEM_NARANJA ) )
        return TRUE;
        return FALSE;
}
...
ch_ret spring_trap( CHAR_DATA *ch, OBJ_DATA *obj )
{
    ...
    switch ( typ ) {
        default:
            txt = "golpeado por una trampa";
            break;
        case TRAP_TYPE_POISON_GAS:
            txt = "rodeado por un gas verdoso";
            break;
        case TRAP_TYPE_POISON_DART:
            txt = "golpeado por un dardo";
            break;
        case TRAP_TYPE_POISON_NEEDLE:
            txt = "pinchado con una aguja";
            break;
        case TRAP_TYPE_POISON_DAGGER:
            txt = "apuñalado por una daga";
            break;
        case TRAP_TYPE_POISON_ARROW:
            txt = "golpeado por una flecha";
            break;
        case TRAP_TYPE_BLINDNESS_GAS:
            txt = "rodeado por una nube de gas rojizo";
            break;
        case TRAP_TYPE_SLEEPING_GAS:
            txt = "rodeado por una nube de gas amarillo";
            break;
        case TRAP_TYPE_FLAME:
            txt = "rodeado por llamas ardientes";
            break;
        case TRAP_TYPE_EXPLOSION:
            txt = "golpeado por una explosión";
            break;
        case TRAP_TYPE_ACID_SPRAY:
            txt = "cubierto por ácido";
            break;
        case TRAP_TYPE_ELECTRIC_SHOCK:
            txt = "sorprendido";
            break;
        case TRAP_TYPE_BLADE:
            txt = "cortado por unas cuchillas";
            break;
        case TRAP_TYPE_SEX_CHANGE:
            txt = "rodeado por un aura misteriosa";
            break;
    }

    dam = number_range( obj->value[2], obj->value[2] * 2 );
    act_printf( AT_HITME, ch, NULL, NULL, TO_CHAR, "¡Eres %s!", txt );
    act_printf( AT_ACTION, ch, NULL, NULL, TO_ROOM, "¡$n es %s.", txt );
    --obj->value[0];
    if ( obj->value[0] <= 0 )
        extract_obj( obj );
    switch ( typ ) {
...
void set_position( CHAR_DATA *ch, int position )
{
    if ( ch->position == POS_CRAWL ) {
        ...
        if ( position == POS_FIGHTING || position == POS_BERSERK || position == POS_EVASIVE
            ...
            send_to_char( "Te levantas y comienzas a luchar.", ch );
        }
    }
                    ...
                    const char *url, CHAR_DATA *ch )
{
    ...
    if ( !xIS_SET( ch->act, PLR_COMMUNICATION ) && !xIS_SET( ch->act, PLR_MUSIC )
         && !xIS_SET( ch->act, PLR_BATTLE ) && !xIS_SET( ch->act, PLR_CRAFTS ) )
        return;
