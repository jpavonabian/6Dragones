...
void violence_update( void )
{
    ...
    for ( ch = last_char; ch; lst_ch = ch, ch = gch_prev ) {
        ...
        if ( gch_prev && gch_prev->next != ch ) {
            bug( "FATAL: %s: %s->prev->next doesn't point to ch. Short-cutting here",
                ...
                if ( 30 + ( ch->level / 4 ) >= number_percent(  ) ) {
                    ...
                    switch ( attacktype ) {
                        ...
                        case ATCK_FEED:
                            ...
                            break;
                        case ATCK_DRAIN:
                            act( AT_MAGIC, "$n pronuncia unos pocos encantamientos.", ch, NULL, NULL,
                                 TO_ROOM );
                            retcode =
                        ...
                        case ATCK_FROSTBREATH:
                            act( AT_MAGIC, "$n pronuncia unos encantamientos.", ch, NULL, NULL,
                                 TO_ROOM );
                            retcode =
                    ...
                    switch ( attacktype ) {
                        case DFND_MINORHEALING:
                            ...
                            if ( ch->hit < ch->max_hit ) {
                                act( AT_MAGIC,
                                     "$n mpronuncia unos encantamientos...and looks a little better.",
                        ...
                        case DFND_HEAL:
                            if ( ch->hit < ch->max_hit ) {
                                act( AT_MAGIC,
                                     "$n mpronuncia unos encantamientos...and looks much healthier.",
                                     ch, NULL, NULL, TO_ROOM );
    ...
    if ( retcode == rNONE ) {
        ...
        if ( !IS_AFFECTED( ch, AFF_FLYING ) && !IS_AFFECTED( ch, AFF_FLOATING ) )
            if ( ch->in_room && ch->in_room->sector_type )
                move =
                    encumbrance( ch,
                                 movement_loss[URANGE
                                               ( 0, ch->in_room->sector_type, SECT_MAX - 1 )] );
            else
...
ch_ret one_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
    ...
    if ( IS_AFFECTED( victim, AFF_UNHOLY_SPHERE ) ) {
        send_to_char( "No puedes atravesar su esfera.\r\n", ch );
        return rNONE;
    }
...
//  chance = number_chance( 1, 10 );
    ...
    if ( number_chance( 1, 10 ) == 10 ) {
        if ( IS_AFFECTED( ch, AFF_SLOW ) ) {
            act( AT_GREEN, "Intentas atacar pero te mueves demasiado lento.", ch, NULL, NULL,
                 TO_CHAR );
            act( AT_GREEN, "$n intenta atacar pero se mueve demasiado lento.", ch, NULL, NULL,
                 TO_ROOM );
            return rNONE;
        }
    }
    ...
    if ( IS_AFFECTED( victim, AFF_FLAMING_SHIELD ) && number_chance( 1, 15 ) >= 9 ) {
        ...
        ch_printf( victim, "&rQuemas a %s con tu escudo de fuego.&D\r\n", ch->name );

        if ( IS_NPC( victim ) )
            ch_printf( ch, "&rEl escudo de fuego de %s te quema..&D\r\n",
                       victim->short_descr );
        else
            ch_printf( ch, "&rEl escudo de fuego de %s te quema..&D\r\n", victim->name );
...
*/
    ...
    if ( ch->race == RACE_ANIMAL && !IS_NPC( ch ) ) {
        thac0 =
            interpolate( ch->level, thac0_00, thac0_32 ) - ( GET_HITROLL( ch ) + ch->level / 5 );
    }
    else {
    ...
    if ( diceroll == 0 || ( diceroll != 19 && diceroll < thac0 - victim_ac ) ) {
        ...
        if ( diceroll < thac0 - victim_ac
            ...
            if ( victim->Class == CLASS_MAGE ) {
                if ( drama > 50 ) {
                    act( AT_LBLUE, "tu ataque rebota en la pared de energÌa de  $N.", ch,
                         NULL, victim, TO_CHAR );
                    act( AT_LBLUE, "Tu pared de energÌa brilla desviando el ataque de $n.", ch,
                         NULL, victim, TO_VICT );
                    act( AT_LBLUE, "El ataque de $n rebota en la pared de energÌa de $N.", ch,
                         NULL, victim, TO_NOTVICT );
                }
                else {
                    act( AT_ORANGE,
                         "Tu ataque atraviesa la pared de energÌa de $N, pero se desvÌa.",
                         ch, NULL, victim, TO_CHAR );
                    act( AT_ORANGE,
                         "el ataque de $n atraviesa tu pared de energÌa, pero logras desviarlo.",
                         ch, NULL, victim, TO_VICT );
                    act( AT_ORANGE,
                         "El ataque de $n atraviesa la pared de energÌa de $N, pero se desvÌa.", ch,
                         NULL, victim, TO_NOTVICT );
                }
            }
            if ( victim->race == RACE_DRAGON ) {
                if ( drama > 50 ) {
                    act( AT_YELLOW,
                         "Saltan chispas cuando tu ataque roza las escamas de dragÛn de $N sin daÒarle.", ch,
                         NULL, victim, TO_CHAR );
                    act( AT_YELLOW, "Saltan chispas cuando el ataque de $n roza tus escamas de dragÛn sin daÒarte.", ch,
                         NULL, victim, TO_VICT );
                    act( AT_YELLOW,
                         "Saltan chispas cuando el ataque de $n roza las escamas de dragÛn de $N sin daÒarle.", ch,
                         NULL, victim, TO_NOTVICT );
                }
                else {
                    act( AT_ORANGE, "tu ataque penetra levemente las lisas escamas de dragÛn de $N.", ch,
                         NULL, victim, TO_CHAR );
                    act( AT_ORANGE, "El ataque de $n apenas penetra en tus lisas escamas de dragÛn.", ch,
                         NULL, victim, TO_VICT );
                    act( AT_ORANGE, "El ataque de $n apenas penetra las lisas escamas de dragÛn de $N.", ch,
                         NULL, victim, TO_NOTVICT );
                }
            }
            if ( victim->Class != CLASS_MAGE && victim->race != RACE_DRAGON ) {
                if ( drama > 50 ) {
                    act( AT_YELLOW, "Las chispas vuelan cuando tu ataque es desviado por la armadura de $N.", ch,
                         NULL, victim, TO_CHAR );
                    act( AT_YELLOW, "Las chispas vuelan cuando tu armadura desvÌa el ataque de $n.", ch, NULL,
                         victim, TO_VICT );
                    act( AT_YELLOW, "Saltan chispas cuando el ataque de $n es desviado por la armadura de $N.", ch,
                         NULL, victim, TO_NOTVICT );
                }
                else {
                    act( AT_ORANGE,
                         "Atraviesas la armadura de $N, pero solo le provocas daÒos superficiales.",
                         ch, NULL, victim, TO_CHAR );
                    act( AT_ORANGE,
                         "El ataque de $n atraviesa tu armadura pero solo te provoca daÒos superficiales.",
                         ch, NULL, victim, TO_VICT );
                    act( AT_ORANGE,
                         "El ataque de $n atraviesa la armadura de $N pero solo le causa daÒos superficiales.",
    ...
    if ( !IS_NPC( ch ) && ch->pcdata->ship > 0 && hull > 8 ) {
        if ( ch->pcdata->hull < 11 ) {
            ...
            if ( ch->pcdata->ship == 1 ) {
                ...
                for ( ch = first_char; ch; ch = ch->next ) {
                    if ( !IS_NPC( ch )
                         && ( ch->in_room->vnum <= hroom || ch->in_room->vnum >= lroom ) ) {
        ...
        if ( ch && ch->pcdata->hull > 10 ) {
            ch->pcdata->hull -= 1;
            ch_printf( ch, "&CLa integridad del casco del barco est· en %d\r\n",
                       ch->pcdata->hull );
...
/*
      if(number_chance(1, 20) == 1) 
      {
        ...
        ch_printf(ch, "&wCuando atacas, una pequeÒa mella se forma en% s, que tendr· que afilarse de nuevo.&D\r\n", wield->short_descr);
        wield->value[6]--;
      ...
      {
        ch_printf(ch, "&w°Cuando atacas %s se daÒa, necesitando afilarse de nuevo!&D\r\n", wield->short_descr);
        wield->value[6] -= 1;
        update_weapon(ch, wield);
      }
      if(wield->value[6] < 25)
 ...
 */
    ...
    if ( chance > 99 ) {
        act( AT_FIRE, "°Lanzas un golpe crÌtico a $N!", ch, NULL, victim, TO_CHAR );
        act( AT_FIRE, "°$n te lanza un golpe crÌtico!", ch, NULL, victim, TO_VICT );
        act( AT_FIRE, "°$n lanza un golpe crÌtico a $N!", ch, NULL, victim, TO_NOTVICT );
        dam += ( int ) ( get_curr_str( ch ) * 2 );
    }

    if ( ch->race == RACE_CENTAUR && chance < 26 ) {
        act( AT_ORANGE, "°Pisoteas con tus pezuÒas a $N!", ch, NULL, victim, TO_CHAR );
        act( AT_ORANGE, "°$n te pisotea con sus pezuÒas!", ch, NULL, victim, TO_VICT );
        act( AT_ORANGE, "°$n pisotea con sus pezuÒas a $N!", ch, NULL, victim, TO_NOTVICT );
        dam += ( int ) ( get_curr_str( ch ) * 2 );
    }
    ...
    if ( !IS_NPC( ch ) && ch->pcdata->learned[gsn_find_weakness] > 0 && fw_chance > 9 ) {
        send_to_char( "&WEncuentras una debilidad en la armadura de tu rival.&D\r\n", ch );
        dam += ( int ) ( get_curr_str( ch ) * 2 );
        ch->move -= 2;
...
//also made more use of the AFF_FURY bit here. -Taon
    ...
    if ( IS_NPC( victim ) && xIS_SET( victim->act, ACT_IMMORTAL ) ) {
        victim->hit += dam;
        if ( number_bits( 4 ) == 0 ) {
            act( AT_RED, "$N no parece afectarse por tu ataque.", ch, NULL, victim, TO_CHAR );
            act( AT_RED, "$N no parece afectarse por el ataque de $n.", ch, NULL, victim, TO_ROOM );
        }
    }
    ...
    if ( IS_AFFECTED( victim, AFF_KINETIC ) ) {
        if ( victim->kinetic_dam > dam ) {
            act( AT_YELLOW, "tu ataque golpea la barrera cinÈtica de $N.", ch, NULL, victim,
                 TO_CHAR );
            act( AT_YELLOW, "El ataque de $n golpea la barrera cinÈtica de $N.", ch, NULL, victim,
                 TO_ROOM );
            victim->kinetic_dam -= dam;
            dam = 0;
        }
        else {
            act( AT_YELLOW, "°Tu ataque ha roto la barrera cinÈtica de $N!", ch, NULL, victim,
                 TO_CHAR );
            act( AT_YELLOW, "°El ataque de $n ha roto la barrera cinÈtica de $N!", ch, NULL, victim,
                 TO_ROOM );
    ...
    if ( IS_AFFECTED( victim, AFF_WARD ) ) {
        if ( victim->ward_dam > dam ) {
            act( AT_YELLOW, "Tu ataque golpea la protecciÛn de $N.", ch, NULL, victim,
                 TO_CHAR );
            act( AT_YELLOW, "El ataque de $n golpea la protecciÛn de $N.", ch, NULL, victim,
                 TO_ROOM );
            victim->ward_dam -= dam;
            dam = 0;
        }
        else {
            act( AT_YELLOW, "°Tu ataque rompe la protecciÛn de $N!", ch, NULL, victim,
                 TO_CHAR );
            act( AT_YELLOW, "°El ataque de $n rompe la protecciÛn de $N!", ch, NULL, victim,
                 TO_ROOM );
...
ch_ret damage( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
    ...
    if ( IS_NPC( ch ) && !ch->soldierset && ch->city && ch->siege ) {
        ...
        if ( city
             && ( city->soldier_mob == ch->pIndexData->vnum
                  || city->archer_mob == ch->pIndexData->vnum ) ) {
            if ( city->soldier_mob == ch->pIndexData->vnum )
        ...
        if ( city
             && ( city->soldier_mob == victim->pIndexData->vnum
                  || city->archer_mob == victim->pIndexData->vnum ) ) {
            if ( city->soldier_mob == victim->pIndexData->vnum )
...
/* On second thought, HTF to check if they HAVEN'T used murder command? need a bool-in-function.
   ...
   first place! */
        ...
        if ( IS_AFFECTED( victim, AFF_SANCTUARY ) && IS_AFFECTED( victim, AFF_SHIELD )
             && number_bits( 5 ) == 0 )
            send_to_char
                ( "&R°Santuario y escudo no son compatibles! °Solo se divirir· el daÒo una vez!\r\n",
                  victim );
        if ( !IS_NPC( victim ) ) {
            if ( victim->Class == CLASS_FISH || victim->race == RACE_DRAGON ) {
                if ( dam > 0 && victim->pcdata->learned[gsn_hardened_scales] > 0 ) {
                    ...
                    if ( victim->pcdata->learned[gsn_hardened_scales] < 25 && skin > 4 )
                        dam = ( int ) ( dam / 10 );
                    if ( ( victim->pcdata->learned[gsn_hardened_scales] < 50
                           && victim->pcdata->learned[gsn_hardened_scales] > 24 ) && skin > 3 )
                        dam = ( int ) ( dam / 5 );
                    if ( ( victim->pcdata->learned[gsn_hardened_scales] < 100
                           && victim->pcdata->learned[gsn_hardened_scales] > 49 ) && skin > 1 )
                        dam = ( int ) ( dam / 2 );
                    if ( skin == 8 )
                        learn_from_success( victim, gsn_hardened_scales );
                }
            }
            if ( victim->Class == CLASS_WOLF || victim->Class == CLASS_BEAR || victim->Class == CLASS_GRYPHON
                 || victim->Class == CLASS_OWL ) {
                if ( dam > 0 && victim->pcdata->learned[gsn_thick_hide] > 0 ) {
                    short                   skin;

                    skin = number_range( 1, 10 );

                    if ( victim->pcdata->learned[gsn_thick_hide] < 25 && skin > 4 )
                        dam = ( int ) ( dam / 10 );
                    if ( ( victim->pcdata->learned[gsn_thick_hide] < 50
                           && victim->pcdata->learned[gsn_thick_hide] > 24 ) && skin > 3 )
                        dam = ( int ) ( dam / 5 );
                    if ( ( victim->pcdata->learned[gsn_thick_hide] < 100
                           && victim->pcdata->learned[gsn_thick_hide] > 49 ) && skin > 1 )
                        dam = ( int ) ( dam / 2 );
                    if ( skin <= 2 )
                        learn_from_success( victim, gsn_thick_hide );
                }
            }
            if ( victim->Class == CLASS_TREANT || victim->Class == CLASS_DRUID ) {
                if ( dam > 0 && victim->pcdata->learned[gsn_bark_skin] > 0 ) {
                    ...
                    if ( victim->pcdata->learned[gsn_bark_skin] < 25 && skin > 4 )
                        dam = ( int ) ( dam / 10 );
                    if ( ( victim->pcdata->learned[gsn_bark_skin] < 50
                           && victim->pcdata->learned[gsn_bark_skin] > 24 ) && skin > 3 )
                        dam = ( int ) ( dam / 5 );
                    if ( ( victim->pcdata->learned[gsn_bark_skin] < 100
                           && victim->pcdata->learned[gsn_bark_skin] > 49 ) && skin > 1 )
                        dam = ( int ) ( dam / 2 );
                    if ( skin == 8 )
        ...
        if ( ( obj = get_eq_char( victim, WEAR_SHIELD ) ) != NULL && deflect >= 2 ) {
            ...
            if ( obj->value[0] < obj->value[1] / 2 && obj->value[0] != 1 ) {
                obj->value[0] = 1;
                act( AT_ACTION, "tu escudo queda inservible.", victim,
                     NULL, NULL, TO_CHAR );
                act( AT_ACTION, "El escudo de $n queda inservible en su brazo.", victim,
                     NULL, NULL, TO_ROOM );
            }
        }
        ...
        if ( shield == TRUE && deflect >= 2 && obj->value[0] != 1 ) {
            act( AT_ACTION, "Tu ataque golpea el escudo de $N.", ch, NULL, victim, TO_CHAR );
            act( AT_ACTION, "El ataque de $n golpea el escudo de $N.", ch, NULL, victim, TO_ROOM );
            dam = ( int ) ( dam / 2 );
        }

        if ( IS_AFFECTED( victim, AFF_WARD ) && victim->level > 70 )
            dam = ( int ) ( dam / 2 );
...
/*
        if ( !IS_NPC( ch ) && ( skill ) )
            if ( xIS_SET( ch->act, PLR_AUTORECAST ) && skill->type == SKILL_SPELL ) {
                ch->pcdata->recast = 0;
                return rNONE;
            }
*/
    ...
    if ( IS_AFFECTED( victim, AFF_REACTIVE ) && dam > 0 && victim->hit > 0
        ...
        act( AT_MAGIC, "Una energÌa reactiva a $n y le sana algunas heridas.", victim, NULL, NULL, TO_ROOM );
        act( AT_MAGIC, "Una energÌa te reactiva y sana algunas de tus heridas.", victim,
             NULL, NULL, TO_CHAR );
        if ( victim->level < 20 )
    ...
    if ( IS_BLOODCLASS( victim ) ) {
        ...
        if ( victim->hit <= ( victim->max_hit / 8 ) && victim->blood > 5 ) {
            ...
            send_to_char( "°La bestia de tu interior ahulla de rabia!\r\n", victim );
        }
    }
    ...
    switch ( victim->position ) {
        case POS_MORTAL:
            act( AT_RED, "$n tiene heridas mortales, y morir· lentamente si nadie le ayuda.", victim, NULL,
                 NULL, TO_ROOM );
            act( AT_RED, "tienes heridas de muerte, y morir·s lentamente si nadie te ayuda.", victim, NULL,
                 NULL, TO_CHAR );
            break;

        case POS_INCAP:
            act( AT_RED, "$n morir· lentamente si nadie le ayuda.", victim, NULL,
                 NULL, TO_ROOM );
            act( AT_RED, "Morir·s lentamente si nadie te ayuda.", victim, NULL,
                 NULL, TO_CHAR );
            break;

        case POS_STUNNED:
            if ( !IS_AFFECTED( victim, AFF_PARALYSIS ) ) {
                act( AT_ACTION, "$n est· inconsciente, pero probablemente se recuperar·.", victim, NULL, NULL,
                     TO_ROOM );
                act( AT_RED, "Est·s inconsciente, pero probablemente te recuperes.", victim, NULL, NULL,
                     TO_CHAR );
            }
            break;
        ...
        case POS_DEAD:
            ...
            act( AT_RED, "°$n ha MUERTO!", victim, 0, 0, TO_ROOM );
            act( AT_RED, "°Has MUERTO!!\r\n", victim, 0, 0, TO_CHAR );
            break;
        ...
        default:
            ...
            if ( dam > victim->max_hit / 4 ) {
                act( AT_RED, "°Eso realmente ha dolido!", victim, 0, 0, TO_CHAR );
                if ( number_bits( 3 ) == 0 )
                    worsen_mental_state( victim, 1 );
            }
            if ( victim->hit < victim->max_hit / 6 ) {
                act( AT_RED, "°DesearÌas que tus heridas dejaran de sangrar tanto!", victim, 0, 0,
                     TO_CHAR );
                if ( number_bits( 2 ) == 0 )
                    worsen_mental_state( victim, 1 );
            }
    ...
    if ( victim->position == POS_DEAD ) {
        ...
        if ( !IS_NPC( ch ) && !IS_NPC( victim ) && in_arena( victim ) ) {
            if ( victim != ch )
                snprintf( buf, MSL, "°%s ha matado a %s!", victim->name, ch->name );
            else
                snprintf( buf, MSL, "°Han matado a %s!", victim->name );
        ...
        if ( !IS_NPC( ch ) && loot ) {
            ...
            if ( xIS_SET( ch->act, PLR_AUTOMONEY ) ) {
                ...
                if ( ( corpse = get_obj_here( ch, ( char * ) "corpse" ) ) ) {
                    ...
                    for ( mobj = corpse->first_content; mobj; mobj = mobj->next_content ) {
                        ...
                        snprintf( mbuf, 80, "%s cuerpo", curr_types[mobj->value[2]] );
            ...
            if ( xIS_SET( ch->act, PLR_AUTOLOOT ) && victim != ch )
                do_get( ch, ( char * ) "todo cuerpo" );
            else
                do_look( ch, ( char * ) "en cuerpo" );

            if ( xIS_SET( ch->act, PLR_AUTOTRASH ) )
                do_trash( ch, ( char * ) "cuerpo" );
        }
...
bool is_safe( CHAR_DATA *ch, CHAR_DATA *victim, bool show_messg )
{
    ...
    if ( IS_SET( victim->in_room->room_flags, ROOM_SAFE ) ) {
        if ( show_messg ) {
            ...
            send_to_char( "Una fuerza m·gica te impide atacar.\r\n", ch );
        }
        return TRUE;
    }
    ...
    if ( ( IS_PACIFIST( ch ) ) && !IN_ARENA( ch ) ) {  /* Fireblade */
        if ( show_messg ) {
            set_char_color( AT_MAGIC, ch );
            ch_printf( ch, "Eres pacifista, no puedes luchar.\r\n" );
        }
        return TRUE;
    }
    ...
    if ( IS_PACIFIST( victim ) ) {                     /* Gorog */
        if ( show_messg ) {
            set_char_color( AT_MAGIC, ch );
            ch_printf( ch, "%s es pacifista y no luchar·.\r\n",
    ...
    if ( !IS_NPC( ch ) && !IS_NPC( victim ) && ch != victim
        ...
        if ( show_messg ) {
            ...
            send_to_char( "La administraciÛn ha prohibido que se haga pk en este area.\r\n", ch );
        }
        return TRUE;
    }
    ...
    if ( ( ch->level < 5 ) && !in_arena( ch ) ) {
        if ( show_messg ) {
            ...
            send_to_char( "Necesitas m·s edad y experiencia. \r\n",
                          ch );
        }
        return TRUE;
    }
    ...
    if ( ch->level <= 10 && !in_arena( ch ) ) {        /* 10 and lower */
        if ( ( ch->level - victim->level >= 3 ) || ( victim->level - ch->level >= 3 ) ) {
            if ( show_messg ) {
                set_char_color( AT_IMMORT, ch );
                send_to_char
                    ( "La administraciÛn de 6 Dragones no permite el asesinato con esa diferencia de niveles.\r\n",
                      ch );
            }
            return TRUE;
        }
    ...
    if ( ( get_timer( victim, TIMER_PKILLED ) > 0 ) && !in_arena( victim ) ) {
        if ( show_messg ) {
            ...
            send_to_char( "Este personaje ha muerto hace menos de un minuto.\r\n", ch );
        }
        return TRUE;
    }
    ...
    if ( !in_arena( ch ) && has_pkilled( ch, victim->name ) ) {
        if ( show_messg ) {
            ...
            send_to_char( "Has matado a ese personaje en los ˙ltimos 30 minutos.\r\n", ch );
        }
        return TRUE;
    }
    ...
    if ( !in_arena( victim ) && has_pkilled( victim, ch->name ) ) {
        if ( show_messg ) {
            set_char_color( AT_RED, ch );
            send_to_char( "Te han matado en los ˙ltimos 30 minutos.\r\n", victim );
        }
        return TRUE;
    }
    ...
    if ( ( get_timer( ch, TIMER_PKILLED ) > 0 ) && !in_arena( ch ) ) {
        if ( show_messg ) {
            set_char_color( AT_GREEN, ch );
            send_to_char( "Te han matado hace menos de un minuto.\r\n", ch );
        }
        return TRUE;
    }
...
void check_killer( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( !IS_NPC( ch ) && !IS_NPC( victim ) && IS_SET( ch->pcdata->flags, PCFLAG_DEADLY )
        ...
        if ( !ch->pcdata->clan || !victim->pcdata->clan
            ...
            update_pos( victim );
            if ( victim != ch ) {
                act( AT_MAGIC, "Rayos de energÌa azul salen del cuerpo, restaurando a $n.", ch,
                     victim->name, NULL, TO_ROOM );
                act( AT_MAGIC, "Rayos azules de energÌa salen del cad·ver y entran en ti.", ch,
    ...
    set_char_color( AT_WHITE, ch );
    send_to_char( "Una sensaciÛn extraÒa crece dentro de ti, y un cosquilleo te sube por la columna vertebral...\r\n",
                  ch );
    set_char_color( AT_IMMORT, ch );
    send_to_char
        ( "Una voz grabe retumba en tu cabeza '°A partir de ahora ser·s conocido como un asesino!'\r\n",
          ch );
    set_char_color( AT_WHITE, ch );
    send_to_char( "Sientes como si tu alma hubiese sido revelada al mundo.\r\n", ch );
    save_char_obj( ch );
    return;
}
...
void update_pos( CHAR_DATA *victim )
{
    ...
    if ( IS_NPC( victim ) || victim->hit <= -11 ) {
        if ( victim->mount ) {
            act( AT_ACTION, "$n cae de $N.", victim, NULL, victim->mount, TO_ROOM );
    ...
    if ( victim->mount ) {
        act( AT_ACTION, "$n cae inconsciente de $N.", victim, NULL, victim->mount, TO_ROOM );
        xREMOVE_BIT( victim->mount->act, ACT_MOUNTED );
        victim->mount = NULL;
    }
}
...
void set_fighting( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( ch->fighting ) {
        bug( "%s: %s -> %s (already fighting %s)", __FUNCTION__, ch->name, victim->name,
    ...
    if ( victim->num_fighting > max_fight( victim ) ) {
        send_to_char( "Ya hay mucha gente en esa pelea.\r\n", ch );
        return;
    }
    ...
    if ( victim->fightingip != NULL && !IS_NPC( ch ) ) {
        if ( ch->pcdata->lastip ) {
            if ( !str_cmp( victim->fightingip, ch->pcdata->lastip ) ) {
                send_to_char( "ya est·s peleando con uno o m·s de un personaje.\r\n",
                              ch );
                return;
            }
        }
...
void death_cry( CHAR_DATA *ch )
{
    ...
    switch ( number_chance( 0, 5 ) ) {
        default:
            msg = "Escuchas el grito de muerte de $n.";
            break;
        case 0:
            msg = "$n grita de furia y su cuerpo cae al suelo!";
            break;
        case 1:
            msg = "$n ha muerto.";
            break;
        case 2:
            msg = "°$n cae al suelo, salpicando tu cuerpo con su sangre!";
            break;
        case 3:
            msg = "$n escupe sangre antes de morir.";
            break;
        case 4:
            msg = "$n suelta un ˙ltimo suspiro antes de morir.";
            break;
        ...
        case 5:
            ...
            if ( xIS_EMPTY( ch->xflags ) ) {
                msg = "Oyes el grito de muerte de $n.";
                break;
            }
            ...
            if ( !msg )
                msg = "Oyes el grito de $n al morir.";
            break;
    }
    ...
    if ( IS_NPC( ch ) )
        msg = "Escuchas un grito desgarrador.";
    else
        msg = "Escuchas un grito desgarrador.";
...
// This check will be for Pet fighting alongside beast master killed mob 
    if ( xIS_SET( ch->act, ACT_BEASTPET ) && !xIS_SET( ch->act, ACT_BEASTMELD ) && ( ch->level >= victim->level )) {
        ...
        if ( master ) {
                if ( ch->level - master->level > 15 ) {
                    send_to_char
                        ( "Tu mascota no puede ganar m·s experiencia.\r\n",
                          master );
                }
                else {
                    if ( master->pcdata->petlevel < 100 ) {
                        master->pcdata->petexp += 1;
                        gain_exp( master, number_range( 5000, 8000 ) );
                        ch_printf( master,
                                   "&W°Tu mascota gana experiencia por la batalla!\r\n" );
                    }

                    if ( master->pcdata->petexp > 19 ) {
                        master->pcdata->petexp = 0;
                        if ( master->pcdata->petlevel < 100 ) {
                            master->pcdata->petlevel += 1;
                            ch_printf( master, "&W°Tu mascota ha subido a nivel %d!\r\n",
                                       master->pcdata->petlevel );
                            ch->level += 1;
                            ch->hit += number_range( 30, 40 );
                            ch->max_hit = ch->hit;
    ...
    if ( victim->level >= ch->level ) {
        if ( BEASTMELD( ch ) ) {
            ...
            if ( ch->level - temp->level > 15 ) {
                send_to_char
                    ( "No puedes ganar m·s experiencia.\r\n",
                      ch );
            }
            else {
                if ( temp->pcdata->petlevel < 100 ) {
                    temp->pcdata->petexp += 1;
                    ch_printf( ch, "&W°Tu mascota gana experiencia por esta batalla!\r\n" );
                }

                if ( temp->pcdata->petexp > 19 ) {
                    temp->pcdata->petexp = 0;
                    if ( temp->pcdata->petlevel < 100 ) {
                        temp->pcdata->petlevel += 1;
                        ch_printf( ch, "&Wtu mascota sube a nivel %d!\r\n",
...
// Only perform if the pet is present in the same room 
        ...
        for ( beastpet = first_char; beastpet; beastpet = beastpet->next ) {
            if ( xIS_SET( beastpet->act, ACT_BEASTPET )
                 && beastpet->master )
            if ( !str_cmp(ch->name, beastpet->master->name ) )
                if ( ch->in_room->vnum == beastpet->in_room->vnum ) {
                    if ( ch->level - beastpet->level > 15 ) {
                        send_to_char
                            ( "Tu mascota no puede subir m·s.\r\n",
                              ch );
                    }
                    else {
                        if ( ch->pcdata->petlevel < 100 ) {
                            ch->pcdata->petexp += 1;
                            ch_printf( ch, "&W°Tu mascota gana experiencia en esta batalla!\r\n" );
                        }
                        if ( ch->pcdata->petexp > 19 ) {
                            ch->pcdata->petexp = 0;
                            if ( ch->pcdata->petlevel < 100 ) {
                                ch->pcdata->petlevel += 1;
                                ch_printf( ch, "&W°Tu mascota ha subido a nivel %d!\r\n",
                                           ch->pcdata->petlevel );
                                beastpet->level += 1;
                                beastpet->hit += number_range( 30, 40 );
         	       	       	beastpet->max_hit = beastpet->hit;
    ...
    if ( IS_NPC( victim ) ) {
        if ( victim->pIndexData->vnum == MOB_VNUM_SOLDIERS
            ...
            if ( rand > 6 ) {
                send_to_char
                    ( "&W°Ganas un punto de gloria por matar a quien est· intentando atacar tu ciudad!\r\n",
                      ch );
                ch->quest_curr += 1;
            }
            else {
            ...
            if ( !IS_NPC( ch ) ) {
                if ( ch->pcdata->city ) {
                    ...
                    else if ( ch->in_room->vnum == DAKAR_DEFEND
                              || ch->in_room->vnum == DAKAR_DEFEND2
                    ...
                    else if ( ch->in_room->vnum == PALEON_SIEGE
                              || ch->in_room->vnum == PALEON_SIEGE2
...
/* Hack way to try to make players think they get some exp gains from mounts when crusader or shadowknight */
    if ( ch->Class == CLASS_BEAST ) {
        ...
        if ( beast == TRUE && ch->in_room->vnum == mob->in_room->vnum
            ...
            ch_printf( ch,
                       "&WTu afinidad con los animales te hace ganar %d puntos de experiencia.\r\n",
                       extra );
            gain_exp( ch, extra );
        }
    }
...
/* Hack way to try to make players think they get some exp gains from mounts when crusader or shadowknight */
    if ( ch->Class == CLASS_SHADOWKNIGHT || ch->secondclass == CLASS_SHADOWKNIGHT
         || ch->thirdclass == CLASS_SHADOWKNIGHT || ch->Class == CLASS_CRUSADER
         || ch->secondclass == CLASS_CRUSADER || ch->thirdclass == CLASS_CRUSADER ) {
        if ( ch->mount && ch->in_room == ch->mount->in_room && IS_STEED( ch )
             && victim->level >= ch->level ) {
            int                     extra = 0;
            ...
            if ( extra > 32000 )
                extra = 32000 + number_range( 1, 1000 );
            ch_printf( ch,
                       "&Wtu afinidad con tu caballo de batalla te hace ganar %d puntos de experiencia.\r\n",
                       extra );
            gain_exp( ch, extra );
        }
    }
    ...
    if ( !IS_NPC( ch ) ) {
        ...
        if ( IS_NPC( victim ) ) {
            if ( ch->pcdata->clan && victim->level >= ch->level ) {
                ...
                clan = ch->pcdata->clan;
                ch_printf( ch,
                           "&G%s ha ganado un punto por tu asesinato, °ahora tiene %d puntos de clan!\r\n",
                           clan->name, clan->totalpoints );
        ...
        if ( !IS_NPC( victim ) && victim != ch ) {
            ...
            if ( ch->pcdata->clan ) {
                ...
                clan = ch->pcdata->clan;
                ch_printf( ch,
                           "&G%s ha ganado tres puntos por tu asesinato, °ahora tiene %d puntos de clan!\r\n",
                           clan->name, clan->totalpoints );
                if ( countpkill( ch, victim ) )
                    increase_clan_pkills( ch->pcdata->clan, ch->level );
                save_clan( clan );
...
/*  Not sure where else to put the announcement - Volk  */
    ...
    if ( !IS_NPC( victim ) ) {
        if ( victim != ch )
            snprintf( buf, MSL, "°%s ha caÌdo a manos de %s!", victim->name,
                      ( IS_NPC( ch ) ? ch->short_descr : ch->name ) );
        else
            snprintf( buf, MSL, "°%s ha muerto!", victim->name );
        announce( buf );
    ...
    if ( victim->in_room->sector_type == SECT_OCEANFLOOR
        ...
        act( AT_BLOOD, "La sangre de $n enrojece el agua.", victim, NULL, NULL,
             TO_ROOM );
    else if ( victim->in_room->sector_type == SECT_AIR )
        act( AT_BLOOD, "La sangre de $n cae libremente por el aire.", victim, NULL, NULL, TO_ROOM );
    else
        make_blood( victim );
 ...
 * the plural form (ie Your pitchfork MASSACRES person!). */
    ...
    if ( ch->success_attack >= 1 && dt != gsn_breath ) {
        if ( xIS_SET( ch->act, PLR_BATTLE ) )
    ...
    if ( dt == TYPE_HIT && ( !IS_NPC( ch ) && ( ch->pcdata->learned[gsn_martial_arts] > 0 ) ) ) {
        if ( chance <= 10 ) {
            snprintf( buf1, 256, "$n %s con el codo a $N %d %s caus·ndole %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N con el codo %d %s caus·ndole %d puntos de daÒo%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s con el codo %d %s caus·ndote %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
        }
        else if ( chance <= 30 ) {
            snprintf( buf1, 256, "$n %s a $N al patearle %d %s caus·ndole %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al patearle %d %s caus·ndole %d puntos de daÒo%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al patearte %d %s caus·ndote %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
        }
        else if ( chance <= 40 ) {
            snprintf( buf1, 256, "$n %s a $N al patearle lateralmente %d %s caus·ndole %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al patearle lateralmente %d %s caus·ndole %d puntos de daÒo%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al patearte lateralmente %d %s caus·ndote %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
        }
        else if ( chance <= 50 ) {
            snprintf( buf1, 256, "$n %s a $N al propinarle un golpe con el canto de la mano %d %s caus·ndole %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al propinarle un golpe con el canto de la mano %d %s caus·ndole %d puntos de daÒo%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al propinarte un golpe con el canto de la mano %d %s caus·ndote %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
        }
        else if ( chance <= 60 ) {
            snprintf( buf1, 256, "$n %s a $N al propinarle un golpe lateral %d %s caus·ndole %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al propinarle un golpe lateral %d %s caus·ndole %d puntos de daÒo%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al propinarte un golpe lateral %d %s caus·ndote %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
        }
        else if ( chance <= 70 ) {
            snprintf( buf1, 256, "$n %s a $N al darle un gran puÒetazo %d %s caus·ndole %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al darle un gran puÒetazo %d %s caus·ndole %d puntos de daÒo%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al darte un gran puÒetazo %d %s caus·ndote %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
        }
        // Following three added in by Taon.
        else if ( chance <= 80 ) {
            snprintf( buf1, 256, "$n %s a $N al darle un ganchoh de derecha %d %s caus·ndole %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al darle un gancho de derecha %d %s caus·ndole %d puntos de daÒo%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al darte un gancho de derecha %d %s caus·ndote %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
        }
        else if ( chance <= 90 ) {
            snprintf( buf1, 256, "$n %s a $N al darle un ganchoh de izquierda %d %s caus·ndole %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al darle un gancho de izquierda %d %s caus·ndole %d puntos de daÒo%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al darte un gancho de izquierda %d %s caus·ndote %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
        }
        else {
            snprintf( buf1, 256, "$n %s a $N al golpearle con furia %d %s caus·ndole %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al golpearle con furia %d %s caus·ndole %d puntos de daÒo%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al golpearte con furia %d %s caus·ndote %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
        }
    }
// start of animal form damage
            ...
            snprintf( buf1, 256, "$n %s a $N al desgarrarle con las zarpas %d %s caus·ndole %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al desgarrarle con las zarpas %d %s caus·ndole %d puntos de daÒo%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al desgarrarte con las zarpas %d %s caus·ndote %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
    }
    else if ( ( ( ch )->Class == CLASS_BEAR || ( ch )->Class == CLASS_WOLF ) ) {
            snprintf( buf1, 256, "$n %s a $N al darle un zarpazo %d %s caus·ndole %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al darle un zarpazo %d %s caus·ndole %d puntos de daÒo%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al darte un zarpazo %d %s caus·ndote %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
    }
    else if ( ( ( ch )->Class == CLASS_FISH ) ) {
            snprintf( buf1, 256, "$n %s a $N al darle un mordisco %d %s caus·ndole %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al darle un mordisco %d %s caus·ndole %d puntos de daÒo%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al darte un mordisco %d %s caus·ndote %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
    }
    else if ( ( ( ch )->Class == CLASS_TREANT ) ) {
            snprintf( buf1, 256, "$n %s a $N al pegarle con las ramas %d %s caus·ndole %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al pegarle con las ramas %d %s caus·ndole %d puntos de daÒo%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al pegarte con las ramas %d %s caus·ndote %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
    }
...
// end of animal
        ...
        if ( chance <= 33 ) {
            snprintf( buf1, 256, "$n %s a $N al atacarle con sus garras %d %s caus·ndole %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al atacarle con tus garras %d %s caus·ndole %d puntos de daÒo%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al  atacarte con sus garras %d %s caus·ndote %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
        }
        else if ( chance >= 34 && chance < 75 ) {
            snprintf( buf1, 256, "$n %s a $N al morderle con sus enormes fauces %d %s caus·ndole %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al morderle con tus enormes fauces %d %s caus·ndole %d puntos de daÒo%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al  morderte con sus enormes fauces %d %s caus·ndote %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
        }
        else {
            snprintf( buf1, 256, "$n %s a $N al golpearle con su cola %d %s caus·ndole %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al golpearle con tu cola %d %s caus·ndole %d puntos de daÒo%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al  golpearte con su cola %d %s caus·ndote %d puntos de daÒo%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
        }
    }
    else if ( dt == TYPE_HIT ) {
        snprintf( buf1, 256, "$n %s a $N %d %s caus·ndole %d puntos de daÒo%c", vs, ch->success_attack,
                  ( ch->success_attack == 1 ) ? "vez" : "veces", ch->damage_amount, punct );
        snprintf( buf2, 256, "%s a $N %d %s caus·ndole %d puntos de daÒo%c", vp, ch->success_attack,
                  ( ch->success_attack == 1 ) ? "vez" : "veces", ch->damage_amount, punct );
        snprintf( buf3, 256, "$n te %s %d %s caus·ndote %d puntos de daÒo%c", vs, ch->success_attack,
                  ( ch->success_attack == 1 ) ? "vez" : "veces", ch->damage_amount, punct );
    }
        ...
        set_char_color( AT_GREEN, ch );
        snprintf( buf1, 256, "El ataque venenoso de $n %s a $N %d %s caus·ndole %d puntos de daÒo%c", vs,
                  ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                  ch->damage_amount, punct );
        snprintf( buf2, 256, "Tu ataque venenoso %s a $N %d %s caus·ndole %d puntos de daÒo%c", vs,
                  ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                  ch->damage_amount, punct );
        snprintf( buf3, 256, "El ataque venenoso de $n te %s %d %s caus·ndote %d puntos de daÒo%c", vs,
                  ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                  ch->damage_amount, punct );
    }
    else {
        ...
        snprintf( buf1, 256, "El %s de $n %s a $N %d %s caus·ndole  %d puntos de daÒo%c", attack, vs,
                  ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                  ch->damage_amount, punct );
        snprintf( buf2, 256, "Tu %s %s a $N %d %s caus·ndole %d puntos de daÒo%c", attack, vs,
                  ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                  ch->damage_amount, punct );
        snprintf( buf3, 256, "El %s de $n te %s %d %s caus·ndote %d puntos de daÒo%c", attack, vs,
                  ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                  ch->damage_amount, punct );
    }
...
/*    CHAR_DATA *obj;  */
    ...
    if ( ch->position == POS_MEDITATING ) {
        send_to_char( "°PerderÌas la concentraciÛn!\r\n", ch );
        return;
    }

    if ( !IS_NPC( ch ) && IS_AFFECTED( ch, AFF_PRAYER ) ) {
        send_to_char( "No mientras estÈs en oraciÛn.\r\n", ch );
        return;
    }

    if ( arg[0] == '\0' ) {
        send_to_char( "øMatar a quiÈn?\r\n", ch );
        return;
    }

    if ( ( victim = get_char_room( ch, arg ) ) == NULL ) {
        send_to_char( "No est· aquÌ.\r\n", ch );
        return;
    }

    if ( IS_AFFECTED( victim, AFF_UNHOLY_SPHERE ) ) {
        send_to_char( "No puedes romper la esfera.\r\n", ch );
        return;
    }
    ...
    if ( IS_NPC( victim ) && victim->morph ) {
        send_to_char
            ( "Esta criatura parece extraÒa. MÌrala m·s de cerca antes de intentar matarla.",
              ch );
        return;
    }
...
/* Volk: Support for arena. */
        ...
        if ( !xIS_SET( victim->act, PLR_KILLER ) && !xIS_SET( victim->act, PLR_THIEF ) ) {
            send_to_char( "Tienes que asesinar a un jugador.\r\n", ch );
            return;
        }
    }

    if ( victim == ch ) {
        send_to_char( "Te das un golpe. °ARGH!\r\n", ch );
    ...
    if ( IS_AFFECTED( ch, AFF_CHARM ) && ch->master == victim ) {
        act( AT_PLAIN, "$N es tu maestro.", ch, NULL, victim, TO_CHAR );
        return;
    }
...
void do_murder( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' ) {
        send_to_char( "øAsesinar a quiÈn?\r\n", ch );
        return;
    }
    ...
    if ( victim == ch ) {
        send_to_char( "El suicidio es de gente est˙pida.\r\n", ch );
        return;
    }

    if ( !IS_NPC( victim ) && xIS_SET( ch->act, PLR_PKSAFE ) ) {
        send_to_char( "°No puedes hacer eso durante el evento!\r\n", ch );
        return;
    }
 ...
 * gets shunted in the arena for accidentally being flagged PLR_NICE :P */
    ...
    WAIT_STATE( ch, 1 * PULSE_VIOLENCE );
    snprintf( buf, MSL, "%s grita histÈricamente '°%s me est· intentando matar!", victim->name,
              IS_NPC( ch ) ? ch->short_descr : ch->name );
...
void do_flee( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_AFFECTED( ch, AFF_BERSERK ) ) {
        send_to_char( "La furia te lo impide...\r\n", ch );
        return;
    }
    if ( IS_AFFECTED( ch, AFF_SNARE ) ) {
        send_to_char( "°No puedes huir!\r\n", ch );
        return;
    }
    if ( IS_AFFECTED( ch, AFF_TANGLED ) ) {
        send_to_char( "No puedes huir.\r\n", ch );
    ...
    if ( !IS_NPC( ch ) && ch->position < POS_FIGHTING ) {
        send_to_char( "la agresividad te impide huir...\r\n", ch );
        return;
    }
    ...
    for ( attempt = 0; attempt < 8; attempt++ ) {
        ...
        if ( ( now_in = ch->in_room ) == was_in )
            continue;
        ch->in_room = was_in;
        act( AT_FLEE, "°$n ha huÌdo!", ch, NULL, NULL, TO_ROOM );
        ch->in_room = now_in;
        act( AT_FLEE, "$n mira a su alredededor por si alguien le persigue.", ch, NULL, NULL, TO_ROOM );
        if ( !IS_NPC( ch ) ) {
    ...
    act( AT_FLEE, "°Intentas huir pero no encuentras una vÌa de escape!", ch, NULL, NULL, TO_CHAR );
    if ( !IS_AVATAR( ch ) && !IS_DUALAVATAR( ch ) && !IS_TRIAVATAR( ch )
...
void make_corpse( CHAR_DATA *ch, CHAR_DATA *killer )
{
     ...
     */
    snprintf( buf, MSL, "cuerpo %s", name );
    STRFREE( corpse->name );
