...
*/
    ...
    if ( diceroll == 0 || ( diceroll != 19 && diceroll < thac0 - victim_ac ) ) {
        ...
        if ( diceroll < thac0 - victim_ac
            ...
            if ( victim->Class == CLASS_MAGE ) {
                if ( drama > 50 ) {
                    act( AT_LBLUE, "tu ataque rebota en la pared de energía de  $N.", ch,
                         NULL, victim, TO_CHAR );
                    act( AT_LBLUE, "Tu pared de energía brilla desviando el ataque de $n.", ch,
                         NULL, victim, TO_VICT );
                    act( AT_LBLUE, "El ataque de $n rebota en la pared de energía de $N.", ch,
                         NULL, victim, TO_NOTVICT );
                }
                else {
                    act( AT_ORANGE,
                         "Tu ataque atraviesa la pared de energía de $N, pero se desvía.",
                         ch, NULL, victim, TO_CHAR );
                    act( AT_ORANGE,
                         "el ataque de $n atraviesa tu pared de energía, pero logras desviarlo.",
                         ch, NULL, victim, TO_VICT );
                    act( AT_ORANGE,
                         "El ataque de $n atraviesa la pared de energía de $N, pero se desvía.", ch,
                         NULL, victim, TO_NOTVICT );
                }
            }
            if ( victim->race == RACE_DRAGON ) {
                if ( drama > 50 ) {
                    act( AT_YELLOW,
                         "Saltan chispas cuando tu ataque roza las escamas de dragón de $N sin dañarle.", ch,
                         NULL, victim, TO_CHAR );
                    act( AT_YELLOW, "Saltan chispas cuando el ataque de $n roza tus escamas de dragón sin dañarte.", ch,
                         NULL, victim, TO_VICT );
                    act( AT_YELLOW,
                         "Saltan chispas cuando el ataque de $n roza las escamas de dragón de $N sin dañarle.", ch,
                         NULL, victim, TO_NOTVICT );
                }
                else {
                    act( AT_ORANGE, "tu ataque penetra levemente las lisas escamas de dragón de $N.", ch,
                         NULL, victim, TO_CHAR );
                    act( AT_ORANGE, "El ataque de $n apenas penetra en tus lisas escamas de dragón.", ch,
                         NULL, victim, TO_VICT );
                    act( AT_ORANGE, "El ataque de $n apenas penetra las lisas escamas de dragón de $N.", ch,
                         NULL, victim, TO_NOTVICT );
                }
            }
            if ( victim->Class != CLASS_MAGE && victim->race != RACE_DRAGON ) {
                if ( drama > 50 ) {
                    act( AT_YELLOW, "Las chispas vuelan cuando tu ataque es desviado por la armadura de $N.", ch,
                         NULL, victim, TO_CHAR );
                    act( AT_YELLOW, "Las chispas vuelan cuando tu armadura desvía el ataque de $n.", ch, NULL,
                         victim, TO_VICT );
                    ...
                    act( AT_ORANGE,
                         "Atraviesas la armadura de $N, pero solo le provocas daños superficiales.",
                         ch, NULL, victim, TO_CHAR );
                    act( AT_ORANGE,
                         "El ataque de $n atraviesa tu armadura pero solo te provoca daños superficiales.",
                         ch, NULL, victim, TO_VICT );
                    act( AT_ORANGE,
                         "El ataque de $n atraviesa la armadura de $N pero solo le causa daños superficiales.",
    ...
    if ( !IS_NPC( ch ) && ch->pcdata->ship > 0 && hull > 8 ) {
        ...
        if ( ch && ch->pcdata->hull > 10 ) {
            ch->pcdata->hull -= 1;
            ch_printf( ch, "&CLa integridad del casco del barco está en %d\r\n",
                       ch->pcdata->hull );
...
/*
      if(number_chance(1, 20) == 1) 
      {
        ...
        ch_printf(ch, "&wCuando atacas, una pequeña mella se forma en% s, que tendrá que afilarse de nuevo.&D\r\n", wield->short_descr);
        wield->value[6]--;
      ...
      {
        ch_printf(ch, "&w¡Cuando atacas %s se daña, necesitando afilarse de nuevo!&D\r\n", wield->short_descr);
        wield->value[6] -= 1;
        update_weapon(ch, wield);
      }
      if(wield->value[6] < 25)
 ...
 */
    ...
    if ( chance > 99 ) {
        act( AT_FIRE, "¡Lanzas un golpe crítico a $N!", ch, NULL, victim, TO_CHAR );
        act( AT_FIRE, "¡$n te lanza un golpe crítico!", ch, NULL, victim, TO_VICT );
        act( AT_FIRE, "¡$n lanza un golpe crítico a $N!", ch, NULL, victim, TO_NOTVICT );
        dam += ( int ) ( get_curr_str( ch ) * 2 );
    }

    if ( ch->race == RACE_CENTAUR && chance < 26 ) {
        act( AT_ORANGE, "¡Pisoteas con tus pezuñas a $N!", ch, NULL, victim, TO_CHAR );
        act( AT_ORANGE, "¡$n te pisotea con sus pezuñas!", ch, NULL, victim, TO_VICT );
        act( AT_ORANGE, "¡$n pisotea con sus pezuñas a $N!", ch, NULL, victim, TO_NOTVICT );
        dam += ( int ) ( get_curr_str( ch ) * 2 );
    }
...
//also made more use of the AFF_FURY bit here. -Taon
    ...
    if ( IS_AFFECTED( victim, AFF_KINETIC ) ) {
        if ( victim->kinetic_dam > dam ) {
            act( AT_YELLOW, "tu ataque golpea la barrera cinética de $N.", ch, NULL, victim,
                 TO_CHAR );
            act( AT_YELLOW, "El ataque de $n golpea la barrera cinética de $N.", ch, NULL, victim,
                 TO_ROOM );
            victim->kinetic_dam -= dam;
            dam = 0;
        }
        else {
            act( AT_YELLOW, "¡Tu ataque ha roto la barrera cinética de $N!", ch, NULL, victim,
                 TO_CHAR );
            act( AT_YELLOW, "¡El ataque de $n ha roto la barrera cinética de $N!", ch, NULL, victim,
                 TO_ROOM );
    ...
    if ( IS_AFFECTED( victim, AFF_WARD ) ) {
        if ( victim->ward_dam > dam ) {
            act( AT_YELLOW, "Tu ataque golpea la protección de $N.", ch, NULL, victim,
                 TO_CHAR );
            act( AT_YELLOW, "El ataque de $n golpea la protección de $N.", ch, NULL, victim,
                 TO_ROOM );
            victim->ward_dam -= dam;
            dam = 0;
        }
        else {
            act( AT_YELLOW, "¡Tu ataque rompe la protección de $N!", ch, NULL, victim,
                 TO_CHAR );
            act( AT_YELLOW, "¡El ataque de $n rompe la protección de $N!", ch, NULL, victim,
                 TO_ROOM );
...
/* On second thought, HTF to check if they HAVEN'T used murder command? need a bool-in-function.
   ...
   first place! */
        ...
        if ( IS_AFFECTED( victim, AFF_SANCTUARY ) && IS_AFFECTED( victim, AFF_SHIELD )
             && number_bits( 5 ) == 0 )
            send_to_char
                ( "&R¡Santuario y escudo no son compatibles! ¡Solo se divirirá el daño una vez!\r\n",
                  victim );
        if ( !IS_NPC( victim ) ) {
...
*/
    ...
    if ( IS_AFFECTED( victim, AFF_REACTIVE ) && dam > 0 && victim->hit > 0
        ...
        act( AT_MAGIC, "Una energía reactiva a $n y le sana algunas heridas.", victim, NULL, NULL, TO_ROOM );
        act( AT_MAGIC, "Una energía te reactiva y sana algunas de tus heridas.", victim,
             NULL, NULL, TO_CHAR );
        if ( victim->level < 20 )
    ...
    switch ( victim->position ) {
        case POS_MORTAL:
            act( AT_RED, "$n tiene heridas mortales, y morirá lentamente si nadie le ayuda.", victim, NULL,
                 NULL, TO_ROOM );
            act( AT_RED, "tienes heridas de muerte, y morirás lentamente si nadie te ayuda.", victim, NULL,
                 NULL, TO_CHAR );
            break;

        case POS_INCAP:
            act( AT_RED, "$n morirá lentamente si nadie le ayuda.", victim, NULL,
                 NULL, TO_ROOM );
            act( AT_RED, "Morirás lentamente si nadie te ayuda.", victim, NULL,
                 NULL, TO_CHAR );
            break;

        case POS_STUNNED:
            if ( !IS_AFFECTED( victim, AFF_PARALYSIS ) ) {
                act( AT_ACTION, "$n está inconsciente, pero probablemente se recuperará.", victim, NULL, NULL,
                     TO_ROOM );
                act( AT_RED, "Estás inconsciente, pero probablemente te recuperes.", victim, NULL, NULL,
                     TO_CHAR );
            }
            break;
        ...
        default:
            ...
            if ( victim->hit < victim->max_hit / 6 ) {
                act( AT_RED, "¡Desearías que tus heridas dejaran de sangrar tanto!", victim, 0, 0,
                     TO_CHAR );
                if ( number_bits( 2 ) == 0 )
                    worsen_mental_state( victim, 1 );
            }
...
bool is_safe( CHAR_DATA *ch, CHAR_DATA *victim, bool show_messg )
{
    ...
    if ( IS_SET( victim->in_room->room_flags, ROOM_SAFE ) ) {
        if ( show_messg ) {
            ...
            send_to_char( "Una fuerza mágica te impide atacar.\r\n", ch );
        }
        return TRUE;
    }
    ...
    if ( IS_PACIFIST( victim ) ) {                     /* Gorog */
        if ( show_messg ) {
            set_char_color( AT_MAGIC, ch );
            ch_printf( ch, "%s es pacifista y no luchará.\r\n",
    ...
    if ( !IS_NPC( ch ) && !IS_NPC( victim ) && ch != victim
        ...
        if ( show_messg ) {
            ...
            send_to_char( "La administración ha prohibido que se haga pk en este area.\r\n", ch );
        }
        return TRUE;
    }
    ...
    if ( ( ch->level < 5 ) && !in_arena( ch ) ) {
        if ( show_messg ) {
            ...
            send_to_char( "Necesitas más edad y experiencia. \r\n",
                          ch );
        }
        return TRUE;
    }
    ...
    if ( ch->level <= 10 && !in_arena( ch ) ) {        /* 10 and lower */
        if ( ( ch->level - victim->level >= 3 ) || ( victim->level - ch->level >= 3 ) ) {
            if ( show_messg ) {
                set_char_color( AT_IMMORT, ch );
                send_to_char
                    ( "La administración de 6 Dragones no permite el asesinato con esa diferencia de niveles.\r\n",
                      ch );
            }
            return TRUE;
        }
    ...
    if ( !in_arena( ch ) && has_pkilled( ch, victim->name ) ) {
        if ( show_messg ) {
            ...
            send_to_char( "Has matado a ese personaje en los últimos 30 minutos.\r\n", ch );
        }
        return TRUE;
    }
    ...
    if ( !in_arena( victim ) && has_pkilled( victim, ch->name ) ) {
        if ( show_messg ) {
            set_char_color( AT_RED, ch );
            send_to_char( "Te han matado en los últimos 30 minutos.\r\n", victim );
        }
        return TRUE;
    }
...
void check_killer( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( !IS_NPC( ch ) && !IS_NPC( victim ) && IS_SET( ch->pcdata->flags, PCFLAG_DEADLY )
        ...
        if ( !ch->pcdata->clan || !victim->pcdata->clan
            ...
            update_pos( victim );
            if ( victim != ch ) {
                act( AT_MAGIC, "Rayos de energía azul salen del cuerpo, restaurando a $n.", ch,
                     victim->name, NULL, TO_ROOM );
                act( AT_MAGIC, "Rayos azules de energía salen del cadáver y entran en ti.", ch,
    ...
    set_char_color( AT_WHITE, ch );
    send_to_char( "Una sensación extraña crece dentro de ti, y un cosquilleo te sube por la columna vertebral...\r\n",
                  ch );
    set_char_color( AT_IMMORT, ch );
    send_to_char
        ( "Una voz grabe retumba en tu cabeza '¡A partir de ahora serás conocido como un asesino!'\r\n",
          ch );
    set_char_color( AT_WHITE, ch );
    ...
    if ( victim->fightingip != NULL && !IS_NPC( ch ) ) {
        if ( ch->pcdata->lastip ) {
            if ( !str_cmp( victim->fightingip, ch->pcdata->lastip ) ) {
                send_to_char( "ya estás peleando con uno o más de un personaje.\r\n",
                              ch );
                return;
            }
        }
...
void death_cry( CHAR_DATA *ch )
{
    ...
    switch ( number_chance( 0, 5 ) ) {
        ...
        case 3:
            ...
            break;
        case 4:
            msg = "$n suelta un último suspiro antes de morir.";
            break;
    ...
    if ( xIS_SET( ch->act, ACT_BEASTPET ) && !xIS_SET( ch->act, ACT_BEASTMELD ) && ( ch->level >= victim->level )) {
        ...
        if ( master ) {
                if ( ch->level - master->level > 15 ) {
                    send_to_char
                        ( "Tu mascota no puede ganar más experiencia.\r\n",
                          master );
                }
                else {
    ...
    if ( victim->level >= ch->level ) {
        if ( BEASTMELD( ch ) ) {
            ...
            if ( ch->level - temp->level > 15 ) {
                send_to_char
                    ( "No puedes ganar más experiencia.\r\n",
                      ch );
            }
            else {
...
// Only perform if the pet is present in the same room 
    ...
    if ( IS_NPC( victim ) ) {
        if ( victim->pIndexData->vnum == MOB_VNUM_SOLDIERS
            ...
            if ( rand > 6 ) {
                send_to_char
                    ( "&W¡Ganas un punto de gloria por matar a quien está intentando atacar tu ciudad!\r\n",
                      ch );
                ch->quest_curr += 1;
            }
            else {
...
/*  Not sure where else to put the announcement - Volk  */
    ...
    if ( !IS_NPC( victim ) ) {
        if ( victim != ch )
            snprintf( buf, MSL, "¡%s ha caído a manos de %s!", victim->name,
 ...
 * the plural form (ie Your pitchfork MASSACRES person!). */
    ...
    if ( dt == TYPE_HIT && ( !IS_NPC( ch ) && ( ch->pcdata->learned[gsn_martial_arts] > 0 ) ) ) {
        if ( chance <= 10 ) {
            snprintf( buf1, 256, "$n %s con el codo a $N %d %s causándole %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N con el codo %d %s causándole %d puntos de daño%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s con el codo %d %s causándote %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
        }
        else if ( chance <= 30 ) {
            snprintf( buf1, 256, "$n %s a $N al patearle %d %s causándole %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al patearle %d %s causándole %d puntos de daño%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al patearte %d %s causándote %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
        }
        else if ( chance <= 40 ) {
            snprintf( buf1, 256, "$n %s a $N al patearle lateralmente %d %s causándole %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al patearle lateralmente %d %s causándole %d puntos de daño%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al patearte lateralmente %d %s causándote %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
        }
        else if ( chance <= 50 ) {
            snprintf( buf1, 256, "$n %s a $N al propinarle un golpe con el canto de la mano %d %s causándole %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al propinarle un golpe con el canto de la mano %d %s causándole %d puntos de daño%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al propinarte un golpe con el canto de la mano %d %s causándote %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
        }
        else if ( chance <= 60 ) {
            snprintf( buf1, 256, "$n %s a $N al propinarle un golpe lateral %d %s causándole %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al propinarle un golpe lateral %d %s causándole %d puntos de daño%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al propinarte un golpe lateral %d %s causándote %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
        }
        else if ( chance <= 70 ) {
            snprintf( buf1, 256, "$n %s a $N al darle un gran puñetazo %d %s causándole %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al darle un gran puñetazo %d %s causándole %d puntos de daño%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al darte un gran puñetazo %d %s causándote %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
        }
        // Following three added in by Taon.
        else if ( chance <= 80 ) {
            snprintf( buf1, 256, "$n %s a $N al darle un ganchoh de derecha %d %s causándole %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al darle un gancho de derecha %d %s causándole %d puntos de daño%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al darte un gancho de derecha %d %s causándote %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
        }
        else if ( chance <= 90 ) {
            snprintf( buf1, 256, "$n %s a $N al darle un ganchoh de izquierda %d %s causándole %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al darle un gancho de izquierda %d %s causándole %d puntos de daño%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al darte un gancho de izquierda %d %s causándote %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
        }
        else {
            snprintf( buf1, 256, "$n %s a $N al golpearle con furia %d %s causándole %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al golpearle con furia %d %s causándole %d puntos de daño%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al golpearte con furia %d %s causándote %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
        }
    }
// start of animal form damage
            ...
            snprintf( buf1, 256, "$n %s a $N al desgarrarle con las zarpas %d %s causándole %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al desgarrarle con las zarpas %d %s causándole %d puntos de daño%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al desgarrarte con las zarpas %d %s causándote %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
    }
    else if ( ( ( ch )->Class == CLASS_BEAR || ( ch )->Class == CLASS_WOLF ) ) {
            snprintf( buf1, 256, "$n %s a $N al darle un zarpazo %d %s causándole %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al darle un zarpazo %d %s causándole %d puntos de daño%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al darte un zarpazo %d %s causándote %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
    }
    else if ( ( ( ch )->Class == CLASS_FISH ) ) {
            snprintf( buf1, 256, "$n %s a $N al darle un mordisco %d %s causándole %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al darle un mordisco %d %s causándole %d puntos de daño%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al darte un mordisco %d %s causándote %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
    }
    else if ( ( ( ch )->Class == CLASS_TREANT ) ) {
            snprintf( buf1, 256, "$n %s a $N al pegarle con las ramas %d %s causándole %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al pegarle con las ramas %d %s causándole %d puntos de daño%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al pegarte con las ramas %d %s causándote %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
    }
...
// end of animal
        ...
        if ( chance <= 33 ) {
            snprintf( buf1, 256, "$n %s a $N al atacarle con sus garras %d %s causándole %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al atacarle con tus garras %d %s causándole %d puntos de daño%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al  atacarte con sus garras %d %s causándote %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
        }
        else if ( chance >= 34 && chance < 75 ) {
            snprintf( buf1, 256, "$n %s a $N al morderle con sus enormes fauces %d %s causándole %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al morderle con tus enormes fauces %d %s causándole %d puntos de daño%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al  morderte con sus enormes fauces %d %s causándote %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
        }
        else {
            snprintf( buf1, 256, "$n %s a $N al golpearle con su cola %d %s causándole %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf2, 256, "%s a $N al golpearle con tu cola %d %s causándole %d puntos de daño%c", vp,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
            snprintf( buf3, 256, "$n te %s al  golpearte con su cola %d %s causándote %d puntos de daño%c", vs,
                      ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                      ch->damage_amount, punct );
        }
    }
    else if ( dt == TYPE_HIT ) {
        snprintf( buf1, 256, "$n %s a $N %d %s causándole %d puntos de daño%c", vs, ch->success_attack,
                  ( ch->success_attack == 1 ) ? "vez" : "veces", ch->damage_amount, punct );
        snprintf( buf2, 256, "%s a $N %d %s causándole %d puntos de daño%c", vp, ch->success_attack,
                  ( ch->success_attack == 1 ) ? "vez" : "veces", ch->damage_amount, punct );
        snprintf( buf3, 256, "$n te %s %d %s causándote %d puntos de daño%c", vs, ch->success_attack,
                  ( ch->success_attack == 1 ) ? "vez" : "veces", ch->damage_amount, punct );
    }
        ...
        set_char_color( AT_GREEN, ch );
        snprintf( buf1, 256, "El ataque venenoso de $n %s a $N %d %s causándole %d puntos de daño%c", vs,
                  ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                  ch->damage_amount, punct );
        snprintf( buf2, 256, "Tu ataque venenoso %s a $N %d %s causándole %d puntos de daño%c", vs,
                  ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                  ch->damage_amount, punct );
        snprintf( buf3, 256, "El ataque venenoso de $n te %s %d %s causándote %d puntos de daño%c", vs,
                  ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                  ch->damage_amount, punct );
    }
    else {
        ...
        snprintf( buf1, 256, "El %s de $n %s a $N %d %s causándole  %d puntos de daño%c", attack, vs,
                  ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                  ch->damage_amount, punct );
        snprintf( buf2, 256, "Tu %s %s a $N %d %s causándole %d puntos de daño%c", attack, vs,
                  ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                  ch->damage_amount, punct );
        snprintf( buf3, 256, "El %s de $n te %s %d %s causándote %d puntos de daño%c", attack, vs,
                  ch->success_attack, ( ch->success_attack == 1 ) ? "vez" : "veces",
                  ch->damage_amount, punct );
    }
...
/*    CHAR_DATA *obj;  */
    ...
    if ( !IS_NPC( ch ) && IS_AFFECTED( ch, AFF_PRAYER ) ) {
        send_to_char( "No mientras estés en oración.\r\n", ch );
        return;
    }
    ...
    if ( IS_NPC( victim ) && victim->morph ) {
        send_to_char
            ( "Esta criatura parece extraña. Mírala más de cerca antes de intentar matarla.",
              ch );
        return;
    }
...
void do_murder( CHAR_DATA *ch, char *argument )
{
    ...
    if ( victim == ch ) {
        send_to_char( "El suicidio es de gente estúpida.\r\n", ch );
        return;
    }
 ...
 * gets shunted in the arena for accidentally being flagged PLR_NICE :P */
    ...
    WAIT_STATE( ch, 1 * PULSE_VIOLENCE );
    snprintf( buf, MSL, "%s grita histéricamente '¡%s me está intentando matar!", victim->name,
              IS_NPC( ch ) ? ch->short_descr : ch->name );
...
void do_flee( CHAR_DATA *ch, char *argument )
{
    ...
    for ( attempt = 0; attempt < 8; attempt++ ) {
        ...
        if ( ( now_in = ch->in_room ) == was_in )
            continue;
        ch->in_room = was_in;
        act( AT_FLEE, "¡$n ha huído!", ch, NULL, NULL, TO_ROOM );
        ch->in_room = now_in;
    ...
    act( AT_FLEE, "¡Intentas huir pero no encuentras una vía de escape!", ch, NULL, NULL, TO_CHAR );
    if ( !IS_AVATAR( ch ) && !IS_DUALAVATAR( ch ) && !IS_TRIAVATAR( ch )
