...
void bug_all_city_info( void )
{
    ...
    for ( city = first_city; city; city = city->next ) {
        bug( "%s: Def(%d) Off(%d) Cat(%d)(%d) Bal(%d) Sol(%d) Gua(%d) Arc(%d)(%d) War(%d)(%d) Arr(%d) Pla(%d) Ram(%d) Pit(%d)", city->name, city->defense, city->offense, city->ocatapult, city->dcatapult, city->ballista, city->soldiers, city->guards, city->oarchers, city->darchers, city->owarriors, city->dwarriors, city->arrows, city->platform, city->ram, city->pitch );
    }
}
...
void proc_chan( CHAR_DATA *ch, char *argument )
{
    ...
    snprintf( bufx, MIL,
              "&GUn pregonero lee apresuradamente una nueva proclamación gritando '¡Oíd! ¡oíd!': %s&D\r\n",
              argument );
...
void do_revoke( CHAR_DATA *ch, char *argument )
{
    ...
    if ( victim->pcdata->city != ch->pcdata->city ) {
        send_to_char( "¡Ese jugador no es de tu ciudad!\r\n", ch );
        return;
    }
    victim->pcdata->city = NULL;
    if ( VLD_STR( victim->pcdata->city_name ) )
        STRFREE( victim->pcdata->city_name );
    act( AT_MAGIC, "Exilias a $N de $t", ch, city->name, victim, TO_CHAR );
    act( AT_MAGIC, "$n exilia a $N de $t", ch, city->name, victim, TO_ROOM );
    act( AT_MAGIC, "$n  te exilia de $t", ch, city->name, victim, TO_VICT );
...
void do_checksieges( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !VLD_STR( argument ) ) {
        send_to_char( "uso: checksieges status\r\n", ch );
    ...
    if ( !str_cmp( argument, "endall" ) || !str_cmp( argument, "status" ) ) {
        for ( siege = first_siege; siege; siege = siege_next ) {
            ...
            if ( !endall ) {
                ch_printf( ch, "%s(%d) está atacando %s(%d).\r\n", siege->acity->name, offense,
                           siege->dcity->name, defense );
                continue;
            }
...
void do_setcity( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !VLD_STR( arg1 ) ) {
        ...
        send_to_char( " soldier archer catapult ballista pitch\r\n", ch );
    ...
    if ( !str_cmp( arg2, "duke" ) ) {
        ...
        for ( siege = first_siege; siege; siege = siege_next ) {
            ...
            ended = TRUE;
            sprintf( buf, "&C%s siege against %s has been ended early by %s!", siege->acity->name,
                     siege->dcity->name, ch->name );
            announce( buf );
            end_siege( siege );
            UNLINK( siege, first_siege, last_siege, next, prev );
            siege->acity = NULL;
...
void do_showcity( CHAR_DATA *ch, char *argument )
{
    ...
    ch_printf( ch, "   &wSoldiers: &W%d", city->soldiers );
    ch_printf( ch, " &wGuards: &W%d", city->guards );
...
void do_city( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !first_city ) {
        send_to_char( "No hay ciudades formadas.\r\n", ch );
        return;
    }
    if ( !VLD_STR( argument ) ) {
        send_to_char( "\r\n&WCiudades actuales de los reinos de &R6 Dragones&W\r\n", ch );
        send_to_char
            ( "&c----------------------------------------------------------------------------\r\n",
              ch );
        send_to_char( "\r\n&cCiudad                       Alianza           Duke\r\n", ch );
        for ( city = first_city; city; city = city->next )
        ...
        send_to_char( "\r\n&cUsa '&Wciudades &c<&Wnombre&c>' para más información.\r\n",
                      ch );
        return;
    }
    ...
    if ( city->duke ) {
        if ( !valid_player( city->duke ) ) {
            ...
            changed = TRUE;
            ch_printf( ch,
                       "Duke eliminado por player inválido. Repite el comando.\r\n" );
        }
    }
    if ( city->baron ) {
        if ( !valid_player( city->baron ) ) {
            ...
            changed = TRUE;
            ch_printf( ch,
                       "Ha habido un error. Repite el comando.\r\n" );
        }
    }
    if ( city->captain ) {
    ...
    if ( city->sheriff ) {
        if ( !valid_player( city->sheriff ) ) {
            ...
            changed = TRUE;
            ch_printf( ch,
                       "ha habido un error. Repite el comando de nuevo.\r\n" );
        }
    }
    if ( city->knight ) {
        if ( !valid_player( city->knight ) ) {
            ...
            changed = TRUE;
            ch_printf( ch,
                       "Se ha producido un error. Repite el comando.\r\n" );
        }
    }
    ...
    ch_printf( ch, "&c\r\n%s\r\n", city->name );
    ch_printf( ch, "&cDuke:       &w%s\r\n", city->duke ? city->duke : "No hay" );
    ch_printf( ch, "&cBarón:      &w%s\r\n", city->baron ? city->baron : "No hay" );
    ch_printf( ch, "&cCapitán:    &w%s\r\n", city->captain ? city->captain : "No hay" );
    ch_printf( ch, "&cSheriff:    &w%s\r\n", city->sheriff ? city->sheriff : "No hay" );
    ch_printf( ch, "&cCaballero:     &w%s\r\n", city->knight ? city->knight : "No hay" );
    ch_printf( ch, "&cAlianza  &w%s\r\n", city->allegiance ? city->allegiance : "No hay" );
    ch_printf( ch, "&cDefensa:    &w%d\r\n", city->defense );
    ch_printf( ch, "&cOfensa:    &w%d\r\n", city->offense );
    ch_printf( ch, "&cVictorias:       &w%d    &cDerrotas:      &w%d\r\n", city->wins, city->loses );
    ch_printf( ch, "&cDescripción:\r\n&w%s\r\n",
               city->description ? city->description : "No hay" );
}
...
void do_mpcity( CHAR_DATA *ch, char *argument )
{
    ...
    save_char_obj( victim );
    act( AT_WHITE, "$n hace ciudadano a $N de $t.", ch, city->name, victim, TO_NOTVICT );
    act( AT_WHITE, "$n te hace ciudadano de $t.", ch, city->name, victim, TO_VICT );

    if ( victim->race != RACE_DRAGON ) {
        // check what homeland they are in
        if ( victim->pcdata->city->min_house <= 0 || victim->pcdata->city->max_house <= 0
             || victim->pcdata->city->max_house <= victim->pcdata->city->min_house ) {
            send_to_char( "lo sentimos, tu ciudad no tiene ninguna vivienda.\r\n", ch );
            return;
        }
...
void update_city_def_off( CITY_DATA * city )
{
    ...
    city->guards += city->return_guards;
    city->defense += ( 2 * city->guards );
    city->return_guards = 0;
    ...
    city->soldiers += city->return_soldiers;
    city->offense += ( 2 * city->soldiers );
    city->return_soldiers = 0;
...
void do_fortify( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !bank ) {
        send_to_char( "¡Esta ciudad no tiene cuenta!\r\n", ch );
        return;
    }

    argument = one_argument( argument, arg1 );
    if ( !VLD_STR( arg1 ) ) {
        send_to_char( "Uso: fortificar <campo>\r\n", ch );
        send_to_char( "\r\nCampo puede ser:\r\n", ch );
        send_to_char( "brea  flechas catapultas guardias guerreros arqueros\r\n", ch );
        send_to_char( " Costes- Lista los costes de las fortificaciones\r\n", ch );
        return;
    }
    ...
    if ( !str_cmp( arg1, "costes" ) ) {
        ch_printf( ch, "Brea cuesta 50 monedas de oro. Tienes %d.\r\n", city->pitch );
        ch_printf( ch, "Flechas cuestan 10 monedas de oro. Tienes %d.\r\n", city->arrows );
        ch_printf( ch, "Catapultas cuestan 50 monedas de oro. Tienes %d.\r\n", city->dcatapult );
        ch_printf( ch, "Guardias cuestan 10 monedas de oro. Tienes %d.\r\n", city->guards );
        ch_printf( ch, "Arqueros cuestan 10 monedas de oro. Tienes %d.\r\n", city->darchers );
        ch_printf( ch, "Guerreros cuestan 10 monedas de oro. tienes %d .\r\n", city->dwarriors );
        ch_printf( ch, "Hay %d monedas de oro en la cuenta de %s.\r\n", bank->gold, city->name );
        return;
    }

    if ( is_in_siege( city ) ) {
        send_to_char( "No puedes fortificar durante un asedio\r\n", ch );
        return;
    }

    if ( !str_cmp( arg1, "brea" ) ) {
        if ( ( city->pitch + count ) > 10 ) {
            ch_printf( ch, "tu ciudad tiene %d calderos de brea, el máximo que puede tener es 10.\r\n",
                       city->pitch );
            return;
        }
        if ( bank->gold < ( 50 * count ) ) {
            ch_printf( ch, "Tu ciudad no puede aportar las 50 monedas de oro requeridas. solo tiene %d.\r\n", count );
            return;
        }
        bank->gold -= ( 50 * count );
        ch_printf( ch, "fortificas tu ciudad con %d calderos de ardiente brea.\r\n", count );
        city->pitch += count;
    }
    else if ( !str_cmp( arg1, "flechas" ) ) {
        if ( ( city->arrows + count ) > 50 ) {
            send_to_char( "No se puede tener más de 50 flechas.\r\n", ch );
            return;
        }
        if ( bank->gold < ( 10 * count ) ) {
            send_to_char( "Tu ciudad no puede permitírselo.\r\n", ch );
            return;
        }
        bank->gold -= ( 10 * count );
        send_to_char( "Fortificas tu ciudad con una docena de flechas.\r\n", ch );
        city->arrows += count;
    }
    else if ( !str_cmp( arg1, "catapultas" ) ) {
        if ( ( city->dcatapult + count ) > 10 ) {
            send_to_char( "solo se puede tener 10 catapultas.\r\n", ch );
            return;
        }
        if ( bank->gold < ( 50 * count ) ) {
            send_to_char( "Tu ciudad no puede permitírselo.\r\n", ch );
        ...
        bank->gold -= ( 50 * count );
        send_to_char( "Fortificas tu ciudad con una nueva catapulta.\r\n", ch );
        city->dcatapult += count;
    }
    else if ( !str_cmp( arg1, "guardias" ) ) {
        if ( ( city->guards + count ) > 50 ) {
            send_to_char( "Tu ciudad no puede tener más de 50 guardias.\r\n", ch );
            return;
        }
        if ( bank->gold < ( 10 * count ) ) {
            send_to_char( "tu ciudad no puede permitírselo.\r\n", ch );
        ...
        bank->gold -= ( 10 * count );
        send_to_char( "Fortificas tu ciudad con un nuevo guardia.\r\n", ch );
        city->guards += count;
    }
    else if ( !str_cmp( arg1, "arqueros" ) ) {
        if ( ( city->darchers + count ) > 50 ) {
            send_to_char( "No se pueden tener más de 50 arqueros.\r\n", ch );
            return;
        }
        if ( bank->gold < ( 10 * count ) ) {
            send_to_char( "Tu ciudad no puede permitírselo.\r\n", ch );
        ...
        bank->gold -= ( 10 * count );
        send_to_char( "fortificas tu ciudad con un nuevo arquero.\r\n", ch );
        city->darchers += count;
    }
    else if ( !str_cmp( arg1, "guerrero" ) ) {
        if ( ( city->dwarriors + count ) > 50 ) {
            send_to_char( "tu ciudad solo puede tener 50 guerreros.\r\n", ch );
            return;
        }
        if ( bank->gold < ( 10 * count ) ) {
            send_to_char( "Tu ciudad no puede permitírselo.\r\n", ch );
        ...
        bank->gold -= ( 10 * count );
        send_to_char( "Fortificas tu ciudad con un nuevo guerrero.\r\n", ch );
...
void do_siege( CHAR_DATA *ch, char *argument )
{
    ...
    city = get_city( ch->pcdata->city_name );
    if ( !city ) {
        send_to_char( "Ciudad inexistente.\r\n", ch );
        return;
    }

    bank = find_bank( city->bank );
    if ( !bank ) {
        send_to_char( "¡No hay cuenta bancaria para esa ciudad!\r\n", ch );
        return;
    }

    argument = one_argument( argument, arg1 );
    if ( !VLD_STR( arg1 ) ) {
        send_to_char( "Uso: asediar <campo> [<cantidad>]\r\n", ch );
        send_to_char( "\r\n:\r\n", ch );
        send_to_char( " catapultas rampas ballestas soldados arqueros guerreros plataformas\r\n",
                      ch );
        send_to_char( " Coste - muestra los precios de los artículos para asediar\r\n", ch );
        return;
    }
    ...
    if ( !str_cmp( arg1, "costs" ) ) {
        ch_printf( ch, "Ballestas cuestan 50 monedas de oro. La ciudad tiene %d.\r\n", city->ballista );
        ch_printf( ch, "Rampas cuestan 40 monedas de oro. tu ciudad tiene %d.\r\n", city->ram );
        ch_printf( ch, "Plataformas cuestan 50 monedas de oro. Tu ciudad tiene %d.\r\n", city->platform );
        ch_printf( ch, "Catapultss cuestan 50 monedas de oro. Tu ciudad tiene %d.\r\n", city->ocatapult );
        ch_printf( ch, "soldados cuestan 10 monedas de oro. Tu ciudad tiene %d.\r\n", city->soldiers );
        ch_printf( ch, "Arqueros cuestan 10 monedas de oro. Tu ciudad tiene %d.\r\n", city->oarchers );
        ch_printf( ch, "Guerreros cuestan 10 monedas de oro. Tu ciudad tiene %d.\r\n", city->owarriors );
        ch_printf( ch, "Tu ciudad tiene %d monedas de oro en la cuenta de %s.\r\n", bank->gold, city->name );
        return;
    }

    if ( is_in_siege( city ) ) {
        send_to_char( "No puedes hacer eso durante un asedio.\r\n", ch );
        return;
    }

    if ( !str_cmp( arg1, "ballesta" ) ) {
        if ( ( city->ballista + count ) > 6 ) {
            send_to_char( "La ciudad no puede tener más de 6 ballestas.\r\n", ch );
            return;
        }
        siegetype = 1;
        cost = ( 50 * count );
    }
    else if ( !str_cmp( arg1, "rampa" ) ) {
        if ( ( city->ram + count ) > 2 ) {
            send_to_char( "La ciudad solo puede tener 2 rampas.\r\n", ch );
            return;
        }
        siegetype = 2;
        cost = ( 40 * count );
    }
    else if ( !str_cmp( arg1, "catapulta" ) ) {
        if ( ( city->ocatapult + count ) > 6 ) {
            send_to_char( "La ciudad solo puede tener 6 catapultas.\r\n", ch );
            return;
        }
        siegetype = 3;
        cost = ( 50 * count );
    }
    else if ( !str_cmp( arg1, "soldado" ) ) {
        if ( ( city->soldiers + count ) > 50 ) {
            send_to_char( "La ciudad solo puede tener 50 soldados.\r\n", ch );
            return;
        }
        siegetype = 4;
        cost = ( 10 * count );
    }
    else if ( !str_cmp( arg1, "arquero" ) ) {
        if ( ( city->oarchers + count ) > 50 ) {
            send_to_char( "La ciudad solo puede tener 50 arqueros.\r\n", ch );
            return;
        }
        siegetype = 5;
        cost = ( 10 * count );
        ...
        if ( ( city->owarriors + count ) > 50 ) {
            send_to_char( "la ciudad solo puede tener 50 guerreros.\r\n", ch );
            return;
        }
        siegetype = 6;
        cost = ( 10 * count );
    }
    else if ( !str_cmp( arg1, "plataforma" ) ) {
        if ( ( city->platform + count ) > 6 ) {
            send_to_char( "La ciudad solo puede tener 6 plataformas.\r\n", ch );
            return;
        }
        siegetype = 7;
        cost = ( 50 * count );
    ...
    if ( bank->gold < cost ) {
        ch_printf( ch, "La ciudad no se puede permitir %d %s.\r\n", count, arg1 );
        return;
    }
    bank->gold -= cost;
...
void do_tax( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !bank ) {
        send_to_char( "¡No hay cuenta para la ciudad!\r\n", ch );
        return;
    }
    ...
    if ( IS_NPC( victim ) ) {
        send_to_char( "No es un ciudadano.\r\n", ch );
        return;
    }
    ...
    if ( !VLD_STR( arg1 ) ) {
        send_to_char( "Uso: impuesto <jugador>\r\n", ch );
        return;
    }

    if ( str_cmp( ch->pcdata->city_name, victim->pcdata->city_name ) ) {
        send_to_char( "No es un ciudadano de tu ciudad.\r\n", ch );
        return;
    }
    ...
    if ( ( currtime - victim->pcdata->warn ) < 3600 ) {
        send_to_char
            ( "Espera una hora de mud para recolectar más impuestos.\r\n",
              ch );
        return;
    }
    ...
    if ( amount > GET_MONEY( victim, CURR_GOLD ) ) {
        ...
        send_to_char( "No puede pagar tantos impuestos.\r\n", ch );
        send_to_char
            ( "No tienes tanto oro para pagar tus impuestos.\r\nYou have one mud hour to come up with 1 gold piece for your taxes.",
              victim );
        return;
    }
    ...
    ch_printf( ch, "\r\n&cCobas %d monedas de oro a %s para añadirlas a la cuenta de la ciudad.\r\n", amount,
               victim->name );
    ch_printf( victim, "\r\n&cPagas  %d monedas de oro como impuestos de tu ciudad.\r\n", amount );
    save_char_obj( victim );
    save_bank(  );
...
void do_contribute( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !bank ) {
        send_to_char( "¡No hay cuenta para esa ciudad!\r\n", ch );
        return;
    }

    if ( arg1[0] == '\0' ) {
        send_to_char( "uso: contribuir <cantidad>\r\n", ch );
        return;
    }
    ...
    ch_printf( ch, "\r\n&cContribuyes con %d monedas de oro a la cuenta del banco de la ciudad.\r\n", amount );
    save_char_obj( ch );
    save_bank(  );
}
        ...
        if ( !location )
            ...
            for ( rch = location->first_person; rch; rch = rch->next_in_room ) {
                ...
                if ( !rch->city || !rch->siege )
                    continue;
                if ( rch->pIndexData->vnum != city->soldier_mob
                     && rch->pIndexData->vnum != city->archer_mob )
                    continue;
    ...
    for ( oncount = 4; oncount <= 6; oncount++ ) {
        ...
        if ( !location )
            ...
            for ( rch = location->first_person; rch; rch = rch->next_in_room ) {
                ...
                if ( !rch->city || !rch->siege )
                    continue;
                if ( rch->pIndexData->vnum != acity->soldier_mob
                     && rch->pIndexData->vnum != acity->archer_mob )
                    continue;
...
void end_siege( SIEGE_DATA * siege )
{
    ...
    for ( oncount = 1; oncount <= 6; oncount++ ) {
        ...
        for ( obj = location->first_content; obj; obj = obj_next ) {
            ...
            if ( !obj->city || !obj->siege || obj->siege != siege )
                continue;
    ...
    if ( attackers > defenders ) {
        sprintf( buf, "&C¡%s ha derrotado a %s y han asaltado las murallas de la ciudad!", acity->name,
                 city->name );
        announce( buf );
        acity->wins += 1;
        city->loses += 1;
        ...
        for ( wch = first_char; wch; wch = wch->next ) {
            if ( IS_NPC( wch ) || !IS_CITY( wch ) || IS_IMMORTAL( wch ) )
                continue;
            if ( VLD_STR( wch->pcdata->city_name )
                 && !str_cmp( wch->pcdata->city_name, acity->name ) ) {
                send_to_char( "&cGanas &W10&c puntos de gloria por ganar el asedio!\r\n", wch );
                wch->quest_curr += 10;
            }
        }
    }
    else {
        sprintf( buf, "&C¡%s ha derrotado a %s y protejido sus murallas!", city->name,
                 acity->name );
        announce( buf );
        city->wins += 1;
        acity->loses += 1;
        ...
        for ( wch = first_char; wch; wch = wch->next ) {
            if ( IS_NPC( wch ) || !IS_CITY( wch ) || IS_IMMORTAL( wch ) )
                continue;
            if ( VLD_STR( wch->pcdata->city_name )
                 && !str_cmp( wch->pcdata->city_name, city->name ) ) {
                send_to_char( "&cGanas &W10&c puntos de gloria por ganar el asedio!\r\n", wch );
                wch->quest_curr += 10;
            }
        }
...
void handle_sieges( void )
{
    ...
    for ( siege = first_siege; siege; siege = siege_next ) {
        ...
        if ( ( ( siege->siege_started + 1800 ) - current_time ) == 1200 ) {
            snprintf( buf, sizeof( buf ), "%s", "&CEn 20 minutos, " );
            if ( offense > defense )
                snprintf( buf + strlen( buf ), sizeof( buf ) - strlen( buf ),
                          "¡%s muestra signos de caer sobre %s!", siege->dcity->name,
                          siege->acity->name );
            else
                snprintf( buf + strlen( buf ), sizeof( buf ) - strlen( buf ),
                          "%s demuestra signos de celebrar la caída de %s!", siege->dcity->name,
                          siege->acity->name );
            announce( buf );
        }
        else if ( ( ( siege->siege_started + 1800 ) - current_time ) == 600 ) {
            snprintf( buf, sizeof( buf ), "%s", "&CEn 10 minutos, " );
            if ( offense > defense )
                snprintf( buf + strlen( buf ), sizeof( buf ) - strlen( buf ),
                          "%s muestra signos de caer sobre %s!", siege->dcity->name,
                          siege->acity->name );
                ...
                snprintf( buf + strlen( buf ), sizeof( buf ) - strlen( buf ),
                          "%s muestra signos de celebrar la derrota de %s!", siege->dcity->name,
                          siege->acity->name );
            announce( buf );
        }
        else if ( ( ( siege->siege_started + 1800 ) - current_time ) == 180 ) {
            snprintf( buf, sizeof( buf ), "%s", "&CEn 3 minutos, " );
            if ( offense > defense )
                snprintf( buf + strlen( buf ), sizeof( buf ) - strlen( buf ),
                          "%s muestra signos de caer sobre %s!", siege->dcity->name,
                          siege->acity->name );
                ...
                snprintf( buf + strlen( buf ), sizeof( buf ) - strlen( buf ),
                          "%s muestra signos de celebrar la caída de off %s!", siege->dcity->name,
                          siege->acity->name );
            announce( buf );
        }
...
void do_laysiege( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_IMMORTAL( ch ) && ch->pcdata->next_siege > current_time ) {
        ...
        send_to_char( "Inténtalo de nuevo en", ch );
        if ( uhours > 0 )
            ch_printf( ch, " %d hora%s", uhours, uhours == 1 ? "" : "s" );
        if ( umins > 0 )
            ch_printf( ch, " %d minuto%s", umins, umins == 1 ? "" : "s" );
        if ( utime > 0 )
            ch_printf( ch, " %d segundo%s", utime, utime == 1 ? "" : "s" );
        send_to_char( "\r\n", ch );
        return;
    }
    ...
    if ( !VLD_STR( arg1 ) ) {
        send_to_char( "uso: asedio <ciudad>\r\n", ch );
        if ( IS_IMMORTAL( ch ) ) {
    ...
    if ( get_timer( ch, TIMER_RECENTFIGHT ) > 0 && !IS_IMMORTAL( ch ) ) {
        ...
        send_to_char( "¡Tienes demasiada adrenalina!\r\n", ch );
        return;
    }
    ...
    if ( IS_IMMORTAL( ch ) && !str_cmp( arg1, "fullwar" ) ) {
        for ( ocity = first_city; ocity; ocity = ocity->next ) {
            for ( dcity = first_city; dcity; dcity = dcity->next ) {
                ...
                if ( !siege ) {
                    ch_printf( ch, "Error al crear un asedio de %s atacando %s.\r\n", ocity->name,
                               dcity->name );
                    continue;
                }

                sprintf( buf, "&C%s ha comenzado el asedio a %s!", ocity->name, dcity->name );
                announce( buf );
    ...
    if ( !ocity ) {
        if ( !IS_IMMORTAL( ch ) || !VLD_STR( argument ) )
            send_to_char( "No eres ciudadano de ninguna ciudad.\r\n", ch );
        else
            ch_printf( ch, "No hay ciudad llamada %s.\r\n", argument );
        return;
    }

    if ( !dcity ) {
        send_to_char( "No existe esa ciudad.\r\n", ch );
        return;
    }
    ...
    if ( ocity == dcity ) {
        ch_printf( ch, "Por qué %s quiere asediarse a si misma?\r\n", ocity->name );
        return;
    }

    if ( !IS_IMMORTAL( ch ) && ch->in_room->vnum != dcity->siege1_room ) {
        send_to_char( "No estás en un lugar adecuado para asediar esta ciudad.\r\n", ch );
        return;
    }

    if ( is_sieging( ocity, dcity ) ) {
        ch_printf( ch, "%s  ya está atacando %s.\r\n", ocity->name, dcity->name );
        return;
    }
    ...
    sprintf( buf, "&C%s ha comenzado el asedio de %s!", ocity->name, dcity->name );
    announce( buf );
...
void do_proclaim( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !VLD_STR( argument ) ) {
        send_to_char( "uso: proclamar <mensaje>\r\n", ch );
        return;
    }
...
void do_sabotage( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !obj || obj->item_type != ITEM_SABOTAGE || !obj->city || !obj->siege ) {
        send_to_char( "Nada que sabotear.\r\n", ch );
        return;
    }

    if ( !VLD_STR( ch->pcdata->city_name ) || !( ucity = get_city( ch->pcdata->city_name ) ) ) {
        send_to_char( "No puedes sabotear nada si no eres un ciudadano de ninguna ciudad.\r\n", ch );
        return;
    }
     ...
     */
    if ( city == ucity ) {
        send_to_char( "¿Para qué dañar tu ciudad?\r\n", ch );
        return;
    }
    ...
    if ( chance < 5 ) {
        ch_printf( ch, "&cIntentas sabotear %s, pero fallas al completar tu tarea.\r\n",
                   obj->short_descr );
        if ( soldier ) {
            interpret( soldier, ( char * ) "mpecho" );
            if ( soldier->pIndexData->vnum == city->archer_mob )
                interpret( soldier, ( char * )
                           "mpecho &c¡Un arquero grita al observar el intento de sabotage!&D" );
            else
                interpret( soldier, ( char * )
                           "mpecho &cUn soldado grita al observar el intento de sabotage!&D" );
            interpret( soldier, ( char * ) "mpecho" );
            if ( !soldier->fighting )
    ...
    ch_printf( ch, "&cIntentas sabotear %s.\r\nYou successfully destroyed %s.\r\n",
               obj->short_descr, obj->short_descr );
    if ( soldier ) {
        interpret( soldier, ( char * ) "mpecho" );
        if ( soldier->pIndexData->vnum == city->archer_mob )
            interpret( soldier, ( char * )
                       "mpecho &c¡Un soldado grita al observar el acto de sabotaje!&D" );
        else
            interpret( soldier, ( char * )
                       "mpecho &c¡Un soldado grita cuando observa el intento de sabotaje!&D" );
        interpret( soldier, ( char * ) "mpecho" );
        if ( !soldier->fighting )
            set_fighting( soldier, ch );
    }
    ...
    if ( rand > 6 ) {
        send_to_char( "&W¡Ganas un punto de gloria al sabotear al enemigo!\r\n", ch );
        ch->quest_curr += 1;
    }
    else
...
void do_rollcall( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !argument || argument[0] == '\0' ) {
        send_to_char( "uso: ciudadanos <ciudad>\r\n", ch );
        if ( IS_IMMORTAL( ch ) || IS_DUKE( ch ) || IS_BARON( ch ) )
            send_to_char( "uso: ciudadanos <ciudad> remove <name>\r\n", ch );
        return;
    }

    argument = one_argument( argument, arg );
    if ( !( city = get_city( arg ) ) ) {
        ch_printf( ch, "No hay una ciudad llamada as %s\r\n", arg );
        return;
    }

    if ( !argument || argument[0] == '\0' ) {
        ch_printf( ch, "Lista de ciudadanos de %s\r\n\r\n", city->name );
        ch_printf( ch, "%-15.15s  %-15.15s %-6.6s %-6.6s %-6.6s %s\r\n", "Nombre", "Classe", "Nivel",
                   "Asesinatos", "Muertes", "Se unió" );
        send_to_char
            ( "-------------------------------------------------------------------------------------\r\n",
              ch );
        for ( rollcall = city->first_citizen; rollcall; rollcall = rollcall->next ) {
    ...
    if ( IS_IMMORTAL( ch ) || ( city->duke && !str_cmp( ch->name, city->duke ) )
        ...
        if ( !str_cmp( arg2, "remove" ) ) {
            ...
            save_city( city );
            ch_printf( ch, "%s deja de ser un ciudadano de %s\r\n", argument,
                       city->name );
            return;
        }
    }
            ...
            case 'C':
                if ( !str_cmp( word, "Class" ) ) {
                    ...
                    if ( Class < 0 || Class >= MAX_NPC_CLASS ) {
                        ...
                        Class = get_npc_class( ( char * ) "soldier" );
                    }
                    STRFREE( temp );
                    rollcall->Class = Class;
                    fMatch = TRUE;
