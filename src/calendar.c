...
void do_timezone( CHAR_DATA *ch, char *argument )
{
    ...
    if ( i == -1 ) {
        send_to_char( "Esa zona horaria no existe. Asegúrate de usar el nombre exacto.\r\n", ch );
        return;
    }
...
const char             *const day_name[] = {
    "de la Luna", "del Toro", "de la Decepción", "del Trueno", "de la Libertad",
...
const char             *const month_name[] = {
    "la Gran Guerra", "la Renovación", "la Naturaleza", "la Esperanza", /* primavera */
    "el Dragón", "el Sol", "el Calor", "la Batalla", /* verano */
...
const char             *const season_name[] = {
    "&gprimavera", "&Yverano", "&Ootoño", "&Cinvierno"
};
    ...
    for ( ;; ) {
        ...
        switch ( UPPER( word[0] ) ) {
            ...
            case 'M':
                ...
                KEY( "Mdía", time_info.day, fread_number( fp ) );
                KEY( "Mmes", time_info.month, fread_number( fp ) );
                KEY( "Maño", time_info.year, fread_number( fp ) );
                break;
        }
...
void do_time( CHAR_DATA *ch, char *argument )
{
    ...
    ch_printf( ch,
               "&wSon las &W%d&w en punto &W%s&w, Día de &W%s&w,&W %d%s&w en el Mes de &W%s&w.\r\n"
               "&wLa estación del año es %s&w, año &W%d&w.\r\n"
               "&wEl mud se inició a las  :  &W %s\r\n" "&wLa hora del sistema es        :  &W %s\r\n",
    ...
    if ( holiday != NULL )
        ch_printf( ch, "&wHoy es un día de fiesta:&W %s\r\n", holiday->name );

    if ( !IS_NPC( ch ) ) {
        if ( day == ch->pcdata->day + 1 && time_info.month == ch->pcdata->month )
            send_to_char( "&WHoy es tu &Pc&pu&Ym&Opl&Ye&pa&Pños&R!&D\r\n", ch );
    }
...
void start_winter( void )
{
    ...
    echo_to_all( AT_CYAN, "El aire adquiere un frío escalofriante... El invierno ha llegado.", ECHOTAR_ALL );
...
void start_spring( void )
{
    ...
    echo_to_all( AT_DGREEN, "El frío se aleja del aire mientras la primavera comienza.",
                 ECHOTAR_ALL );
...
{
    echo_to_all( AT_YELLOW, "Los días se alargan y se tornan calurosos con el comienzo del verano.\r\n",
                 ECHOTAR_ALL );
    return;
}
...
{
    echo_to_all( AT_ORANGE, "Las hojas comienzan a cambiar el color, ha llegado el otoño.\r\n",
                 ECHOTAR_ALL );
    return;
}
...
void do_holidays( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_pager( "&RFiesta		       &YMes	        &GDía\r\n", ch );
    send_to_pager( "&g----------------------+----------------+---------------\r\n", ch );
...
/* What's the month? */
    ...
    if ( !str_cmp( arg2, "announce" ) ) {
        if ( *arg3 == '\0' || !str_cmp( arg3, " " ) || is_number( arg3 ) ) {
            send_to_char( "¿Qué anuncio quieres poner?\r\n", ch );
            return;
        }
...
/* Change the name */
    if ( !str_cmp( arg2, "name" ) ) {
        if ( *arg3 == '\0' || !str_cmp( arg3, " " ) || is_number( arg3 ) ) {
            send_to_char( "¿Qué nombre le quieres poner?\r\n", ch );
            return;
        }
...
/* Release the good...err... name */
    ...
    if ( !str_cmp( arg2, "delete" ) ) {
        if ( str_cmp( arg3, "yes" ) ) {
            send_to_char( "Si estás seguro, escribe 'delete yes'.\r\n", ch );
            return;
        }
