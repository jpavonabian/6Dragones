...
const char             *const chan_types[] = {
    "Global", "Zone", "alianza", "Council", "PK", "Log", "Room", "Secret", "multitud",
    "Halcyon", "Paleon", "Dakar", "prohibida"
};
...
void do_listen( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !argument || argument[0] == '\0' ) {
        send_to_char( "&GSintaxis: escuchar\r\n", ch );
        send_to_char( "&GSintaxis: escuchar todos\r\n", ch );
        send_to_char( "&GSintaxis: escuchar ninguno\r\n", ch );
        send_to_char( "&GPara una lista de canales, escribe &Wcanales\r\n", ch );
        send_to_char( "&YEstás escuchando los canales siguientes:\r\n\r\n", ch );
        ch_printf( ch, "&W%s\r\n", ch->pcdata->chan_listen );
        return;
    }
    if ( !str_cmp( argument, "todos" ) ) {
        for ( channel = first_channel; channel; channel = channel->next ) {
        ...
        send_to_char( "&YAhora escuchas todos los canales disponibles.\r\n", ch );
        return;
    }
    ...
    if ( !str_cmp( argument, "ninguno" ) ) {
        ...
        channel = find_channel( "decir" );
        addname( &ch->pcdata->chan_listen, channel->name );
        /*
         * Volk - still should listen to the SAY channel 
         */
        send_to_char( "&Yya no escuchas nada.\r\n", ch );
        return;
    }
    if ( hasname( ch->pcdata->chan_listen, argument ) ) {
        if ( !str_cmp( argument, "decir" ) && !IS_IMMORTAL( ch ) ) {
            send_to_char( "Los mortales no pueden dejar de escuchar este canal.\r\n", ch );
            return;
        }
        removename( &ch->pcdata->chan_listen, argument );
        ch_printf( ch, "&YYa no escuchas el canal &W%s\r\n", argument );
    }
    else {
        if ( !( channel = find_channel( argument ) ) ) {
        ...
        if ( ( channel->level > ch->level ) && ( channel->level > ch->trust ) ) {
            send_to_char( "Ese canal no es de tu nivel.\r\n", ch );
            return;
        }
        addname( &ch->pcdata->chan_listen, argument );
        ch_printf( ch, "&YAhora escuchas el canal &W%s\r\n", channel->name );
    }
    return;
}
...
void do_channels( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char( "&YLos siguientes canales están disponibles:\r\n", ch );
    send_to_char( "Para activar o desactivar un canal usa &Wescuchar&Y.\r\n\r\n", ch );
    send_to_char( "&WCanales        On/Off&D\r\n", ch );
    send_to_char( "&B-----------------------&D\r\n", ch );
    for ( channel = first_channel; channel; channel = channel->next ) {
        if ( ( ch->level >= channel->level ) || ( ch->trust >= channel->level ) ) {
...
void send_tochannel( CHAR_DATA *ch, MUD_CHANNEL * channel, char *argument )
{
    ...
    if ( ch->chan_invite != 1 && channel->type == CHAN_SECRET ) {
        send_to_char( "No tienes permiso para hablar en ese canal.\r\n", ch );
        return;
    }
    ...
    if ( !IS_CLANNED( ch ) && !IS_IMMORTAL( ch ) ) {
        if ( !str_cmp( channel->name, "alianza" ) || !str_cmp( channel->name, "multitud" )
    ...
    if ( !IS_CITY( ch ) && !IS_IMMORTAL( ch ) ) {
        if ( !str_cmp( channel->name, "halcyon" ) || !str_cmp( channel->name, "paleon" )
             || !str_cmp( channel->name, "prohibida" ) ) {
            send_to_char( "Huh?\r\n", ch );
            return;
        }
    }
    ...
    if ( IS_CLANNED( ch ) && !IS_IMMORTAL( ch ) ) {
        if ( str_cmp( ch->pcdata->clan_name, "multitud" ) && !str_cmp( channel->name, "multitud" ) ) {
            send_to_char( "Huh?\r\n", ch );
            return;
        }
        ...
        if ( str_cmp( ch->pcdata->clan_name, "Alianza" ) && !str_cmp( channel->name, "alianza" ) ) {
            send_to_char( "Huh?\r\n", ch );
            return;
        }
    }
    ...
    if ( IS_CITY( ch ) && !IS_IMMORTAL( ch ) ) {
        if ( str_cmp( ch->pcdata->city_name, "Ciudad Paleon" )
             && !str_cmp( channel->name, "paleon" ) ) {
            send_to_char( "Huh?\r\n", ch );
            return;
        }
        if ( str_cmp( ch->pcdata->city_name, "Ciudad Dakar " ) && !str_cmp( channel->name, "dakar" ) ) {
            send_to_char( "Huh?\r\n", ch );
            return;
        }
        if ( str_cmp( ch->pcdata->city_name, "Ciudad Prohibida" )
             && !str_cmp( channel->name, "prohibida" ) ) {
            send_to_char( "Huh?\r\n", ch );
            return;
        }
    ...
    if ( ch->in_room && IS_SET( ch->in_room->room_flags, ROOM_SILENCE ) && !IS_IMMORTAL( ch ) ) {
        if ( !IS_NPC( ch ) )
            send_to_char( "¡La sala absorbe tus palabras!\r\n", ch );
        return;
    }
    ...
    if ( !VLD_STR( argument ) ) {
        ...
        ch_printf( ch, "&cLos 20 últimos mensajes del canal %s:\r\n", channel->name );
        if ( !str_cmp( channel->name, "decir" ) ) {
            for ( x = 0; x < 20; x++ ) {
     ...
     */
    color = -1;
    if ( xIS_SET( ch->act, PLR_COMMUNICATION ) && ( !str_cmp( channel->name, "ooc" ) || !str_cmp( channel->name, "chat" ) ||
        !str_cmp( channel->name, "icc" )))
        ch_printf( ch, "!!SOUND(sound/ooc.wav)\r\n" );
   	    if ( IS_IMMORTAL(ch) && !str_cmp( channel->name, "sooc" )) {
    ...
    if ( !str_cmp( channel->name, "ooc" ) )
        color = AT_LBLUE;
    if ( !str_cmp( channel->name, "alianza" ) )
        color = AT_LBLUE;
    if ( !str_cmp( channel->name, "multitud" ) )
        color = AT_RED;
    ...
    if ( !str_cmp( channel->name, "paleon" ) )
        color = AT_LBLUE;
    if ( !str_cmp( channel->name, "prohibida" ) )
        color = AT_LBLUE;
    ...
    if ( !str_cmp( channel->name, "think" ) )
        color = AT_THINK;
    if ( !str_cmp( channel->name, "subasta" ) )
        color = AT_AUCTION;
    ...
    if ( ( !str_cmp( channel->name, "decir" ) ) || ( !str_cmp( channel->name, "osay" ) ) ) {
        ch_printf( ch, "%sdices '%s'\r\n", col, argument );
    }
    else {
        if ( social ) {
    ...
    for ( vch = first_char; vch; vch = vch->next ) {
        ...
        if ( IS_PUPPET( vch ) ) {
            ...
            if ( channel->type == CHAN_ALLIANCE ) {
                if ( IS_NPC( vch ) )
                    continue;
                if ( !vch->pcdata->clan_name || str_cmp( vch->pcdata->clan_name, "alianza" ) )
                    continue;
            }
            ...
            if ( channel->type == CHAN_THRONG ) {
                if ( IS_NPC( vch ) )
                    continue;
                if ( !vch->pcdata->clan_name || str_cmp( vch->pcdata->clan_name, "multitud" ) )
                    continue;
            }
            ...
            if ( channel->type == CHAN_PALEON ) {
                if ( IS_NPC( vch ) )
                    continue;
                if ( !vch->pcdata->city_name || str_cmp( vch->pcdata->city_name, "Ciudad Paleon" ) )
                    continue;
            }
            ...
            if ( channel->type == CHAN_DAKAR ) {
                if ( IS_NPC( vch ) )
                    continue;
                if ( !vch->pcdata->city_name || str_cmp( vch->pcdata->city_name, "Ciudad Dakar" ) )
                    continue;
            }
            ...
            if ( channel->type == CHAN_FORBIDDEN ) {
                ...
                if ( !vch->pcdata->city_name
                     || str_cmp( vch->pcdata->city_name, "Ciudad Prohibida" ) )
                    continue;
            }
        ...
        if ( !IS_NPC( ch ) && ( !str_cmp( channel->name, "ooc" ) || !str_cmp( channel->name, "chat" ) ||
        !str_cmp( channel->name, "icc" ))) {
            if ( xIS_SET( vch->act, PLR_COMMUNICATION ) )
                ch_printf( vch, "!!SOUND(sound/ooc.wav)\r\n" );
        }
        ...
        if ( vch->desc->connected == CON_PLAYING
            ...
            if ( channel->type == CHAN_ALLIANCE ) {
                if ( IS_NPC( vch ) )
                    continue;
                if ( str_cmp( vch->pcdata->clan_name, "alianza" ) )
                    continue;
            }
            ...
            if ( channel->type == CHAN_THRONG ) {
                if ( IS_NPC( vch ) )
                    continue;
                if ( str_cmp( vch->pcdata->clan_name, "multitud" ) )
                    continue;
            }
            ...
            if ( ( !str_cmp( channel->name, "decir" ) )  /* || (!str_cmp(channel->name, * * 
                                                        * "osay")) */  ) {
                if ( !social && !emote ) {
                    snprintf( buf, MSL, "$n dice '$t'"  );
                    act( AT_SAY, strcat( lbuf, buf ), ch, sbuf, vch, TO_VICT );
                ...
                if ( !IS_NPC( vch ) ) {
                    snprintf( logbuf, MSL,
                              "&R[%-2.2d/%-2.2d %-2.2d:%-2.2d] &B%s dijo '&W %s &D&B'&D",
     ...
     * Do programs 
     */
    if ( !str_cmp( channel->name, "osay" ) || !str_cmp( channel->name, "decir" ) ) {
