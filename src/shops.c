...
CHAR_DATA              *find_keeper( CHAR_DATA *ch )
{
    ...
    if ( !IS_NPC( ch ) && xIS_SET( ch->act, PLR_KILLER ) ) {
        ...
        snprintf( buf, MSL, "grita %s el ASESINO está aquí!\r\n", ch->name );
    ...
    if ( !IS_NPC( ch ) && xIS_SET( ch->act, PLR_THIEF ) ) {
        ...
        snprintf( buf, MSL, "grita  %s el ladrón está aquí!\r\n", ch->name );
        interpret( keeper, buf );
        return NULL;
    }
    ...
    if ( time_info.hour < pShop->open_hour ) {
        interpret( keeper, ( char * ) "dice  Lo siento, está cerrado, vuelve ma´s tarde." );
        return NULL;
    }

    if ( time_info.hour > pShop->close_hour ) {
        interpret( keeper, ( char * ) "say Lo siento, está cerrado, vuelve mañana." );
        return NULL;
    }
    ...
    if ( ( number_percent(  ) % 65 ) > speakswell ) {
        if ( speakswell > 60 )
            ...
            snprintf( buf, MSL, "%s Podrías decirlo un poco más claramente?", ch->name );
        else if ( speakswell > 40 )
            snprintf( buf, MSL, "%s Disculpa... Que querías?", ch->name );
        else
...
CHAR_DATA              *find_fixer( CHAR_DATA *ch )
{
    ...
    if ( time_info.hour < rShop->open_hour ) {
        interpret( keeper, ( char * ) "dice Lo siento, está cerrado, vuelve más tarde." );
        return NULL;
    }

    if ( time_info.hour > rShop->close_hour ) {
        interpret( keeper, ( char * ) "Dice Lo siento, está cerrado, vuelve mañana." );
        return NULL;
    }
    ...
    if ( ( number_percent(  ) % 65 ) > speakswell ) {
        if ( speakswell > 60 )
            ...
            snprintf( buf, MSL, "%s Lo siento... Qué querías?", ch->name );
        else
            snprintf( buf, MSL, "%s No te entiendo.", ch->name );
        do_tell( keeper, buf );
        return NULL;
...
void do_buy( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_SET( ch->in_room->room_flags, ROOM_PET_SHOP ) ) {
        ...
        act( AT_ACTION, "$n compró $N como mascota.", ch, NULL, pet, TO_ROOM );
        return;
    }
...
void do_list( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_SET( ch->in_room->room_flags, ROOM_PET_SHOP ) ) {
        ...
        if ( !str_cmp( arg, "stat" ) ) {
            ...
            act( AT_LBLUE, buf, ch, obj, keeper, TO_CHAR );
            snprintf( buf, MSL, "Tamaño necesario para llevarlo: %s\r\n", obj_sizes[obj->size] );
            act( AT_LBLUE, buf, ch, obj, keeper, TO_CHAR );
            if ( inventory )
            ...
            switch ( obj->item_type ) {
                ...
                case ITEM_WEAPON:
                    ...
                    act( AT_LBLUE, buf, ch, obj, keeper, TO_CHAR );
                    snprintf( buf, MSL, "Tipo de daño:  %s",
...
void do_sell( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !can_see_obj( keeper, obj ) ) {
        send_to_char( "Qué intentas venderme? No compro aire!\r\n", ch );
        return;
    }
    ...
    if ( obj->timer > 0 ) {
        act( AT_TELL, "$n te cuenta, '$p se deprecia en valor demasiado rápido...'", keeper, obj, ch,
             TO_VICT );
        return;
    }
    ...
    if ( GET_VALUE( obj, type ) == CURR_GOLD ) {
        ...
        if ( GET_MONEY( keeper, CURR_GOLD ) < cost ) {
            act( AT_TELL, "$n te cuenta 'No llevo suficiente oro para comprar $p.'", keeper, obj, ch,
                 TO_VICT );
            act_printf( AT_TELL, keeper, obj, ch, TO_VICT,
                        "$n te cuenta 'Solo tengo %d de oro inténtalo más tarde'",
    ...
    if ( GET_VALUE( obj, type ) == CURR_SILVER ) {
        if ( GET_MONEY( keeper, CURR_SILVER ) < cost ) {
            act( AT_TELL, "$n te cuenta 'No tengo suficiente plata para comprar $p.'", keeper, obj, ch,
                 TO_VICT );
            act_printf( AT_TELL, keeper, obj, ch, TO_VICT,
                        "$n te cuenta 'Lo máximo que te puedo dar por esto es %d de plata'",
    ...
    if ( GET_VALUE( obj, type ) == CURR_BRONZE ) {
        if ( GET_MONEY( keeper, CURR_BRONZE ) < cost ) {
            act( AT_TELL, "$n te cuenta 'No tengo suficiente bronce para comprar $p.'", keeper, obj, ch,
                 TO_VICT );
            act_printf( AT_TELL, keeper, obj, ch, TO_VICT,
                        "$n te cuenta 'Lo máximo que te puedo dar por eso es %d de bronce.'",
    ...
    if ( GET_VALUE( obj, type ) == CURR_COPPER ) {
        if ( GET_MONEY( keeper, CURR_COPPER ) < cost ) {
            act( AT_TELL, "$n te cuenta 'No tengo suficiente cobre para comprar $p.'", keeper, obj, ch,
                 TO_VICT );
            act_printf( AT_TELL, keeper, obj, ch, TO_VICT,
                        "$n te cuenta 'Lo máximo que te puedo dar por eso es %d de cobre.'",
...
void do_value( CHAR_DATA *ch, char *argument )
{
    ...
    primcurr = get_primary_curr( keeper->in_room );
    if ( GET_VALUE( obj, type ) == CURR_GOLD ) {
        snprintf( buf, MSL, "$n te cuenta 'Te daré %d %s monedas por $p.'", cost,
                  curr_types[primcurr] );
        act( AT_TELL, buf, keeper, obj, ch, TO_VICT );
        ch->reply = keeper;
    }
    else if ( GET_VALUE( obj, type ) == CURR_SILVER ) {
        snprintf( buf, MSL, "$n te cuenta 'Te daré %d monedas de plata por $p.'", cost );
        act( AT_TELL, buf, keeper, obj, ch, TO_VICT );
        ch->reply = keeper;
    }
    else if ( GET_VALUE( obj, type ) == CURR_BRONZE ) {
        snprintf( buf, MSL, "$n te cuenta 'Te daré %d monedas de bronce por $p.'", cost );
        act( AT_TELL, buf, keeper, obj, ch, TO_VICT );
        ch->reply = keeper;
    }
    else if ( GET_VALUE( obj, type ) == CURR_COPPER ) {
        snprintf( buf, MSL, "$n te cuenta 'Te daré %d monedas de cobre por $p.'", cost );
        act( AT_TELL, buf, keeper, obj, ch, TO_VICT );
        ch->reply = keeper;
    }
    else {
                     ...
                     char *fixstr, char *fixstr2 )
{
    ...
    if ( !can_drop_obj( ch, obj ) )
        ...
        snprintf( buf, MSL, "$N te cuenta, 'Te costará %d moneda%s de %s a %s %s...'", cost,
        ...
        snprintf( buf, MSL, "$n le da $p a $N, que rápidamente lo %s.", fixstr2 );
        ...
        switch ( obj->item_type ) {
            default:
                send_to_char( "Por algún motivo, crees que has sido estafado...\r\n", ch );
                break;
            case ITEM_ARMOR:
...
void do_repair( CHAR_DATA *ch, char *argument )
{
    ...
    if ( argument[0] == '\0' ) {
        send_to_char( "Qué quieres reparar?\r\n", ch );
        return;
    }
...
void appraise_all( CHAR_DATA *ch, CHAR_DATA *keeper, char *fixstr )
{
    ...
    for ( obj = ch->first_carrying; obj != NULL; obj = obj->next_content ) {
        if ( obj->wear_loc == WEAR_NONE && can_see_obj( ch, obj )
            ...
            if ( !can_drop_obj( ch, obj ) )
                ...
                snprintf( buf, MSL, "$N te cuenta, 'Te costará %d pieza%s de %s a %s %s'", cost,
    ...
    if ( total > 0 ) {
        send_to_char( "\r\n", ch );
        snprintf( buf, MSL, "$N te cuenta, 'Te costará %d pieza%s de %s en total.'", total,
...
void do_appraise( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' ) {
        send_to_char( "Qué quieres evaluar?\r\n", ch );
        return;
    }
    ...
    snprintf( buf, MSL, "$N te cuenta, 'Te costará %d pieza%s de bronce a %s eso...'", cost,
              cost == 1 ? "" : "s", fixstr );
    act( AT_TELL, buf, ch, NULL, keeper, TO_CHAR );
...
void do_shopset( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !str_cmp( arg2, "buy0" ) ) {
        ...
        if ( value < 0 || value > MAX_ITEM_TYPE ) {
            send_to_char( "Tipo de objeto no válido!\r\n", ch );
            return;
        }
...
void do_makerepair( CHAR_DATA *ch, char *argument )
{
    ...
    if ( mob->rShop ) {
        send_to_char( "Este mob ya tiene una tienda de reparación.\r\n", ch );
        return;
    }
...
void do_repairset( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !mob->rShop ) {
        send_to_char( "Este mob no guarda una tienda de reparación.\r\n", ch );
        return;
    }
    repair = mob->rShop;
    value = atoi( argument );
    ...
    if ( !str_cmp( arg2, "keeper" ) ) {
        ...
        if ( !can_medit( ch, mob ) )
            return;
        if ( mob2->rShop ) {
            send_to_char( "Ese mob ya tiene una tienda de reparación.\r\n", ch );
            return;
        }
        mob->rShop = NULL;
        mob2->rShop = repair;
...
void do_repairshops( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !first_repair ) {
        send_to_char( "No hay tiendas de reparación.\r\n", ch );
        return;
    }
