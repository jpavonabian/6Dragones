...
void do_htown_outcast( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !VLD_STR( arg ) ) {
        send_to_char( "¿Expulsar a quien de la ciudad?\r\n", ch );
        return;
    }
    ...
    if ( victim->pcdata->htown != ch->pcdata->htown ) {
        send_to_char( "¡No pertenece a tu ciudad!\r\n", ch );
        return;
    }
    --htown->members;
    ...
    act( AT_MAGIC, "Expulsas a $N de $t", ch, htown->name, victim, TO_CHAR );
    act( AT_MAGIC, "$n expulsa a $N de $t", ch, htown->name, victim, TO_ROOM );
    act( AT_MAGIC, "$n te expulsa de from $t", ch, htown->name, victim, TO_VICT );
...
void do_htowns( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !VLD_STR( argument ) ) {
        send_to_char( "\r\n&WCiudades natales disponibles en los reinos de &R6 Dragones&W\r\n", ch );
        send_to_char
            ( "&c----------------------------------------------------------------------------\r\n",
              ch );
        send_to_char
            ( "\r\n&cNatal                Nación               Gobernador             General\r\n",
              ch );
        for ( htown = first_htown; htown; htown = htown->next )
