...
/****************************************************************************
y                                         * AFKMud Copyright 1997-2002 Alsherok. Contributors: Samson, Dwip, Whir,   *
...
void do_alias( CHAR_DATA *ch, char *argument )
{
    ...
    for ( p = argument; *p != '\0'; p++ ) {
        if ( *p == '~' ) {
            send_to_char( "Comando no aceptable, no se puede usar el carácter ~ .\r\n", ch );
            return;
        }
    }
    ...
    if ( !*arg ) {
        if ( !ch->pcdata->first_alias ) {
            send_to_char( "¡No tienes alias definidos!\r\n", ch );
            return;
        }
        pager_printf( ch, "%-20s son\r\n", "Alias" );
        for ( pal = ch->pcdata->first_alias; pal; pal = pal->next )
    ...
    if ( !*argument ) {
        if ( ( pal = find_alias( ch, arg ) ) != NULL ) {
            ...
            send_to_char( "Ese alias no existe.\r\n", ch );
        return;
    }
    ...
    if ( ( pal = find_alias( ch, arg ) ) == NULL ) {
        ...
        send_to_char( "Alias Creado.\r\n", ch );
    }
    else {
        if ( pal->cmd );
        STRFREE( pal->cmd );
        pal->cmd = STRALLOC( argument );
        send_to_char( "Alias modificado.\r\n", ch );
    }
}
...
bool check_alias( CHAR_DATA *ch, char *command, char *argument )
{
    ...
    if ( ch->cmd_recurse == -1 || ++ch->cmd_recurse > 50 ) {
        if ( ch->cmd_recurse != -1 ) {
            send_to_char( "No se ha podido avanzar en el proceso de comandos, demasiados recursos .\r\n", ch );
            ch->cmd_recurse = -1;
        }
        return FALSE;
    }
