...
const char             *const curr_types[] = {
    "ninguno", "oro", "plata", "bronce", "cobre"
};
...
void do_listcurrency( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !first_curr_index ) {
        send_to_char( "No hay monedas.\r\n", ch );
        return;
    }
    for ( currindex = first_curr_index; currindex; currindex = currindex->next_currindex ) {
        ch_printf( ch, "%-5d: %-41s Cobrar: %d%%\r\n", currindex->vnum, currindex->name,
                   currindex->charge );
        if ( !currindex->first_currency )
            send_to_char( " Este índice de monedas no tiene moneda.\r\n", ch );
        else
            for ( curr = currindex->first_currency; curr; curr = curr->next_currency )
...
void do_exchange( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !VLD_STR( arg1 ) || !VLD_STR( arg2 ) || !VLD_STR( arg3 ) ) {
        send_to_char( "&CSintaxis: intercambiar <cantidad> <moneda> <moneda>\r\n", ch );
        send_to_char( "&cValores\r\n", ch );
        send_to_char( "&CUna moneda de oro   = 10 monedas de plata\r\n", ch );
        send_to_char( "&CUna moneda de plata= 10 monedas de bronce\r\n", ch );
        send_to_char( "&CUna moneda de bronce= 10 moneda de cobre\r\n", ch );
        return;
    }
    value = atoi( arg1 );
    if ( value <= 0 ) {
        send_to_char( "usa un número superior a 0.\r\n", ch );
        return;
    }

    if ( str_cmp( arg2, "oro" ) && str_cmp( arg2, "plata" ) && str_cmp( arg2, "bronce" )
         && str_cmp( arg2, "cobre" ) ) {
        do_exchange( ch, ( char * ) "" );
        return;
    }
    if ( str_cmp( arg3, "oro" ) && str_cmp( arg3, "plata" ) && str_cmp( arg3, "bronce" )
         && str_cmp( arg3, "cobre" ) ) {
        do_exchange( ch, ( char * ) "" );
        return;
    }
     ...
     */
    if ( !str_cmp( arg2, "oro" ) && GET_MONEY( ch, CURR_GOLD ) < value ) {
        send_to_char( "No tienes tanta cantidad que cambiar.\r\n", ch );
        return;
    }
    else if ( !str_cmp( arg2, "plata" ) && GET_MONEY( ch, CURR_SILVER ) < value ) {
        send_to_char( "No tienes tanta cantidad que cambiar.\r\n", ch );
        return;
    }
    else if ( !str_cmp( arg2, "bronce" ) && GET_MONEY( ch, CURR_BRONZE ) < value ) {
        send_to_char( "No tienes tanta cantidad que cambiar.\r\n", ch );
        return;
    }
    else if ( !str_cmp( arg2, "cobre" ) && GET_MONEY( ch, CURR_COPPER ) < value ) {
        send_to_char( "No tienes tanta cantidad que cambiar.\r\n", ch );
        return;
    }
    ...
    else if ( !str_cmp( arg2, "plata" ) )
        exchangetype = CURR_SILVER;
    else if ( !str_cmp( arg2, "bronce" ) )
        exchangetype = CURR_BRONZE;
    else if ( !str_cmp( arg2, "cobre" ) )
        exchangetype = CURR_COPPER;

    if ( !str_cmp( arg3, "oro" ) )
        newtype = CURR_GOLD;
    else if ( !str_cmp( arg3, "plata" ) )
        newtype = CURR_SILVER;
    else if ( !str_cmp( arg3, "bronce" ) )
        newtype = CURR_BRONZE;
    else if ( !str_cmp( arg3, "cobre" ) )
        newtype = CURR_COPPER;
    ...
    if ( exchangetype == newtype ) {
        send_to_char( "Eso no tiene sentido ninguno.\r\n", ch );
        return;
    }
    ...
    ch_printf( ch, "Cambias %d monedas de %s por %d monedas de %s.\r\n", value, arg2, newvalue, arg3 );
    if ( change > 0 )
        ch_printf( ch, "Tu cambio de %d monedas de %s te ha sido devuelto.\r\n", change, arg2 );
    return;
}
