...
void do_skin( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_PKILL( ch ) && !IS_IMMORTAL( ch ) ) {
        send_to_char( "¡Deja eso para los asesinos!\n", ch );
        return;
    }
    if ( argument[0] == '\0' ) {
        send_to_char( "¿De qué cadáver deseas la piel?\r\n", ch );
        return;
    }
    ...
    if ( ( obj = get_eq_char( ch, WEAR_WIELD ) ) == NULL ) {
        send_to_char( "no posees un arma para hacer eso.\r\n", ch );
        return;
    }
    if ( corpse->item_type != ITEM_CORPSE_PC ) {
        send_to_char( "Solo puedes despellejar cuerpos de jugadores.\r\n", ch );
        return;
    }
    if ( obj->value[3] != 1 && obj->value[3] != 2 && obj->value[3] != 3 && obj->value[3] != 11 ) {
        send_to_char( "Ese cadáver no tiene piel.\r\n", ch );
        return;
    }
...
void learn_from_success( CHAR_DATA *ch, int sn )
{
    ...
    if ( xIS_SET( ch->act, PLR_AUTORECAST ) ) {
        if ( ch->pcdata->learned[sn] > 80 && ( number_range( 1, 10 ) > 6 ) )    // 40%
            // chance 
            // to do
            // noth$
            return;
        if ( ch->pcdata->learned[sn] > 60 && ( number_range( 1, 10 ) > 7 ) )    // 30%
            // chance 
            // to do
            // noth$
            return;
        if ( ch->pcdata->learned[sn] > 40 && ( number_range( 1, 10 ) > 8 ) )    // 20%
            // chance 
            // to do
            // not$
            return;
        if ( ch->pcdata->learned[sn] > 29 && ( number_range( 1, 10 ) > 9 ) )    // 10%
            // chance 
            // to do
            // not$
            return;
    ...
    if ( sn == gsn_forge || sn == gsn_tan || sn == gsn_bake || sn == gsn_mix ||
         sn == gsn_mine || sn == gsn_gather || sn == gsn_hunt || sn == gsn_jewelry
...
//   int luck;
    ...
    if ( sn == gsn_forge || sn == gsn_tan || sn == gsn_bake || sn == gsn_mix || sn == gsn_mine
         || sn == gsn_gather || sn == gsn_hunt || sn == gsn_jewelry || sn == gsn_mill
         || sn == gsn_fell ) {
...
void do_steal( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC( ch ) && !IS_NPC( victim ) ) {
        ...
        send_to_char( "The staff forbid theft between players.\r\n", ch );
        return;
    }
...
void do_kick( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' ) {
        if ( ch->fighting )
            ...
            send_to_char( "¿Dar una patada a quién?\r\n", ch );
            return;
        }
    }
    ...
    if ( IS_NPC( ch ) || can_use_skill( ch, number_percent(  ), gsn_kick ) ) {
        if ( chance == 1 ) {
            act( AT_PLAIN, "Intentas dar una patada frontal a $N.", ch, NULL, victim, TO_CHAR );
            act( AT_PLAIN, "¡$n te intenta dar una patada frontal!", ch, NULL, victim, TO_VICT );
            act( AT_PLAIN, "¡$n intenta dar una patada frontal a $N!", ch, NULL, victim, TO_NOTVICT );
        }
        else if ( chance == 2 ) {
            act( AT_PLAIN, "Lanzas una patada lateral a $N.", ch, NULL, victim, TO_CHAR );
            act( AT_PLAIN, "¡$n te lanza una patada lateral!", ch, NULL, victim, TO_VICT );
            act( AT_PLAIN, "¡$n lanza una patada lateral a $N!", ch, NULL, victim, TO_NOTVICT );
        }
        else if ( chance == 3 ) {
            act( AT_PLAIN, "Intentas patear a $N.", ch, NULL, victim, TO_CHAR );
            act( AT_PLAIN, "¡$n te intenta patear!", ch, NULL, victim, TO_VICT );
            act( AT_PLAIN, "¡$n intenta patear a $N!", ch, NULL, victim, TO_NOTVICT );
        }
        else if ( chance == 4 ) {
            act( AT_PLAIN, "Intentas darle una poderosa patada ascendente a $N.",
                 ch, NULL, victim, TO_CHAR );
            act( AT_PLAIN, "¡$n te intenta dar una patada ascendente!",
                 ch, NULL, victim, TO_VICT );
            act( AT_PLAIN, "¡$n intenta dar una patada ascendente a $N!",
                 ch, NULL, victim, TO_NOTVICT );
        }
        else if ( chance == 5 ) {
            act( AT_PLAIN, "Intentas darle una poderosa patada ascendente a $N.",
            ...
            act( AT_PLAIN, "Te colocas de lado para intentar dar una patada giratoria a $N.", ch,
                 NULL, victim, TO_CHAR );
            act( AT_PLAIN, "¡$n se coloca de lado para intentar darte una patada giratoria!", ch,
                 NULL, victim, TO_VICT );
            act( AT_PLAIN, "¡$n se coloca de lado para dar una patada giratoria a $N!", ch,
            ...
            act( AT_PLAIN,
                 "Levantas tu pierna con fuerza tratando de golpear la cara de $N.",
                 ch, NULL, victim, TO_CHAR );
            act( AT_PLAIN, "$n levanta la pierna con fuerza, tratando de golpear tu cara.",
                 ch, NULL, victim, TO_VICT );
            act( AT_PLAIN, "$n levanta la pierna con fuerza tratando de golpear a $N en la cara.",
                 ch, NULL, victim, TO_NOTVICT );
        }
...
void do_bite( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC( ch ) || can_use_skill( ch, number_percent(  ), gsn_bite ) ) {
        ...
        if ( IS_NPC( ch )
             || ( ch->pcdata->learned[gsn_crushing_jaw] > 0 && ch->race == RACE_ANIMAL ) ) {
            act( AT_PLAIN, "Your jaws crushing bite latches onto $N!", ch, NULL, victim, TO_CHAR );
            act( AT_PLAIN, "$n's jaws crushing bite latches onto you!", ch, NULL, victim, TO_VICT );
            act( AT_PLAIN, "$n's jaws crushing bite latches onto $N!", ch, NULL, victim,
                 TO_NOTVICT );
            short                   plaus;
...
void do_recall( CHAR_DATA *ch, char *argument )
{
    ...
    if ( xIS_SET( ch->act, PLR_MUSIC ) ) {
        if ( ch->in_room && !str_cmp( ch->in_room->area->filename, "paleon.are" )
             || !str_cmp( ch->in_room->area->filename, "middenir" ) )
...
void do_cityrecall( CHAR_DATA *ch, char *argument )
{
    ...
    if ( xIS_SET( ch->act, PLR_MUSIC ) ) {
        if ( ch->in_room && !str_cmp( ch->in_room->area->filename, "paleon" )
             || !str_cmp( ch->in_room->area->filename, "middenir" ) )
            send_to_char( "!!SOUND(sound/paleon.wav)\r\n", ch );
    }
    else if ( xIS_SET( ch->act, PLR_BATTLE ) )
...
bool check_dodge( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( can_use_skill( victim, number_percent(  ), gsn_dodge ) ) {
        if ( ( ( IS_NPC( victim ) && xIS_SET( victim->defenses, DFND_BLADEMASTER ) )
            ...
            help = number_range( 1, 100 );
            if ( !IS_NPC( victim ) )
            {
            if ( victim->pcdata->learned[gsn_dodge] < 40 && help > 85 ) {
                victim->pcdata->learned[gsn_dodge] += 1;
            }
             }
            if ( bm_chance > 13 ) {
...
void do_fly( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char( "Despliegas tus alas y te elevas en el aire.\r\n", ch );
    ...
    if ( !arg || arg[0] == '\0' ) {
        ...
        for ( morph = morph_start; morph; morph = morph->next ) {
            ...
            if ( !str_cmp( morph->short_desc, "a lady" ) ||
                 !str_cmp( morph->short_desc, "a peasant" ) ||
                 !str_cmp( morph->short_desc, "an elderly old man" ) ||
                 !str_cmp( morph->short_desc, "a soldier" ) ||
                 !str_cmp( morph->short_desc, "a merchant" ) )
                continue;
 ...
 * look for > 0 anyway, so shouldn't affect anything besides make for specialised msgs. */
        ...
        send_to_char
            ( "You hack up the head in a poor attempt to behead the corpse, leaving nothing worthwhile.\r\n",
              ch );
...
// Now let's make it look cool..
    ...
    bufptr = one_argument( corpse->description, name ); /* name = The, bufptr = corpse of 
                                                         * (name) lies here. */
    mudstrlcpy( name2, bufptr, MSL );                  /* name2 should be 'corpse of
                                                        * (name) lies here. */
...
void do_quivering_palm( CHAR_DATA *ch, char *argument )
{
    ...
    if ( can_use_skill( ch, number_percent(  ), gsn_quivering_palm ) ) {
        ...
        if ( !ch->fighting )
            global_retcode =
                damage( ch, victim, ( insane + get_curr_str( ch ) ), gsn_quivering_palm );
        else
...
void do_overhead_slash( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC( ch ) || can_use_skill( ch, number_percent(  ), gsn_overhead_slash ) ) {
        if ( ch->mount ) {
            act( AT_SKILL,
                 "Apon your mount you pull your reigns in and execute an overhead slash at $N!",
                 ch, NULL, victim, TO_CHAR );
            act( AT_SKILL,
                 "Apon $s mount $n's steed suddenly slows down and $n hits you with a overhead slash!",
                 ch, NULL, victim, TO_VICT );
            act( AT_SKILL,
                 "Apon $s mount $n's steed suddenly slows down and $n hits $N with a overhead slash!",
                 ch, NULL, victim, TO_NOTVICT );
        ...
        if ( ch->mount ) {
            act( AT_SKILL,
                 "Apon your mount you attempt to get your steed to slowdown, but fail to execute properly.",
                 ch, NULL, victim, TO_CHAR );
            act( AT_SKILL,
                 "Apon $s mount $n attempts to get $s steed to slowdown, but fails to do so.", ch,
                 NULL, victim, TO_VICT );
        }
        else {
            act( AT_SKILL,
                 "You attempt to execute a overhead slash but fail to hit $N.",
                 ch, NULL, victim, TO_CHAR );
            act( AT_SKILL,
                 "$n attempts to hit you with a overhead slash but misses you.", ch,
                 NULL, victim, TO_VICT );
        }
...
void do_downward_slash( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC( ch ) || can_use_skill( ch, number_percent(  ), gsn_downward_slash ) ) {
        if ( ch->mount ) {
            act( AT_SKILL,
                 "Apon your mount you downward slash at $N, slashing with your sword into $S body!",
                 ch, NULL, victim, TO_CHAR );
            act( AT_SKILL,
                 "Apon $s mount $n downward slashes at you, slashing $s sword into your body!",
                 ch, NULL, victim, TO_VICT );
            act( AT_SKILL,
                 "Apon $s mount $n downward slashes at $N, slashing the sword into $S body!",
                 ch, NULL, victim, TO_NOTVICT );
            global_retcode = damage( ch, victim, ludicrous, gsn_downward_slash );
        }
        else {
            act( AT_SKILL,
                 "You downwardslash with your sword into $S body!", ch, NULL, victim, TO_CHAR );
            act( AT_SKILL,
                 "$n downward slashes at you, slashing $s sword into your body!",
                 ch, NULL, victim, TO_VICT );
            act( AT_SKILL,
                 "$n downward slashes at $N, slashing the sword into $S body!",
                 ch, NULL, victim, TO_NOTVICT );
        ...
        if ( ch->mount ) {
            act( AT_SKILL,
                 "Apon your mount you wildly downward slash at $N, but $E dodges out of the way!",
                 ch, NULL, victim, TO_CHAR );
            act( AT_SKILL,
                 "Apon $s mount $n wildly downward slashes at you, but you quickly dodge out of $s way.",
                 ch, NULL, victim, TO_VICT );
            act( AT_SKILL,
                 "Apon $s mount $n wildly downward slashes at $N, but $E dodges out of the way!",
                 ch, NULL, victim, TO_NOTVICT );
        }
        else {
            act( AT_SKILL,
                 "You wildly downward slash at $N, but $E dodges out of the way!", ch,
                 NULL, victim, TO_CHAR );
            act( AT_SKILL,
                 "$n wildly downward slashes at you, but you quickly dodge out of $s way.",
                 ch, NULL, victim, TO_VICT );
            act( AT_SKILL,
                 "$n wildly downward slashes at $N, but $E dodges out of the way!", ch,
                 NULL, victim, TO_NOTVICT );
...
void do_backhand( CHAR_DATA *ch, char *argument )
{
    ...
    if ( can_use_skill( ch, number_percent(  ), gsn_backhand ) ) {
        ...
        global_retcode = damage( ch, victim, mediumhigh, gsn_backhand );
    }
    else {
        act( AT_SKILL,
...
void do_tackle( CHAR_DATA *ch, char *argument )
{
    ...
    if ( can_use_skill( ch, number_percent(  ), gsn_tackle ) ) {
        ...
        global_retcode = damage( ch, victim, medium, gsn_tackle );
    }
    else {
        act( AT_SKILL,
...
void do_ward( CHAR_DATA *ch, char *argument )
{
    ...
    if ( can_use_skill( ch, number_percent(  ), gsn_spectral_ward ) ) {
        ...
        af.level = ch->level;
        affect_to_char( victim, &af );
        victim->ward_dam = ( 200 + ch->level * 10 );
...
void do_vines( CHAR_DATA *ch, char *argument )
{
    ...
    if ( can_use_skill( ch, ( number_percent(  ) ), gsn_vines ) ) {
        ...
        act( AT_ORANGE, "$n tangles $N's feet into the ground with vines.", ch, NULL, victim,
             TO_ROOM );
    }
    else {
        learn_from_failure( ch, gsn_vines );
        send_to_char
            ( "&OYou attempt to tangle your victim with vines, but fail to summon them fast enough.\r\n",
              ch );
    }
    return;
}
...
void do_control( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_AFFECTED( victim, AFF_CHARM )
         || schance == 1000 || IS_AFFECTED( ch, AFF_CHARM )
    ...
    if ( can_use_skill( ch, number_percent(  ), gsn_control ) ) {
        ...
        act( AT_PLAIN, "You cannot concentrate enough to control someone.", ch,
             NULL, NULL, TO_CHAR );
