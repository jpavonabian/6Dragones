...
void do_mpgenmob( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !str_cmp( ch->in_room->area->filename, "psewer.are" )
         || !str_cmp( ch->in_room->area->filename, "dsewer.are" ) )
        vnum = 8722;
    else {
...
void do_mptrade( CHAR_DATA *ch, char *argument )
{
    ...
    if ( victim->race == RACE_DRAGON ) {
        send_to_char
            ( "&cThe city manager is shaking too hard in fear that a dragon is presentto think of trades.\r\n",
              victim );
        return;
    }
