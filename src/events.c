...
WOLLA( event_doubleexp )
{
    ...
    if ( ( !eventexp ) && ( chance( 5 ) ) ) {
        ...
        eventexp = FALSE;
        echo_to_all( AT_RED, "[&WEvento&R] &CSe acabó la doble experiencia.\r\n", ECHOTAR_ALL );
    }
}
...
WOLLA( event_happyhour )
{
    ...
    if ( ( !happyhouron ) && ( chance( 3 ) ) ) {
        ...
        !happyhouron;
        echo_to_all( AT_RED, "[&Wevento&R] &CSe acabó la hora feliz.\r\n", ECHOTAR_ALL );
        return;
    }
}
    ...
    for ( d = first_descriptor; d; d = d->next ) {
        if ( d->connected == CON_PLAYING
            ...
            if ( !IS_NPC( ch ) && !xIS_SET( ch->act, PLR_AFK ) ) {
                send_to_char
                    ( "&Y¡La administración de 6 Dragones te da las gracias por jugar!\r\n\r\n",
                      ch );
                ...
                if ( xIS_SET( ch->act, PLR_RP ) && chance( 60 ) ) {
                    ch_printf( ch,
                               "&W¡Ganas 5 puntos de gloria por tu participación en el rol!&D\r\n" );
                    ch->quest_curr += 5;
                }
