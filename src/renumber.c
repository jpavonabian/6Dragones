...
void                    warn_in_prog( CHAR_DATA *ch, int low, int high, const char *where, int vnum,
...
/* this function actualy gathers all the renumber data for an area */
{
    ...
    r_area->r_mob =
        gather_one_list( REN_MOB, area->low_m_vnum, area->hi_m_vnum, new_base, fill_gaps, &max );
    r_area->low_mob = new_base;
    r_area->hi_mob = max;

    r_area->r_obj =
        gather_one_list( REN_OBJ, area->low_o_vnum, area->hi_o_vnum, new_base, fill_gaps, &max );
    r_area->low_obj = new_base;
    r_area->hi_obj = max;

    r_area->r_room =
        gather_one_list( REN_ROOM, area->low_r_vnum, area->hi_r_vnum, new_base, fill_gaps, &max );
...
void renumber_area( CHAR_DATA *ch, AREA_DATA *area, RENUMBER_AREA * r_area, bool area_is_proto,
                    bool verbose )
   ...
   to the renumber data in "r_area". "ch" is to show messages. */
{
    ...
    for ( r_data = r_area->r_room; r_data; r_data = r_data->next ) {
        ...
        if ( room_index_hash[iHash] == room )
            ...
            for ( room_prev = room_index_hash[iHash]; room_prev && room_prev->next != room;
    ...
    for ( r_data = r_area->r_mob; r_data; r_data = r_data->next ) {
        ...
        if ( mob_index_hash[iHash] == mob )
            ...
            for ( mob_prev = mob_index_hash[iHash]; mob_prev && mob_prev->next != mob;
                  mob_prev = mob_prev->next );
            if ( mob_prev == NULL ) {
    ...
    for ( r_data = r_area->r_obj; r_data; r_data = r_data->next ) {
        ...
        if ( obj_index_hash[iHash] == obj )
            ...
            for ( obj_prev = obj_index_hash[iHash]; obj_prev && obj_prev->next != obj;
    ...
    for ( i = area->low_o_vnum; i <= area->hi_o_vnum; i++ ) {
        ...
        if ( obj->item_type == ITEM_CONTAINER ) {
            ...
            if ( IS_SET( obj->value[0], TRIG_TELEPORT )
                 ...
                 || IS_SET( obj->value[0], TRIG_RAND4 ) || IS_SET( obj->value[0], TRIG_RAND6 )
            ...
            *p = cTmp;
            if ( num >= low && num <= high ) {
                pager_printf( ch,
                              ...
                              find_translation( num, r_area->r_room ), find_translation( num,
                                                                                         r_area->
                                                                                         r_obj ),
