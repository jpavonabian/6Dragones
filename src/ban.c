...
void fread_ban( FILE * fp, int type )
{
    ...
    if ( type == BAN_SITE ) {                          /* Sites have 2 extra numbers
                                                        * written out */
    ...
    if ( type == BAN_RACE || type == BAN_CLASS ) {
        if ( fMatch )
            pban->flag = i;
        else {                                         /* The file is corupted throw out
                                                        * * * * * this ban structure */
    ...
    else {                                             /* Bad type throw out the ban * *
                                                        * * * structure */
...
void save_banlist( void )
{
    ...
    for ( pban = first_ban; pban; pban = pban->next ) {
        fprintf( fp, "SITE\n" );
        fprintf( fp, "%s~\n", pban->name );
        fprintf( fp, "%d %d %d %d %d %d\n", pban->level, pban->duration, pban->unban_date,
                 pban->prefix, pban->suffix, pban->warn );
...
void do_allow( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' || arg2[0] == '\0' )
        goto syntax_message;
    if ( arg2[0] == '#' ) {                            /* Use #1 to ban the first ban in
                                                        * * the list specified */
        temp = arg2;
        temp++;
        if ( !is_number( temp ) ) {
        ...
        case SUB_NONE:
            {
                ...
                smash_tilde( arg );                    /* Make sure the immortals don't * 
                                                        * put a ~ in it. */
                    ...
                    case BAN_SITE:
                        {
                            ...
                            if ( !name ) {             /* Double check to make sure name
                                                        * * isnt null */
                                /*
        ...
        case BAN_SITE:
            ...
            for ( bnum = 1; pban; pban = pban->next, bnum++ ) {
                pager_printf( ch, "[%2d] %-4s (%2d) %-24s %-15s %4d  %c%s%c\r\n",
                              bnum, ( pban->warn ) ? "YES" : "no", pban->level, pban->ban_time,
                              pban->ban_by, pban->duration, ( pban->prefix ) ? '*' : ' ',
    ...
    for ( ; pban; pban = pban->next ) {
        if ( type == BAN_CLASS && pban->flag == ch->Class ) {
            ...
            if ( ch->level > pban->level ) {
                if ( pban->warn ) {
                    snprintf( buf, MSL, "%s class logging in from %s.", pban->name,
                              ch->desc->host );
        ...
        if ( type == BAN_SITE ) {
            ...
            if ( fMatch ) {
                ...
                if ( ch->level > pban->level ) {
                    if ( pban->warn ) {
                        snprintf( buf, MSL, "%s logging in from site %s.", ch->name,
                                  ch->desc->host );
                        log_string_plus( buf, LOG_WARN, sysdata.log_level );
