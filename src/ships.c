...
CHAR_DATA              *find_shipwright( CHAR_DATA *ch )
{
    ...
    for ( shipwright = ch->in_room->first_person; shipwright;
          shipwright = shipwright->next_in_room )
...
void do_hull_repair( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->pcdata->hull == 100 ) {
        snprintf( buf, MSL, "say %s, your ship hull looks like it is at 100 percent already.",
                  ch->name );
    ...
    if ( ( GET_MONEY( ch, CURR_SILVER ) < cost ) ) {
        snprintf( buf, MSL,
                  "say %s, you don't have enough silver to get your hull repaired you need %d silver.",
                  ch->name, cost );
        interpret( shipwright, buf );
        return;
    }
...
void do_deed( CHAR_DATA *ch, char *argument )
{
    ...
    if ( obj->owner == NULL ) {
        ...
        obj->owner = STRALLOC( victim->name );
            ...
            snprintf( buf, MSL, "%s's ship deed", victim->name );
...
void do_depart( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !str_cmp( argument, "abandon" ) ) {
        ...
        send_to_char
            ( "You throw caution to the wind, and abandon ship jumping off the gang plank!\r\n",
              ch );
        act( AT_CYAN, "$n runs up to the gang plank and jumps off into the water!", ch, NULL, NULL,
             TO_ROOM );
...
// Check to see in view water
    ...
    send_to_char
        ( "You salute the Officer of the Day first and say, 'I request permission  to  leave\r\nthe  ship,  sir/maâ€™am.'  After receiving  permission, you then\r\nface and salute the ensign and depart!\r\n",
          ch );
    act( AT_CYAN,
         "$n salutes the Officer of the Day requesting permission to leave the ship.  $n then salutes the ensign and walks off the ship.",
...
// Is there a dock?
    ...
    if ( ch->pcdata->hull == 0 ) {
        send_to_char
            ( "Your ship is wrecked, first you will have to use the hull command to get it repaired.\r\n",
              ch );
        return;
    }
...
// Check to see if player has the deed to a ship
    ...
    for ( deed = ch->first_carrying; deed; deed = deed->next_content ) {
        if ( deed->pIndexData->vnum == OBJ_VNUM_DEED && deed->owner
    ...
    if ( str_cmp( argument, "enter" ) && str_cmp( argument, "leave" )
         && str_cmp( argument, "setsail" )
         && str_cmp( argument, "dropanchor" ) ) {
    ...
    if ( !str_cmp( argument, "setsail" ) ) {
        ...
        act( AT_CYAN, "Several deck hands spring into action raising the anchor and sails.", ch,
             NULL, NULL, TO_CHAR );
    ...
    if ( !str_cmp( argument, "dropanchor" ) ) {
        ...
        act( AT_CYAN, "Several deck hands spring into action lowering the anchor and sails.", ch,
             NULL, NULL, TO_CHAR );
...
// Check for owner of ship
    ...
    if ( ch->in_room->vnum != ch->pcdata->watervnum ) {
        send_to_char
            ( "You must use the view water command before you can steer the ship any direction.\r\n",
              ch );
        return;
    }
    ...
    if ( str_cmp( argument, "north" ) && str_cmp( argument, "n" ) && str_cmp( argument, "east" )
         && str_cmp( argument, "e" )
         && str_cmp( argument, "west" ) && str_cmp( argument, "w" ) && str_cmp( argument, "south" )
         && str_cmp( argument, "s" ) ) {
        send_to_char( "Proper syntax steer direction\r\n", ch );
        return;
    }

    act( AT_CYAN, "You bark out an order, to trim the sails and turn the rudder!", ch, NULL, NULL,
         TO_CHAR );

    if ( !str_cmp( argument, "north" ) || !str_cmp( argument, "n" ) ) {
        act( AT_CYAN, "The exec officer yells, 'Sheering to the north, Captain!'", ch, NULL, NULL,
             TO_CHAR );
        act( AT_CYAN, "The exec officer hollars at the crew, 'Sheer to the north, double quick!'",
    ...
    if ( !str_cmp( argument, "east" ) || !str_cmp( argument, "e" ) ) {
        act( AT_CYAN, "The exec officer yells, 'Sheering to the east, Captain!'", ch, NULL, NULL,
             TO_CHAR );
        act( AT_CYAN, "The exec officer hollars at the crew, 'Sheer to the east, double quick!'",
             ch, NULL, NULL, TO_CHAR );
    ...
    if ( !str_cmp( argument, "west" ) || !str_cmp( argument, "w" ) ) {
        act( AT_CYAN, "The exec officer yells, 'Sheering to the west, Captain!'", ch, NULL, NULL,
             TO_CHAR );
        act( AT_CYAN, "The exec officer hollars at the crew, 'Sheer to the west, double quick!'",
             ch, NULL, NULL, TO_CHAR );
    ...
    if ( !str_cmp( argument, "south" ) || !str_cmp( argument, "s" ) ) {
        act( AT_CYAN, "The exec officer yells, 'Sheering to the south, Captain!'", ch, NULL, NULL,
             TO_CHAR );
        act( AT_CYAN, "The exec officer hollars at the crew, 'Sheer to the south, double quick!'",
             ch, NULL, NULL, TO_CHAR );
