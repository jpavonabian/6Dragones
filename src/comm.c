...
/*   Removed causes crashes when cleanup_memory called           if ( VLD_STR( title_table[hash][loopa][2] ) )
...
void accept_new( int ctrl )
{
    ...
    if ( FD_ISSET( ctrl, &exc_set ) ) {
        bug( "%s Exception raise on controlling descriptor %d", __FUNCTION__, ctrl );
        FD_CLR( ctrl, &in_set );
        FD_CLR( ctrl, &out_set );
    }
...
/*  time_t  last_check = 0;  */
    ...
    while ( !mud_down ) {
        ...
        for ( d = first_descriptor; d; d = d_next ) {
            if ( d == d->next ) {
                bug( "%s descriptor_loop: loop found & fixed", __FUNCTION__ );
                d->next = NULL;
            }
            d_next = d->next;
            ...
            else if ( d->idle > 10 && d->character && d->character->pcdata
                      && xIS_SET( d->character->act, PLR_AUTORECAST )
                      && d->character->pcdata->recast == 1 ) {
                d->idle = 0;
                d->character->pcdata->recast = 0;
                if ( d->character->pcdata->recastsn != 0 ) {
                    do_cast( d->character, skill_table[d->character->pcdata->recastsn]->name );
                    continue;
                }
            }
...
// Input by Volk - command clearer
    if ( str_has_command( d->inbuf, "clear" ) ) {
        d->inbuf[0] = '\0';
        send_to_char( "Cola de comandos limpiada.\r\n", d->character );
        return;
    }
    ...
    for ( i = 0, k = 0; d->inbuf[i] != '\n' && d->inbuf[i] != '\r'; i++ ) {
        if ( k >= MAX_INBUF_SIZE ) {
             ...
             */
            /*
             * for ( ; d->inbuf[i] != '\0' || i>= MAX_INBUF_SIZE ; i++ ) { if (
             * d->inbuf[i] == '\n' || d->inbuf[i] == '\r' ) break; } 
             */
            d->inbuf[i] = '\n';
            d->inbuf[i + 1] = '\0';
            ...
            switch ( *str ) {
                ...
                case 'e':
                    if ( ch->sex > 2 || ch->sex < 0 ) {
                        ...
                        i = should_upper ?
                            !can_see( to,
                                      ch ) ? "It" : capitalize( he_she[URANGE( 0, ch->sex, 2 )] ) :
                ...
                case 'E':
                    if ( vch->sex > 2 || vch->sex < 0 ) {
                        ...
                        i = should_upper ?
                            !can_see( to,
                                      vch ) ? "It" : capitalize( he_she[URANGE( 0, vch->sex, 2 )] )
                ...
                case 'm':
                    if ( ch->sex > 2 || ch->sex < 0 ) {
                        ...
                        i = should_upper ?
                            !can_see( to,
                                      ch ) ? "It" : capitalize( him_her[URANGE( 0, ch->sex, 2 )] ) :
                ...
                case 'M':
                    if ( vch->sex > 2 || vch->sex < 0 ) {
                        ...
                        i = should_upper ?
                            !can_see( to,
                                      vch ) ? "It" : capitalize( him_her[URANGE( 0, vch->sex, 2 )] )
                ...
                case 'p':
                    if ( !obj1 ) {
                        ...
                        i = should_upper ? ( ( !to || can_see_obj( to, obj1 ) ) ?
                                             capitalize( obj_short( obj1 ) ) : "Something" )
                            : ( ( !to
                                  || can_see_obj( to, obj1 ) ) ? obj_short( obj1 ) : "something" );
                    break;
                ...
                case 'P':
                    if ( !obj2 ) {
                        ...
                        i = should_upper ? ( !to
                                             || can_see_obj( to,
                                                             obj2 ) ?
                                             capitalize( obj_short( obj2 ) ) : "Something" )
                ...
                case 's':
                    if ( ch->sex > 2 || ch->sex < 0 ) {
                        ...
                        i = should_upper ?
                            !can_see( to,
                                      ch ) ? "It" : capitalize( his_her[URANGE( 0, ch->sex, 2 )] ) :
                ...
                case 'S':
                    if ( vch->sex > 2 || vch->sex < 0 ) {
                        ...
                        i = should_upper ?
                            !can_see( to,
                                      vch ) ? "It" : capitalize( his_her[URANGE( 0, vch->sex, 2 )] )
          ...
          int type )
{
    ...
    for ( ; to;
        ...
        if ( type == TO_NOTVICT && ( to == ch || to == vch ) )
            continue;
        if ( type == TO_CANSEE && ( to == ch || ( !IS_IMMORTAL( to ) && !IS_NPC( ch )
